        ...
        cs.only_lock_reg_coremode_updated(*old(cs), set![], set![spec_PT_lockid()]),
{
    ...
    if !shared {
        ...
        let attr = RmpAttr::empty().set_vmpl(RICHOS_VMPL as u64).set_perms(entry.osperm.into()).set_vmsa(0);
        proof {
...
#[verifier(external_body)]
fn clear_kern_if_private(entry: OSMemEntry, Tracked(cs): Tracked<&mut SnpCoreSharedMem>) -> (ret: OSMemEntry)
requires
    entry.wf(),
ensures
    entry.wf(),
{
        ...
        ),
    {
        ...
        let check = if tmp_start < 0x1000 {Some(true)} else {check_is_encrypted(tmp_start.to_addr(), Tracked(&page_perm), Tracked(cs))};
        match check {
            Some(encrypted) => {
                if encrypted {
                    rmpadjust_check(tmp_start.to_addr() as u64, attr, Tracked(&mut cs.snpcore), Tracked(&mut page_perm));
                } else {
                }
            }
            None => {
