...
impl GuestPTRam {
    ...
    pub proof fn proof_pte_addr_must_in_ptram(
        ...
        requires
            ...
            self.map_entry_gpa(sysmap, memid, gvn, lvl) is Some,
            map_gpa === self.map_entry_gpa(sysmap, memid, gvn, lvl)->Some_0,
        ensures
            sysmap.translate(map_gpa.to_page()) is Some,
            self.spec_ram().rmp.dom().contains(sysmap.translate(map_gpa.to_page())->Some_0),
    {
        ...
        assert(memtype(memid, map_gpa.to_page()) is PTE);
    }
    ...
    {
        assert forall |gvn|
            self.to_mem_map(sysmap, memid).db[gvn] is Some
        implies
            ...
            self.to_mem_map(sysmap, memid).is_identity_map(),
    {
        ...
        assert forall|gvn: GVN|
            gvn.is_valid() && (#[trigger] memmap.translate(gvn)) is Some implies memmap.translate(
            gvn,
        )->Some_0.as_int() == gvn.as_int() by {
            assert(self.map_entry(sysmap, memid, gvn, PTLevel::L0) is Some);
            let ppn = self.map_entry(sysmap, memid, gvn, PTLevel::L0)->Some_0.spec_ppn();
            assert(ppn == memmap.translate(gvn)->Some_0);
    ...
    pub proof fn lemma_map_entry_gpa_valid(
        ...
        requires
            ...
            self.map_entry_gpa(sysmap, memid, gvn, lvl) is Some,
        ensures
            self.map_entry_gpa(sysmap, memid, gvn, lvl)->Some_0.to_page().is_valid(),
    {
    }
            ...
            self.map_entry_gpa_ok(memid, gvn, lvl)->Some_0.to_page().is_valid(),
    {
        ...
        assert(memtype(memid, pte_gpa_ok->Some_0.to_page()).is_pt(lvl)) by {
            reveal(GuestPTRam::inv_content_ok);
            ...
            match lvl.parent_lvl() {
                Option::Some(next_lvl) => {
                    ...
                    assert(prev_pte is Some);
                    let prev_pte = prev_pte->Some_0;
                    prev_pte.lemma_each_table_is_one_page(idx);
                    assert(pte_gpa_ok->Some_0.to_page() === prev_pte.spec_ppn());
                    assert(self.inv_content_gpa_ok(memid, gvn));
                },
                Option::None => {
                    l0_entry.lemma_each_table_is_one_page(idx);
                    assert(pte_gpa_ok->Some_0.to_page() === l0_entry.spec_ppn());
                },
            }
        }
    }
    ...
    pub proof fn proof_memop_inv(
        ...
        requires
            ...
            //old_pt.spec_ram().op(sysmap, memop) is Ok,
            memop.to_memid().is_sm(memid) ==> old_pt.spec_ram().gpmemop_requires(
                memop,
                sysmap,
            ),
    //memop is Write ==> Self::write_pt_requires(&old_pt.spec_ram(), memop.to_addr_memid(), memop->Write_1, memop->Write_2, sysmap)
    ...
    proof fn lemma_write_pte_inv_ppn_enc(
        ...
        requires
            ...
            //Self::write_pt_requires(&old_pt.spec_ram(), memop.to_addr_memid(), memop->Write_1, memop->Write_2, sysmap),
            ...
            new_pt.map_entry_ok(memid, gvn, lvl) is Some,
            old_pt.spec_ram().op(sysmap, memop) is Ok,
        ensures
            old_pt.map_entry_exe_ok(memid, gvn, lvl) is Some,
            (old_pt.need_c_bit(memid, gvn) && lvl is L0) ==> new_pt.map_entry_ok(
                memid,
                gvn,
                lvl,
            )->Some_0.is_encrypted(),
            new_pt.map_entry_ok(memid, gvn, lvl)->Some_0.spec_ppn() === old_pt.map_entry_ok(
                memid,
                gvn,
                lvl,
            )->Some_0.spec_ppn(),
    {
        ...
        let write_pte: GuestPTEntry = stream_to_data(memop->Write_2);
        let old_pte_gpa = old_pt.map_entry_gpa_ok(memid, gvn, lvl);
        let old_gpn = old_pt.map_entry_ok(memid, gvn, lvl)->Some_0.spec_ppn();
        assert(memtype(memid, old_pte_gpa->Some_0.to_page()).is_pt(lvl)) by {
            old_pt.lemma_map_entry_gpa_is_pte_type(memid, gvn, lvl);
        }
        let old_pte = old_pt.map_entry_exe_ok(memid, gvn, lvl);
        if old_pte_gpa->Some_0 === wgpmem && memop.to_memid().is_sm(memid)
            && memop.to_memid().to_asid() == memid.to_asid() {
            assert(old_pte is Some);
            assert(old_pte === old_pt.spec_ram().get_enc_data_ok::<GuestPTEntry>(
                AddrMemID { range: wgpmem, memid },
            ));
            assert(old_pt.spec_ram().op(sysmap, memop) is Ok);
            assert(old_pte is Some);
    ...
    requires
        ...
        new_pt.l0_entry(memid) === old_pt.l0_entry(memid),
        new_pt === &old_pt.spec_set_ram(old_pt.spec_ram().op(sysmap, memop).to_result()),
        new_pt.map_entry_exe_ok(memid, gvn, lvl) is Some,
        old_pt.spec_ram().op(sysmap, memop) is Ok,
    ensures
        old_pt.map_entry_exe_ok(memid, gvn, lvl) is Some,
        ({
            ...
            new_pt.map_entry_exe_ok(memid, gvn, lvl)->Some_0 === old_pt.map_entry_exe_ok(memid, gvn, lvl)->Some_0
        },
        ({
            &&& old_pt.map_entry_gpa_ok(memid, gvn, lvl)->Some_0 === memop.to_mem()
            &&& memop.to_memid().is_sm(memid)
            ...
            new_pt.map_entry_exe_ok(memid, gvn, lvl)->Some_0.view().spec_ppn() === old_pt.map_entry_exe_ok(memid, gvn, lvl)->Some_0.view().spec_ppn()
        }
    decreases
        lvl.as_int(),
    {
        ...
        if let MemOp::Write(_, enc, data) = memop {
            ...
            assert(old_pt.map_entry_gpa_ok(memid, gvn, lvl) is Some);
            let old_pte_gpa = old_pte_gpa->Some_0;
            assert(memtype(memid, old_pte_gpa.to_page()) is PTE) by {
                old_pt.lemma_map_entry_gpa_is_pte_type(memid, gvn, lvl);
            }
    ...
    proof fn lemma_safe_write(
        ...
        requires
            ...
            old_pt.ram.op_write(gpa_id, enc, data, sysmap) is Ok,
            ...
            new_pt.inv(memid),
    {
        ...
        assert(new_pt.inv_for_identity_map_ok(memid)) by {
            ...
            #[trigger] new_pt.map_entry_ok(memid, gvn, MAX_PT_LEVEL))->Some_0.spec_ppn().value()
                ...
                assert(old_pt.map_entry_ok(memid, gvn, MAX_PT_LEVEL)->Some_0.spec_ppn().value()
                    === gvn.value());
            }
        }
        assert(new_pt.inv_encrypted_priv_mem_ok(memid)) by {
            ...
            assert forall|gvn: GVN|
                ...
                let pte_gpa = new_pt.map_entry_gpa_ok(memid, gvn, MAX_PT_LEVEL)->Some_0;
                Self::lemma_write_pte_inv_ppn_enc(
                    old_pt,
                    new_pt,
                    sysmap,
                ...
                assert(old_pt.map_entry_ok(memid, gvn, MAX_PT_LEVEL)->Some_0.is_encrypted())
            }
        }
        assert forall|gvn: GVN| gvn.is_valid() implies #[trigger] new_pt.inv_content_gpa_ok(
            ...
            assert forall|lvl: PTLevel|
                ...
                assert(old_pt.map_entry_ok(memid, gvn, lvl)->Some_0.spec_ppn()
                    === new_pt.map_entry_ok(memid, gvn, lvl)->Some_0.spec_ppn());
            }
        }
        assert(new_pt.spec_ram().inv_sw(memid));
    ...
    pub proof fn lemma_pgtb_walk_addrs_recursive_any_sysmap(
        ...
        requires
            self.pgtb_walk_addrs_recursive(sysmap, memid, gvn, lvl) is Some,
            self.spec_ram().inv_sw(memid),
            gvn.is_valid(),
        ensures
        ...
        decreases lvl.as_int(),
    {
        ...
        match lvl.parent_lvl() {
            ...
            Option::Some(next_lvl) => {
                ...
                assert(next_pte_gpmem is Some);
                let next_pte_gpmem = next_pte_gpmem->Some_0;
    ...
    pub proof fn lemma_map_entry_gpa_model1_eq(
        ...
        ensures
            self.map_entry_gpa_ok(memid, gvn, lvl) is Some ==> self.map_entry_gpa_ok(
                memid,
                gvn,
                lvl,
            ) === other.map_entry_gpa_ok(memid, gvn, lvl),
            other.map_entry_gpa_ok(memid, gvn, lvl) is None ==> self.map_entry_gpa_ok(
                memid,
                gvn,
                lvl,
            ) is None,
    ...
    pub proof fn lemma_map_entry_model1_eq(
        ...
        requires
            ...
            self.model1_eq(other, memid),
        ensures
            self.map_entry_ok(memid, gvn, lvl) is Some ==> self.map_entry_ok(memid, gvn, lvl)
                === other.map_entry_ok(memid, gvn, lvl),
            other.map_entry_ok(memid, gvn, lvl) is None ==> self.map_entry_ok(
                memid,
                gvn,
                lvl,
            ) is None,
    {
        ...
        if self.map_entry_ok(memid, gvn, lvl) is Some {
            self.lemma_map_entry_gpa_ok_valid(memid, gvn, lvl);
            assert(pte_gpa is Some);
            assert(pte_gpa === pte_gpa2);
            let pte_gpa = pte_gpa->Some_0;
            let pte_gpa2 = pte_gpa2->Some_0;
            assert(memtype(memid, pte_gpa2.to_page()) is PTE) by {
    ...
    pub proof fn lemma_map_entry_any_sysmap(
        ...
        requires
            gvn.is_valid(),
            self.map_entry(sysmap, memid, gvn, lvl) is Some,
            self.spec_ram().inv_sw(memid),
        ensures
