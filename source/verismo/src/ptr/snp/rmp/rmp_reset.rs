...
verus!{
        pub open spec fn spec_reset_perm_at(perm: SnpPointsToRaw, old_perm: SnpPointsToRaw, vmpl: nat) -> bool {
        ...
        invariant
                ...
                snpcore.inv(),
                forall |i| 0 < i < vmpl ==>
                        #[trigger]spec_reset_perm_at(*perm, old_perm, i),
        {
                let rmp_attr = RmpAttr::empty().set_vmpl(vmpl as u64).set_perms(0);
                rmpadjust(page.to_addr() as u64, RMP_4K, rmp_attr, Tracked(snpcore), Tracked(None), Tracked(perm));
                assert forall |i: int| 0 < i < vmpl
                implies
                #[trigger]spec_reset_perm_at(*perm, old_perm, i as nat)
                by {
                        assert(perm@.snp().rmp@.perms[VMPL::from_int(i)]  =~~= PagePerm::empty())
                }
        }
