//! Specifications for external, unverified library surface, vendored from
//! COCONUT-SVSM's `verify_external` crate: only the items this crate uses.
//!
//! Trusted: these are specifications for code Verus does not verify, so they are
