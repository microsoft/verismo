...
//! decide, so it is assumed. Every such step lives in
//! [`points_to_axiom`](super::points_to_axiom), and nothing else in this crate
...
impl<T> GeneralPointsTo<T> {
    ...
    /// Everything a borrow must leave alone: a borrow is a window on the value,
    /// not an opportunity to come back as a permission to *different* memory.
    /// See [`GeneralPointsTo::borrow_mut`], which is where this is needed.
