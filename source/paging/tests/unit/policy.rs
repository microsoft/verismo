...
fn exclusive_canonical_boundary_does_not_include_the_kernel_half() {
    fn check<const START: usize, const END: usize>(root: PageLevel) {
        ...
        let policy = UserPolicy::<RootComplement<CoveredRange<START, END>>>::new();
        assert!(policy.owns_range(root, low, high));
        assert!(policy.owns_range(root, high, high));
        assert!(!policy.owns_range(root, low, high + 4096));
...
fn ranges_cannot_hide_protected_indexes_by_wrapping_a_small_root() {
    ...
    let policy = UserPolicy::<RootComplement<CoveredRange<1, 2>>>::new();
    assert!(!policy.owns_range(root, VirtAddr::from(0usize), VirtAddr::from(span)));
    assert!(policy.owns_range(root, VirtAddr::from(span - size), VirtAddr::from(span + size)));
    assert_eq!(
...
fn every_reserved_entry_is_immutable_and_non_owned() {
    let policy = UserPolicy::<RootComplement<CoveredRange<1, 3>>>::new();
...
fn empty_reserved_set_allows_every_entry() {
    let policy = UserPolicy::<AllRootEntries>::new();
...
fn unions_protect_each_disjoint_range_and_own_the_gaps() {
    type Reserved = RootComplement<RootUnion<CoveredRange<1, 3>, CoveredRange<7, 9>>>;
