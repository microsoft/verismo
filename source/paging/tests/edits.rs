...
#[test]
fn entry_shape_is_independent_of_presence() {
    let empty = Entry::empty();
    assert!(!empty.is_leaf(SMALL_LEVEL));
    assert!(!empty.is_present_leaf(SMALL_LEVEL));
    assert!(!empty.is_table(LARGE_LEVEL));
    assert!(!empty.is_present_table(LARGE_LEVEL));

    let table = Entry::new(PhysAddr::from(FRAME), PTEntryFlags::empty());
    assert!(table.is_table(LARGE_LEVEL));
    assert!(!table.is_present_table(LARGE_LEVEL));

    let leaf = Entry::new(PhysAddr::from(FRAME), PTEntryFlags::HUGE);
    assert!(leaf.is_leaf(LARGE_LEVEL));
    assert!(!leaf.is_present_leaf(LARGE_LEVEL));
}
...
unsafe fn seed_pat(root: PhysAddr, address: VirtAddr) {
    ...
    let entry = unsafe { load_entry(pte) };
    assert!(entry.is_present_leaf(level));
...
fn architecture_policy_can_require_break_before_make_for_a_split() {
    ...
    map_at!(table, base, FRAME.into(), LARGE_LEVEL, old_flags(), false).unwrap();
    let (pte, level) = unsafe { leaf_pte(table.root_paddr(), base) };
    assert_eq!(level, LARGE_LEVEL);
    ...
    split_at!(table, base + PAGE, LARGE_LEVEL, true).unwrap().expect_no_flush();
    clear_flush_hook();
...
fn architecture_bbm_range_breaks_only_structural_boundaries() {
    ...
    set_flush_hook(move |_, _| {
        ...
        for (index, pte) in entries.iter().copied().enumerate() {
            let entry = unsafe { load_entry(pte as *const BbmEntry) };
            if index == 1 {
                assert!(entry.is_present_leaf(HUGE_LEVEL));
            } else if entry.present() {
                assert!(entry.is_present_leaf(HUGE_LEVEL) || entry.is_present_table(HUGE_LEVEL));
            } else {
                invalid += 1;
            }
        }
...
fn two_level_range_can_split_both_boundaries_before_updating() {
    ...
    let (result, flush) = table.set_flags_range(start, end, new_flags(), true);
...
macro_rules! edit_tests {
    ($module:ident, $fixture:ident) => {
        mod $module {
            ...
            #[test]
            fn same_size_protection_preserves_mapping_attributes() {
                ...
                for (index, level) in [SMALL_LEVEL, LARGE_LEVEL, HUGE_LEVEL].into_iter().enumerate()
                {
                    for shared in [false, true] {
                        ...
                        map_at!(table, address, PhysAddr::from(FRAME), level, old_flags(), shared)
                            .unwrap();
                        ...
                        for requested in [
                            ...
                            let flush = set_flags_at!(table, address, level, requested, true).unwrap();
                            assert_flush_covers(
                                flush,
                                address.bits(),
            ...
            fn protecting_a_subpage_of_a_huge_leaf_changes_only_that_page() {
                ...
                for (index, level) in [LARGE_LEVEL, HUGE_LEVEL].into_iter().enumerate() {
                    ...
                    map_at!(table, VirtAddr::from(base), PhysAddr::from(FRAME), level, old_flags(), false)
                        .unwrap();
                    // SAFETY: the fixture has no concurrent users.
                    unsafe { seed_pat(table.root_paddr(), target) };
                    let allocated = fixture.arena.allocated();
                    let flush = set_flags_at!(table, target, SMALL_LEVEL, new_flags(), true).unwrap();
            ...
            fn range_protection_handles_small_heads_and_tails_around_a_one_gib_leaf() {
                ...
                for (address, frame, level) in [
                    ...
                    map_at!(table, VirtAddr::from(address), PhysAddr::from(frame), level, old_flags(), false)
                        .unwrap();
                }
                let allocated = fixture.arena.allocated();
                ...
                set_flush_hook(move |scope, all_cpus| {
                    ...
                    for (index, (pte, _)) in originals.iter().enumerate() {
                        ...
                        if index == 1 {
                            ...
                            assert!(
                                entry.is_present_leaf(LARGE_LEVEL)
                                    || entry.is_present_table(LARGE_LEVEL)
                            );
                        }
                    }
                });
                let (result, flush) = table.set_flags_range(
                    VirtAddr::from(start),
                    VirtAddr::from(end),
                    new_flags(),
                    true,
            ...
            fn range_protection_keeps_existing_large_and_small_leaf_sizes_without_allocating() {
                ...
                for (address, level) in [
                    ...
                    map_at!(table, VirtAddr::from(address), PhysAddr::from(FRAME), level, old_flags(), true)
                        .unwrap();
                }
                let allocated = fixture.arena.allocated();
                fixture.allow_allocations(0);
            ...
            fn a_range_stops_at_a_hole_and_returns_the_successful_prefix_flush() {
                ...
                for page in [0, 1, 3] {
                    table
                        .map(common::page_4k(VirtAddr::from(BASE + page * PAGE)), common::frame_4k(PhysAddr::from(FRAME + page * PAGE)), old_flags(), false)
                        .unwrap();
                }
                let (result, flush) = table.set_flags_range(
                    VirtAddr::from(BASE),
            ...
            fn empty_and_invalid_ranges_do_not_change_the_tree() {
                ...
                table.map(common::page_4k(address), common::frame_4k(PhysAddr::from(FRAME)), old_flags(), false).unwrap();
                let original = table.walk(address).read().raw();
                let allocated = fixture.arena.allocated();
                fixture.allow_allocations(0);
                for empty in [address, VirtAddr::from(BASE + HUGE)] {
                    let (result, flush) = table.set_flags_range(empty, empty, new_flags(), true);
                    assert_eq!(result, Ok(()));
                    flush.expect_no_flush();
                }
                for (start, end) in [
                ...
                let (result, flush) =
                    table.set_flags_range(address, address + PAGE, PTEntryFlags::WRITABLE, true);
            ...
            fn failed_private_splits_leave_the_original_leaf_unchanged_and_reclaim_staging() {
                ...
                for operation in 0..4 {
                    let budgets: &[usize] = if operation == 0 { &[0] } else { &[0, 1] };
                    for &allowed in budgets {
                        fixture.allow_allocations(usize::MAX);
                        let base = BASE + (operation * 2 + allowed) * HUGE;
                        let address = VirtAddr::from(base + 7 * PAGE);
                        let mapping_level =
                            if operation == 0 { LARGE_LEVEL } else { HUGE_LEVEL };
                        map_at!(table, VirtAddr::from(base), PhysAddr::from(FRAME), mapping_level, old_flags(), false)
                            .unwrap();
                        ...
                        let result = match operation {
                            0 => split_at!(table, address, LARGE_LEVEL, true).map(discharge),
                            1 => set_flags_at!(table, address, SMALL_LEVEL, new_flags(), true)
                                .map(discharge),
                            2 => table.set_shared(common::page_4k(address), true).map(discharge),
                            _ => table.set_private(common::page_4k(address), true).map(discharge),
                        };
                        assert_eq!(result, Err(PagingError::AllocFrame));
                        assert_eq!(table.walk(address).level(), mapping_level);
                        assert_eq!(table.walk(address).read().raw(), original);
                        ...
                        for offset in [0, 7 * PAGE, mapping_level.size() - PAGE] {
                            assert_eq!(
            ...
            fn range_allocation_failure_retains_only_completed_prefix_edits_and_their_flush() {
                ...
                for offset in [0, HUGE] {
                    map_at!(table, VirtAddr::from(BASE + offset), PhysAddr::from(FRAME + offset), HUGE_LEVEL, old_flags(), false)
                        .unwrap();
                }
            ...
            #[test]
            fn split_requires_a_huge_leaf_and_preserves_every_mapping() {
                let (fixture, mut table) = $fixture();
                let start = VirtAddr::from(BASE);
                let target = start + 7 * PAGE;
                assert!(matches!(
                    split_at!(table, target, SMALL_LEVEL, true),
                    Err(PagingError::InvalidLevel)
                ));
                let allocated = fixture.arena.allocated();
                map_at!(table, start, PhysAddr::from(FRAME), LARGE_LEVEL, old_flags(), false)
                    .unwrap();
                // SAFETY: this fixture has no other users.
                unsafe { seed_pat(table.root_paddr(), start) };
                let flush = split_at!(table, target, LARGE_LEVEL, true).unwrap();
                assert_completed(flush, BASE, BASE + LARGE, LARGE_LEVEL);
                assert!(matches!(
                    split_at!(table, target, LARGE_LEVEL, true),
                    Err(PagingError::InvalidLevel)
                ));
                for small in 0..512 {
                    let offset = small * PAGE;
                ...
                let (mapped, flush) = table.unmap_region(start, start + LARGE).unwrap();
                assert!(mapped);
            ...
            fn protection_refuses_finer_subtrees_and_leaf_edits_refuse_holes_without_changes() {
                ...
                for page in [0, 1, 511] {
                    table
                        .map(common::page_4k(address + page * PAGE), common::frame_4k(PhysAddr::from(FRAME + page * PAGE)), old_flags(), false)
                        .unwrap();
                }
                let allocated = fixture.arena.allocated();
                fixture.allow_allocations(0);
                assert!(matches!(
                    set_flags_at!(table, address, LARGE_LEVEL, new_flags(), true),
                    Err(PagingError::NotLeafEntry)
                ));
                for missing in [address + 2 * PAGE, address + HUGE] {
                    assert!(matches!(
                        set_flags_at!(table, missing, SMALL_LEVEL, new_flags(), true),
                        Err(PagingError::NotMapped)
                    ));
                    assert!(matches!(
                    ...
                    assert!(matches!(
                        table.set_shared(common::page_4k(missing), true),
                        Err(PagingError::NotMapped)
                    ));
                    assert!(matches!(
                        table.set_private(common::page_4k(missing), true),
                        Err(PagingError::NotMapped)
                    ));
            ...
            #[test]
            fn partial_one_gib_range_splits_only_to_two_mib_leaves() {
                let (fixture, mut table) = $fixture();
                let base = VirtAddr::from(BASE);
                table
                    .map(
                        common::page_1g(base),
                        common::frame_1g(PhysAddr::from(FRAME)),
                        old_flags(),
                        false,
                    )
                    .unwrap();
                let allocated = fixture.arena.allocated();
                let start = base + LARGE;
                let end = base + 3 * LARGE;
                ...
                for address in [base, start, start + LARGE, end] {
                    assert_eq!(table.walk(address).level(), LARGE_LEVEL);
                }
                assert!(!table.walk(base).read().writable());
                assert!(table.walk(start).read().writable());
                assert!(table.walk(start + LARGE).read().writable());
                assert!(!table.walk(end).read().writable());
                assert_eq!(table.validate_page_table(), Ok(()));
            }
            ...
            fn invalid_leaf_edit_arguments_are_rejected_before_allocation_or_mutation() {
                let (fixture, mut table) = $fixture();
                let address = VirtAddr::from(BASE);
                table.map(common::page_2m(address), common::frame_2m(PhysAddr::from(FRAME)), old_flags(), false).unwrap();
                let original = table.walk(address).read().raw();
                let allocated = fixture.arena.allocated();
                fixture.allow_allocations(0);
                for flags in [PTEntryFlags::empty(), PTEntryFlags::WRITABLE | PTEntryFlags::NX] {
                    assert!(matches!(
                        set_flags_at!(table, address, SMALL_LEVEL, flags, true),
                        Err(PagingError::InvalidFlags)
                    ));
                }
                assert!(paging::page::Page::<paging::sizes::Regular>::from_start_address(
                    address + 1
                )
                .is_err());
                assert!(paging::page::Page::<paging::sizes::Huge>::from_start_address(
                    address + PAGE
                )
                .is_err());
...
fn concurrent_disjoint_subpage_protections_survive_the_shared_huge_leaf_split() {
    ...
    let address = VirtAddr::from(BASE);
    table
        .map(common::page_2m(address), common::frame_2m(PhysAddr::from(FRAME)), old_flags(), false)
        .unwrap();
    ...
    let workers: Vec<_> = (0..8)
        .map(|worker| {
            ...
            spawn(move || {
                ...
                let flush = set_flags_at!(table, address, SMALL_LEVEL, flags, true).unwrap();
...
fn a_coarse_mapping_losing_to_growth_rejects_an_absent_entry_in_the_finer_subtree() {
    ...
    let worker = spawn({
        ...
        move || {
            table.map(
                common::page_2m(address),
                common::frame_2m(PhysAddr::from(OTHER_FRAME)),
                old_flags(),
                false,
            )
        }
    ...
    assert!(matches!(unmap_at!(table, address, LARGE_LEVEL), Err(PagingError::WrongPageSize)));
    assert!(matches!(split_at!(table, address, LARGE_LEVEL, true), Err(PagingError::InvalidLevel)));
...
fn paused_path_allocations_publish_nothing_and_do_not_block_a_competing_mapper() {
    for target in [SMALL_LEVEL, LARGE_LEVEL] {
        ...
        let worker = spawn({
            let table = table.clone();
            move || map_at!(table, address, PhysAddr::from(OTHER_FRAME), target, old_flags(), false)
        });
...
fn a_fine_mapping_losing_to_a_huge_leaf_reclaims_its_preparation_after_unlock() {
    let (fixture, table) = fixture();
    let address = VirtAddr::from(BASE);
    map_at!(table, address + HUGE, PhysAddr::from(OTHER_FRAME), HUGE_LEVEL, old_flags(), false)
        .unwrap();
    let original = table.walk(address);
    assert_eq!(original.level(), HUGE_LEVEL);
    assert!(!original.read().present());
    ...
    assert_eq!(table.walk(address).level(), original.level());
    assert_eq!(table.walk(address).read().raw(), original.read().raw());
    map_at!(table, address, PhysAddr::from(FRAME), HUGE_LEVEL, old_flags(), false).unwrap();
    release.send(()).unwrap();
    assert_eq!(finish(worker), Err(PagingError::EntryAlreadyPresent { level: HUGE_LEVEL }));
    assert_eq!(fixture.arena.allocated(), before + 2);
    assert_eq!(fixture.arena.freed().len(), 2);
    assert_eq!(
        fixture.arena.freed().into_iter().collect::<BTreeSet<_>>(),
...
fn unmapping_rechecks_a_split_published_before_lock_acquisition() {
    for target in [SMALL_LEVEL, LARGE_LEVEL] {
        ...
        table
            .map(
                common::page_2m(address),
                common::frame_2m(PhysAddr::from(FRAME)),
                old_flags(),
                false,
            )
            .unwrap();
        ...
        let worker = spawn({
            let table = table.clone();
            move || {
                let page = if target == LARGE_LEVEL { address } else { selected };
                unmap_at!(table, page, target).map(|(entry, flush)| (entry.map(|_| target), flush))
            }
        });
        entered.recv_timeout(WAIT).expect("unmapping did not reach its content lock");
        assert_completed(
            split_at!(table, selected, LARGE_LEVEL, true).unwrap(),
            BASE,
            BASE + LARGE,
            LARGE_LEVEL,
        );
        release.send(()).unwrap();
        let result = finish(worker);
        if target == LARGE_LEVEL {
            assert_eq!(result, Err(PagingError::WrongPageSize));
            assert_eq!(table.phys_addr(selected), Ok(PhysAddr::from(FRAME + 7 * PAGE)));
        } else {
            let (removed, flush) = result.unwrap();
...
fn a_fine_protection_retries_after_a_split_before_lock_acquisition() {
    ...
    let worker = spawn({
        let table = table.clone();
        move || set_flags_at!(table, selected, SMALL_LEVEL, new_flags(), true)
    });
...
fn a_split_waiting_for_its_lock_preserves_a_completed_same_entry_protection() {
    ...
    let worker = spawn({
        let table = table.clone();
        move || {
            let flush = split_at!(table, address + 7 * PAGE, LARGE_LEVEL, true);
            (flush, take_flushes())
        }
    });
    entered.recv_timeout(WAIT).expect("split did not reach its content lock");
    let flush = set_flags_at!(table, address, LARGE_LEVEL, new_flags(), true).unwrap();
    assert_flush_covers(flush, BASE, BASE + LARGE);
    release.send(()).unwrap();
...
fn a_coarse_protection_losing_to_a_split_refuses_instead_of_overwriting_children() {
    ...
    let worker = spawn({
        let table = table.clone();
        move || set_flags_at!(table, address, LARGE_LEVEL, new_flags(), true)
    });
    entered.recv_timeout(WAIT).expect("protection did not reach its content lock");
    assert_completed(
        split_at!(table, address + 7 * PAGE, LARGE_LEVEL, true).unwrap(),
        BASE,
        BASE + LARGE,
        LARGE_LEVEL,
    );
    ...
    assert_flush_covers(
        set_flags_at!(table, protected, SMALL_LEVEL, flags, true).unwrap(),
        protected.bits(),
        protected.bits() + PAGE,
    );
    release.send(()).unwrap();
...
fn a_range_observes_finer_levels_after_acquiring_its_whole_domain_guard() {
    ...
    let worker = spawn({
        let table = table.clone();
        move || table.set_flags_range(address, address + HUGE, new_flags(), true)
    });
    ...
    assert_completed(
        table
            .set_flags(
                paging::page::Page::<paging::sizes::Huge>::containing_address(target),
                old_flags(),
                true,
            )
            .unwrap(),
    ...
    for large in 0..512 {
        ...
        assert_eq!(table.walk(current).level(), LARGE_LEVEL);
        assert_eq!(
        ...
        assert_eq!(
            table.phys_addr(current + LARGE - 1),
            Ok(PhysAddr::from(FRAME + offset + LARGE - 1))
        );
    }
    assert_eq!(table.validate_page_table(), Ok(()));
...
fn concurrent_split_allocates_once_and_preserves_hardware_ad_accrued_during_preparation() {
    for protect in [false, true] {
        ...
        assert_eq!(fixture.arena.allocated(), allocated + 1);
        assert!(fixture.arena.freed().is_empty());
        for offset in [0, PAGE, target_offset, LARGE - 1] {
            let current = address + offset;
            assert_eq!(table.walk(current).level(), LARGE_LEVEL);
        ...
        assert_eq!(table.walk(target).level(), LARGE_LEVEL);
        ...
        for small in 0..512 {
            let offset = small * PAGE;
            let current = address + offset;
            let updated = leaf_word(FRAME + offset, SMALL_LEVEL, flags | history, false, true);
            let expected = if protect && offset == target_offset {
                protected_word(updated, new_flags())
            } else {
                updated
            };
            assert_eq!(table.walk(current).level(), SMALL_LEVEL);
            assert_eq!(
                table.walk(current).read().raw(),
                expected,
            ...
            assert_eq!(
                table.phys_addr(current + PAGE - 1),
                Ok(PhysAddr::from(FRAME + offset + PAGE - 1))
            );
        }
        ...
        mod $module {
            ...
            fn point_edits_publish_split_before_one_selected_global_barrier() {
                for all_cpus in [false, true] {
                    for level in [LARGE_LEVEL, HUGE_LEVEL] {
                        for operation in 0..4 {
                            if operation == 0 && level != LARGE_LEVEL {
                                continue;
                            }
                            let (fixture, mut table) = $fixture();
                            let base = VirtAddr::from(BASE);
                            ...
                            let allocated = fixture.arena.allocated();
                            let split_allocations = if operation == 0 { 1 } else { level.depth() };
                            ...
                            set_flush_hook(move |scope, selected| {
                                ...
                                assert_eq!(arena.allocated(), allocated + split_allocations);
                                assert!(arena.freed().is_empty());
                            });
                            let flush = match operation {
                                0 => split_at!(table, target, level, all_cpus).unwrap(),
                                1 => {
                                    set_flags_at!(table, target, SMALL_LEVEL, new_flags(), all_cpus)
                                        .unwrap()
                                }
                                2 => table.set_shared(common::page_4k(target), all_cpus).unwrap(),
                                _ => table.set_private(common::page_4k(target), all_cpus).unwrap(),
                            };
                            clear_flush_hook();
                            flush.expect_no_flush();
                            assert_eq!(
                            ...
                            assert_eq!(fixture.arena.allocated(), allocated + split_allocations);
                            assert!(fixture.arena.freed().is_empty());
                            for current in
                ...
                for (bases, start, end, pages) in [
                    ...
                    set_flush_hook(move |scope, selected| {
                        ...
                        for (pte, _) in &originals {
                            let entry = unsafe { load_entry(*pte as *const Entry) };
                            assert!(
                                entry.is_present_leaf(HUGE_LEVEL)
                                    || entry.is_present_table(HUGE_LEVEL)
                            );
                        }
                    });
                    let (result, flush) =
            ...
            fn callback_panic_keeps_published_x86_splits_owned_and_valid() {
                for operation in 0..6 {
                    ...
                    let level = if operation == 0 { LARGE_LEVEL } else { HUGE_LEVEL };
                    let span = level.size();
                    for index in 0..original_count {
                        map_at!(
                            table,
                            base + index * span,
                            (FRAME + index * span).into(),
                            level,
                            old_flags(),
                            false
                        )
                        .unwrap();
                        unsafe { seed_pat(table.root_paddr(), base + index * span) };
                    }
                    let originals: Vec<_> = (0..original_count)
                        .map(|index| {
                            let address = base + index * span;
                            let (pte, observed_level) =
                                unsafe { leaf_pte(table.root_paddr(), address) };
                            assert_eq!(observed_level, level);
                    ...
                    *fixture.locks.0.before_unlock.lock().unwrap() = Some(Box::new(move || {
                    ...
                    let target = base + span - PAGE;
                    let result = catch_unwind(AssertUnwindSafe(|| match operation {
                        0 => split_at!(table, target, LARGE_LEVEL, true).unwrap().expect_no_flush(),
                        1 => set_flags_at!(table, target, SMALL_LEVEL, new_flags(), true)
                            .unwrap()
                            .expect_no_flush(),
                        2 => table
                            .set_shared(common::page_4k(target), true)
                            .unwrap()
                            .expect_no_flush(),
                        3 => table
                            .set_private(common::page_4k(target), true)
                            .unwrap()
                            .expect_no_flush(),
                        _ => {
                            let (result, flush) = table.set_flags_range(
                    ...
                    assert_eq!(take_flushes().len(), 1);
                    let staging = if operation == 0 { 1 } else { 2 };
                    ...
                    for (index, _) in originals.into_iter().enumerate() {
                        ...
                        assert_eq!(
                            table
                                .walk(address + if split && index == 0 { span - PAGE } else { 0 },)
                                .level(),
                            if split { SMALL_LEVEL } else { level }
                        );
                        assert_eq!(
                            table.phys_addr(address + span - 1),
                            Ok((FRAME + (index + 1) * span - 1).into())
                        );
                    }
                    assert_eq!(table.validate_page_table(), Ok(()));
                    unsafe { table.free_children() };
            ...
            fn unchanged_leaf_edits_neither_flush_synchronously_nor_return_pending_work() {
                ...
                let base = VirtAddr::from(BASE);
                table
                    .map(common::page_4k(base), common::frame_4k(FRAME.into()), new_flags(), true)
                    .unwrap();
                table
                    .map(
                        common::page_4k(base + PAGE),
                        common::frame_4k((FRAME + PAGE).into()),
                        new_flags(),
                        false,
                    )
                    .unwrap();
                ...
                let allocated = fixture.arena.allocated();
                assert!(matches!(
                    split_at!(table, base, SMALL_LEVEL, false),
                    Err(PagingError::InvalidLevel)
                ));
                set_flags_at!(table, base, SMALL_LEVEL, new_flags(), false)
                    .unwrap()
                    .expect_no_flush();
                table.set_shared(common::page_4k(base), false).unwrap().expect_no_flush();
                table.set_private(common::page_4k(base + PAGE), false).unwrap().expect_no_flush();
                let (result, flush) = table.set_flags_range(base, base + PAGE, new_flags(), false);
                result.unwrap();
                flush.expect_no_flush();
                assert!(take_flushes().is_empty());
                assert_eq!(fixture.arena.allocated(), allocated);
            ...
            fn overflow_uses_one_whole_tlb_barrier_and_returns_no_pending_split() {
                let (_fixture, mut table) = $fixture();
                let base = VirtAddr::from(usize::MAX & !(LARGE - 1));
                map_at!(table, base, FRAME.into(), LARGE_LEVEL, old_flags(), false).unwrap();
                let (pte, level) = unsafe { leaf_pte(table.root_paddr(), base) };
                assert_eq!(level, LARGE_LEVEL);
                let pte = pte as usize;
                ...
                split_at!(table, VirtAddr::from(usize::MAX), LARGE_LEVEL, false)
                    .unwrap()
                    .expect_no_flush();
                clear_flush_hook();
            ...
            fn range_hole_flushes_the_prepared_prefix_once_without_touching_later_mappings() {
                ...
                table
                    .map(
                        common::page_4k(boundary + PAGE),
                        common::frame_4k(OTHER_FRAME.into()),
                        old_flags(),
                        true,
                    )
                    .unwrap();
                ...
                let allocated = fixture.arena.allocated();
                let (result, flush) =
                    table.set_flags_range(boundary - PAGE, boundary + 2 * PAGE, new_flags(), true);
                clear_flush_hook();
                assert_eq!(result, Err(PagingError::NotMapped));
                assert_completed(flush, BASE, BASE + HUGE, HUGE_LEVEL);
                assert_eq!(fixture.arena.allocated(), allocated + 2);
...
fn paused_point_and_range_barriers_exclude_writers_but_not_lock_free_walkers() {
    for range in [false, true] {
        ...
        for (address, frame) in [(left, FRAME), (right, FRAME + HUGE)] {
            let level = if !range && address == right { LARGE_LEVEL } else { HUGE_LEVEL };
            map_at!(table, address, frame.into(), level, old_flags(), false).unwrap();
        }
        let table = Arc::new(table);
        ...
        let editor = spawn({
            ...
            move || {
                ...
                let result = if range {
                    ...
                    (Ok(()), split_at!(table, right + PAGE, LARGE_LEVEL, true).unwrap())
                };
                clear_flush_hook();
                (result, take_flushes())
            }
        ...
        let writer = spawn({
            let table = table.clone();
            move || map_at!(table, right, OTHER_FRAME.into(), HUGE_LEVEL, new_flags(), true)
        });
        ...
        if range {
            ...
            assert_eq!(
                calls,
                vec![(
                    FlushScope::Range { start: right, end: right + LARGE, level: LARGE_LEVEL },
                    true
                )]
            );
        }
