        ...
        verus!{
            impl #impl_generics VPrint for #name #ty_generics #where_clause {
                ...
                fn early_print2(&self, Tracked(snpcore): Tracked<&mut crate::registers::SnpCore>, Tracked(console): Tracked<SnpPointsToRaw>) -> (newconsole: Tracked<SnpPointsToRaw>)
                {
                    ...
                    proof {
                        // Justification: VPrint derives a linear sequence of field prints, each of which
                        // preserves the SnpCore/console postcondition; SMT does not compose the chained
                        // print_ensures_snp_c facts reliably for larger structs.
                        assume(crate::debug::print_ensures_snp_c(*old(snpcore), oldconsole, *snpcore, console));
                    }
                    Tracked(console)
                }
            }
