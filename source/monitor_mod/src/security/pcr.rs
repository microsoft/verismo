...
verus! {
    ...
    {
        |vec: Vec<SHA512Type>| vec.len() > 1 &&
            forall |i| 0 < i < vec.len() ==> vec[i].wf()
    }
}

verus! {
    #[verifier(external_body)]
    pub fn extend_pcr(index: usize, data: &Array<u8_s, 64>, Tracked(cs): Tracked<&mut SnpCoreSharedMem>)
    requires
        data.wf(),
        ...
        cs.only_lock_reg_coremode_updated(*old(cs), set![], set![spec_PCR_lockid()]),
    {
        ...
        let (pcr_ptr, Tracked(mut pcr_perm), Tracked(mut pcr_lock))= PCR().acquire(Tracked(pcr_lock), Tracked(&cs.snpcore.coreid));
    ...
    pub fn attest_pcr(
        ...
        ghcb: GhcbHandle, secret: &SnpSecretsPageLayout,
        index: usize, report: VBox<OnePage>, Tracked(cs): Tracked<&mut SnpCoreSharedMem>)
    ...
    ensures
        cs.inv_stage_pcr(),
        cs.only_lock_reg_coremode_updated(*old(cs), set![], set![spec_PCR_lockid(), spec_ALLOCATOR_lockid()]),
        ret.0.wf(),
        ...
        ret.2.only_val_updated(ghcb),
    {
        ...
        proof{
            cs1.lemma_update_prop(cs2, *cs, set![], set![spec_PCR_lockid()], set![], set![spec_ALLOCATOR_lockid()]);
            assert(set![spec_PCR_lockid()].union(set![spec_ALLOCATOR_lockid()]) =~~= set![spec_PCR_lockid(), spec_ALLOCATOR_lockid()]);
        }
        ret
    }
}
