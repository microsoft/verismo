...
pub open spec fn spec_is_default_pages_const_to_vmpl(page_perms: Map<int, SnpPointsToRaw>, start_page: int, npages: nat, vmpl: nat) -> bool {
...
pub open spec fn spec_contains_page_perm(
    page_perms: Map<int, SnpPointsToRaw>, i: int, osperm: OSMemPerm) -> bool {
...
pub open spec fn spec_contains_page_perms(
    page_perms: Map<int, SnpPointsToRaw>, start_page: int, npages: nat, osperm: OSMemPerm) -> bool {
    &&& forall |i| #[trigger] page_perms.contains_key(i) ==>
        spec_contains_page_perm(page_perms, i, osperm)
    ...
    cs.only_lock_reg_coremode_updated(*old(cs), set![], set![spec_PT_lockid()]),
{
    ...
    proof{
        assert(old_page_perms.dom() =~~= Set::new(|i| start_page as int <= i < start_page + npages));
    }
    ...
    if !shared {
        ...
        ghcb_h = ghcb_h.mk_private(start_page as u64, npages as u64, Tracked(cs), Tracked(&mut page_perms));
        // TODO: zero all pages;
        let start = start_page.to_addr();
        let size = npages.to_addr();
        proof {
            assert forall |i| (start as int).to_page() <= i < ((start + size) as int).to_page()
            implies page_perms.contains_key(i) &&
            page_perms[i]@.snp_wf_range((i.to_addr(), PAGE_SIZE as nat))
            by {
        ...
        let attr = RmpAttr::empty().set_vmpl(RICHOS_VMPL as u64)
                                   .set_perms(entry.osperm.into()).set_vmsa(0);
        proof {
        ...
        ghcb_h = ghcb_h.mk_shared(start_page as u64, npages as u64, Tracked(cs), Tracked(&mut page_perms));
    }
    ...
    proof{
        ...
        assert(entry.page_perms@.dom() =~~= Set::new(|i| start_page as int <= i < start_page + npages));
        assert forall |i| #[trigger] page_perms.contains_key(i)
        implies
            spec_contains_page_perm(page_perms, i, entry.spec_osperm())
        by {
            assert(page_perms.contains_key(i));
            let page_perm = page_perms[i];
        ...
        assert(spec_contains_page_perms(
            entry.page_perms@, start_page as int,
            npages as nat, entry.spec_osperm()));
        assert(entry.wf());
    }
    (ghcb_h, entry)
}
...
pub fn osmem_add(
    ...
    start_page: usize, npages: usize, osperm: OSMemPerm,
    shared: bool,
...
requires
    ...
    !shared ==> spec_is_default_pages_const_to_vmpl(page_perms, start_page as int, npages as nat, RICHOS_VMPL as nat),
    old(snpcore).inv(),
...
ensures
    ...
    spec_contains_page_perms(osmem@.last().page_perms@, start_page as int, npages as nat, osperm),
    *snpcore === *old(snpcore),
{
    ...
    if !shared {
        let attr = RmpAttr::empty().set_vmpl(RICHOS_VMPL as u64).set_perms(osperm.value as u64).set_vmsa(0);
        proof {
            assert(start_page.spec_valid_pn_with(npages as nat));
            assert forall |i| old_page_perms.contains_key(i)
            implies
                spec_rmpadjmem_requires_at(old_page_perms[i], i, attr@)
            by{
                assert(start_page <= i < start_page + npages);
                assert(spec_is_default_pages_const_to_vmpl(old_page_perms, start_page as int, npages as nat, RICHOS_VMPL as nat));
    ...
    osmem.push(entry);
    proof {
        assert forall |i| 0 <= i < osmem.len()
        implies osmem[i].wf()
        by {
            if i == osmem.len() - 1 {
                assert forall |k| page_perms.contains_key(k)
                implies
                    spec_contains_page_perm(page_perms, k, osperm)
                by{
                ...
                assert(spec_contains_page_perms(page_perms, start_page as int, npages as nat, osperm));
            }
        }
    }
}
...
pub fn osmem_entry_split(entry: OSMemEntry, start: usize, npages: usize) -> (ret: (OSMemEntry, OSMemEntry, OSMemEntry))
requires
    entry.wf(),
...
ensures
    ...
    {
    let Tracked(mut page_perms) = entry.page_perms;
    let tracked perms0 = page_perms.tracked_remove_keys(Set::new(|i| entry.spec_start() <= i < start));
    let tracked perms1 = page_perms.tracked_remove_keys(Set::new(|i| start <= i < start + npages));
    let tracked perms2 = page_perms.tracked_remove_keys(Set::new(|i| start + npages <= i < entry.spec_end()));
    let entry0 = OSMemEntry {start_page: entry.start_page, npages: (start - entry.start_page.reveal_value()).into(), osperm: entry.osperm, page_perms: Tracked(perms0)};
    let entry1 = OSMemEntry {start_page: start.into(), npages: npages.into(), osperm: entry.osperm, page_perms: Tracked(perms1)};
    let entry2 = OSMemEntry {start_page: (start + npages).into(), npages: (entry.npages.reveal_value() - npages + entry.start_page.reveal_value() - start).into(), osperm: entry.osperm, page_perms: Tracked(perms2)};
...
ensures
    ret.is_Some() ==>
        (0 <= ret.unwrap() < osmem.len() &&
        osmem[ret.unwrap() as int].spec_start() <= vpage < osmem[ret.unwrap() as int].spec_end()),
{
    let mut i = 0;
    while i < osmem.len()
    invariant
    ...
    {
        let (start_page, npages): (usize, usize) = (osmem[i].start_page.into(), osmem[i].npages.into());
        proof {
...
pub open spec fn spec_ensure_check_osperm(ppage: int, osperm: OSMemPerm, entry: OSMemEntry) -> bool {
    let perm = osperm@;
    let is_full = perm.spec_read() > 0 && perm.spec_write() > 0 && perm.spec_kern_exe() > 0 && perm.spec_user_exe() > 0;
...
ensures
    ret ==> (osperm.value == 0 ||
        exists |i| 0 <= i < osmem.len() &&
            spec_ensure_check_osperm(ppage as int, osperm, osmem[i]))
{
    match osmem_find(osmem, ppage) {
        Some(i) => {
            OSMemPerm::new(osmem[i].osperm.into()).is_super_of(&osperm)
        }
        _ => {
            false
        }
...
pub fn osmem_check_and_get(
    osmem: &mut Vec<OSMemEntry>, ppage: usize, osperm: OSMemPerm
) -> (ret: Option<(usize, OSMemEntry)>)
requires
    osmem_wf(old(osmem)@),
ensures
    ret.is_None() ==> old(osmem) === osmem,
    ret.is_Some() ==>
        osmem@ === old(osmem)@.remove(ret.get_Some_0().0 as int) &&
        ret.get_Some_0().1 === old(osmem)@[ret.get_Some_0().0 as int] &&
        0 <= ret.get_Some_0().0 < old(osmem)@.len(),
    ret.is_Some() ==>
        spec_ensure_check_osperm(ppage as int, osperm, ret.get_Some_0().1)
{
    match osmem_find(osmem, ppage) {
        Some(i)
        if OSMemPerm::new(osmem[i].osperm.into()).is_super_of(&osperm) => {
...
pub fn osmem_check_and_get_page<T: IsConstant + SpecSize + WellFormed + VTypeCast<SecSeqByte>>(
    osmem: &mut Vec<OSMemEntry>, ppage: usize, osperm: OSMemPerm, Tracked(cs): Tracked<&mut SnpCoreSharedMem>
...
verus! {
        ...
        allocator.wf(),
    {
        ...
        invariant
            ...
            *snpcore === prevcore,
            ...
            range_mem.is_Some() ==>
                range_mem.get_Some_0().1@@.wf_const_default((range_mem.get_Some_0().0.0 as int, range_mem.get_Some_0().0.1 as nat)),
        ensures
            osmem_wf(tmposmem@),
            range_mem.is_None(),
            allocator@.inv(),
            ...
            *snpcore === prevcore,
        {
            ...
            if start_page < end_page {
                ...
                let tracked (left, mut aligned_perm) = perm.trusted_split((start_addr - range.0) as nat);
                ...
                proof {
                    ...
                    by{
                        ...
                        assert(page_perms[i]@.bytes() =~~= aligned_perm@.bytes().subrange(offset, offset + PAGE_SIZE as int));
                        assert(aligned_perm@.bytes().is_constant());
                        assert(page_perms[i]@.bytes().is_constant()) by {
                            let b = page_perms[i]@.bytes();
                            assert forall |k| 0 <= k < b.len()
                            implies b[k].is_constant()
                            by {
                                let sub = aligned_perm@.bytes().subrange(offset, offset + PAGE_SIZE as int);
                                assert(b[k] === sub[k]);
                ...
                osmem_add(&mut tmposmem, start_page, npages, OSMemPerm::ram(), false, Tracked(page_perms), Tracked(snpcore));
            }
    ...
    pub fn add_osmem_to_e820(
        e820: VBox<E820Table>,
        e820_count: usize,
        osmem: &Vec<OSMemEntry>,
    ) -> (ret: VBox<E820Table>)
    requires
        e820@.is_constant(),
    ...
    pub fn create_osmem_from_allocator(
        vmpl: u8,
        Tracked(cs): Tracked<&mut SnpCoreSharedMem>,
    ) -> (osmem: Vec<OSMemEntry>)
    requires
        ...
        cs.only_lock_reg_coremode_updated(*old(cs), set![], set![spec_ALLOCATOR_lockid()]),
    {
        ...
        let (alloc_ptr, Tracked(mut allocperm), Tracked(alloc_lockperm)) =
            ALLOCATOR().acquire(Tracked(alloc_lockperm), Tracked(&cs.snpcore.coreid));
        ...
        let tmposmem = _osmem_add_ram_from_allocator(&mut allocator, tmposmem, Tracked(&mut cs.snpcore));
        ...
        ALLOCATOR().release(Tracked(&mut alloc_lockperm), Tracked(allocperm), Tracked(&cs.snpcore.coreid));
        proof {
            cs.lockperms.tracked_insert(spec_ALLOCATOR_lockid(), alloc_lockperm);
            oldcs.lemma_update_prop(prevcs, *cs, set![], set![spec_ALLOCATOR_lockid()], set![], set![spec_ALLOCATOR_lockid()]);
        }
        ...
        *snpcore === *old(snpcore),
    {
            ...
            *snpcore === *old(snpcore),
        {
            if let Some(e) = osmem_check_and_get(osmem, start, OSMemPerm::ram()) {
                ...
                let tmp_end = if end > next_start {next_start} else {end};
                let tmp_start = start;
                ...
                let Tracked(mut page_perms) = mid.page_perms;
                let attr = RmpAttr::empty()
                            .set_vmpl(RICHOS_VMPL as u64)
                            .set_perms(OSMemPerm::new(mid.osperm.into()).set_write(0).value as u64).set_vmsa(0);
                proof {
                    assert forall |i| tmp_start <= i < tmp_start + tmp_npages
                    implies
                        #[trigger] page_perms.contains_key(i) &&
                        spec_rmpadjmem_requires_at(page_perms[i], i, attr@)
                    by{
                    ...
                    assert(page_perms.dom() =~~= Set::new(|i| tmp_start <= i < (tmp_start + tmp_npages)));
                }
                rmpadjmem(
                    tmp_start, tmp_npages, attr,
                    Tracked(snpcore), Tracked(&mut page_perms)
                );
    ...
    requires
        osmem_wf(old(osmem)@),
        forall |i| 0 <=i < ranges@.len() ==>
            (ranges@[i].0 as int).spec_valid_pn_with(ranges@[i].1 as nat),
        old(snpcore).inv(),
    ensures
        osmem_wf(osmem@),
        snpcore.inv(),
