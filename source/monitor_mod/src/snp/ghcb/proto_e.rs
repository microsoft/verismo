...
requires
    subcode < 0x100,
{
    proof {assert(subcode << SUBCODE_OFFSET!() < 0x10000) by(bit_vector)
    requires  subcode < 0x100;}
...
verus! {
    impl GHCBProto {
        ...
        {
            GHCB_SNP_PAGE_STATE_CHANGE_REQ as u64 |
            (ppage as u64 & 0xfff_ffff_ffff) << 12u64 |
        ...
        pub open spec fn restored_ghcb(new: SnpCore, prev: SnpCore) -> bool {
            !spec_attack() ==> new.regs[GHCB_REGID()].view::<usize_s>().value == prev.regs[GHCB_REGID()].view::<usize_s>().value
        }
    }
}
...
verus! {
    ...
    pub open spec fn spec_ghcb_send_core_update(oldsnpcore: SnpCore, snpcore: SnpCore, req_resp: (nat, nat)) -> bool {
    ...
    pub fn ghcb_msr_send(val: u64, Tracked(memperm): Tracked<&mut Option<SnpPointsToRaw>>, Tracked(snpcore): Tracked<&mut SnpCore>) -> (ret: u64)
    requires
...
verus! {
    pub fn ghcb_proto(val: u64, Tracked(memperm): Tracked<&mut Option<SnpPointsToRaw>>, Tracked(snpcore): Tracked<&mut SnpCore>) -> (ret: u64)
    requires
        old(snpcore).inv_reg_cpu(),
        val.is_constant(),
        is_none_or_sharedmem(*old(memperm)),
...
verus! {
    ...
    pub fn early_vc_terminate_debug(reason_code: u64_t, Tracked(cc): Tracked<&mut SnpCoreConsole>) -> (ret: !)
    requires
        old(cc).wf(),
    ensures
        false,
    ...
    proof fn trusted_ghcb_change_page_state(tracked perm: &mut SnpPointsToRaw, op: PageOps, tracked snpcore: &SnpCore)
    requires
        snpcore.last_ghcb_req() == GHCBProto::spec_msr_page_state_req(old(perm)@.ppage() as usize, op) as nat,
    ...
    pub fn ghcb_change_page_state_via_msr(ppage: usize_t, op: PageOps,
        Tracked(memperm): Tracked<&mut SnpPointsToRaw>, Tracked(snpcore): Tracked<&mut SnpCore>)
    requires
        old(snpcore).inv_reg_cpu(),
        old(memperm)@.wf(),
    ...
    ensures
        ...
        (snpcore.last_ghcb_resp() as u64 & GHCB_MSR_INFO_MASK) != GHCB_MSR_REGISTER_GHCB_RES ==> spec_attack(),
        (snpcore.last_ghcb_resp() as u64 & !GHCB_MSR_INFO_MASK) != GVN::new(ppage as int).to_addr().to_u64() ==> spec_attack(),
    ...
    ensures
        snpcore.inv_reg_cpu(),
        spec_ghcb_send_core_update(*old(snpcore), *snpcore, (GHCB_MSR_SEV_INFO_REQ as nat, snpcore.last_ghcb_resp())),
        (snpcore.last_ghcb_resp() as u64 & GHCB_MSR_INFO_MASK) != GHCB_MSR_SEV_INFO_RES ==> spec_attack(),
