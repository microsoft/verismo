...
proof fn trusted_ghcb_change_pages_state_via_pg(ppage: int, npages: nat, tracked page_perms: &mut Map<int, SnpPointsToRaw>, op: PageOps, tracked snpcore: &SnpCore)
requires
...
requires
    ...
    forall |i| ppage <= i < (ppage + npages) ==>
        old(page_perms).contains_key(i) &&
        old(page_perms)[i]@.wf_range((i.to_addr(), PAGE_SIZE as nat)),
ensures
    ...
    ensure_pages_perm_change_state(*old(page_perms), *page_perms, ppage as int, npages as nat, op),
    forall |i| ppage <= i < (ppage + npages) ==>
            page_perms.contains_key(i) &&
            ensure_page_perm_change_state(old(page_perms)[i], #[trigger]page_perms[i], i as int, op),
{
    let mut offset = 0;
    ...
    invariant
        ...
        ensure_pages_perm_change_state(old_page_perms, *page_perms, ppage as int, offset as nat, op),
        forall |i| (ppage + offset) <= i < (ppage + npages) ==>
            page_perms.contains_key(i),
        forall |i| (ppage + offset) <= i < (ppage + npages) ==>
            old_page_perms[i] === page_perms[i],
        requires_pages_perms(old_page_perms, ppage as int, npages as nat),
    {
        ...
        assert forall |i| subdom.contains(i)
        ...
        assert forall |i| !subdom.contains(i) && prev_page_perms.contains_key(i)
        implies page_perms.contains_key(i) && page_perms[i] === prev_page_perms[i]
        by {
        }
        internal::ghcb_change_page_state_via_pg_internal(
            ghcb_ptr.clone(), ppage + offset as u64, n, op,
            Tracked(&mut sub_page_perms), Tracked(&mut ghcbpage_perm0), Tracked(cs));
        ...
        proof {
            ...
            assert forall |i| !sub_page_perms.contains_key(i) && prev_page_perms.contains_key(i)
            implies page_perms.contains_key(i) && page_perms[i] === prev_page_perms[i]
            by {
            ...
            assert forall |i| ppage <= i < (ppage + offset)
            implies
                #[trigger] page_perms.contains_key(i) &&
                ensure_page_perm_change_state(old_page_perms[i], page_perms[i], i as int, op)
            by {
                if i < ppage + offset - n {
            ...
            assert forall |i| (ppage + offset) <= i < (ppage + npages)
            implies
                (page_perms.contains_key(i) &&
                old_page_perms[i] === page_perms[i])
            by{
...
mod internal {
        ...
        ensure_pages_perm_change_state(*old(page_perms), *page_perms, ppage as int, npages as nat, op),
    {
        ...
        let tracked(left, right) = ghcbpage_perm.tracked_into_raw().trusted_split(GhcbPage::spec_shared_buffer_offset());
        let tracked(scratch_perm, right) = right.trusted_split(spec_size::<SnpPageStateChange>());
        let mut scratch: VBox<SnpPageStateChange> = VBox::from_raw(scratch_ptr.to_usize(), Tracked(scratch_perm.trusted_into()));
        // Clear the buffer
        ...
        let tracked mut ghcbpage_perm = left.trusted_join(scratch_perm.tracked_into_raw()).trusted_join(right).tracked_into();
        let (mut header, Tracked(mut ghcbpage_perm)) = scratch_ptr.header().copy_with::<GhcbPage>(Tracked(ghcbpage_perm));
        let ghost oldcs = *cs;
            ...
            page_perms === old(page_perms),
        {
            ...
            let (tmpheader, Tracked(mut tmp_ghcb_perm)) = scratch_ptr.header().copy_with::<GhcbPage>(Tracked(ghcbpage_perm0.tracked_remove(0)));
            header = tmpheader;
            //header.leak_debug();
            proof{
        ...
        proof{
            trusted_ghcb_change_pages_state_via_pg(ppage as int, npages as nat, page_perms, op, &cs.snpcore);
            ghcbpage_perm0.tracked_insert(0, ghcbpage_perm);
        }
    }
    }
...
impl SpecSnpPageStateChangeEntry {
    pub open spec fn req_entry(gpn: u64, opval: u64, psize: u64) -> SpecSnpPageStateChangeEntry{
        SpecSnpPageStateChangeEntry::empty().spec_set_gpn(gpn).spec_set_operation(opval).spec_set_psize(psize)
    }
}
}
verismo_simple! {
...
impl <'a> MutFnTrait<'a, FillPageStateChange, bool> for SnpPageStateChange  {
    ...
    open spec fn spec_update(&self, prev: &Self, params: FillPageStateChange, ret: bool) -> bool {
        ...
        &&& self.is_constant()
        &&& forall |k: int| 0 <= k < npages ==> SnpPageStateChangeEntry::spec_new(self.entries@[k].vspec_cast_to())@ ===
    ...
    fn box_update(&'a mut self, params: FillPageStateChange) -> (ret: bool)
    {
        ...
        invariant
            ...
            opval == op.as_int(),
            npages <= SNP_PAGE_STATE_CHANGE_MAX_ENTRY,
            forall |k: int| 0 <= k < i ==> SnpPageStateChangeEntry::spec_new(self.entries@[k].vspec_cast_to())@ ===
                SpecSnpPageStateChangeEntry::req_entry((ppage + k) as u64, opval, 0)
        {
            let gpn = ppage + i as u64;
            let entry = SnpPageStateChangeEntry::empty()
                .set_gpn(gpn)
                .set_operation(opval)
                .set_psize(0);
