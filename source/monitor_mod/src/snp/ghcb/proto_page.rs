...
verus! {
    ...
    ensures
        forall |val: u8, b1: u8| 0 <= b1 < 8 ==>
            (val | (1<< b1)) & (1 << b1)  != 0,
        forall |val: u8, b1: u8, b2: u8| 0 <= b1 < 8 && 0 <= b2 < 8 ==>
            ((val | (1 << b2)) & (1 << b1) != 0) == ((val & (1 << b1) != 0) || (b1 == b2))
    {
        ...
        assert forall |val: u8, b1: u8, b2: u8|
            0 <= b1 < 8 && 0 <= b2 < 8
        implies
            ((val | (1 << b2)) & (1 << b1) != 0) == ((val & (1 << b1) != 0) || b1 == b2)
        by {
...
impl GhcbPage {
    ...
    ensures
        self.is_constant(),
        ensure_set_bitmap(self.spec_valid_bitmap()@, old(self).spec_valid_bitmap()@, offset as nat),
        ...
        *self === old(self).spec_set_valid_bitmap(self.spec_valid_bitmap()),
    {
        ...
        proof{lemma_bit8_setbit(oldv, bit);}
...
impl VBox<GhcbPage> {
            ...
            ensure_page_perm_change_state(old(page_perms)[i], page_perms[i], i, op),
    {
        ...
        proof{ghcbpage_perm0.tracked_insert(0, ghcbpage_perm);}
        ghcb_change_page_state_via_pg(ghcb_ptr.clone(), ppage, npages, op, Tracked(page_perms), Tracked(&mut ghcbpage_perm0), Tracked(cs));
        VBox::from_raw(ghcb_ptr.to_usize(), Tracked(ghcbpage_perm0.tracked_remove(0)))
    }

    pub fn alloc_ghcb_handle(
        Tracked(cs): Tracked<&mut SnpCoreSharedMem>
    ) -> (ghcb: Self)
    requires
        (*old(cs)).inv(),
    ensures
        ghcb.ghcb_wf(),
        cs.inv(),
        cs.only_lock_reg_coremode_updated((*old(cs)), set![GHCB_REGID()], set![spec_ALLOCATOR().lockid(), spec_PT().lockid()]),
    {
        let ghost cs1 = *cs;
        ...
        proof {
            let ghost cs3 = *cs;
            cs1.lemma_update_prop(cs2, cs3, set![], set![spec_ALLOCATOR().lockid()], set![GHCB_REGID()], set![spec_PT().lockid()]);
            assert(set![spec_ALLOCATOR().lockid()].union(set![spec_PT().lockid()]) =~~= set![spec_ALLOCATOR().lockid(), spec_PT().lockid()]);
        }
    ...
    pub fn ghcb_page_proto(
        ...
        exit1: &mut u64,
        exit2: &mut u64,
        Tracked(cs): Tracked<&mut SnpCoreSharedMem>) -> (ret: (SvmStatus, Self))
    requires
        (*old(cs)).inv(),
        self.ghcb_wf(),
    ensures
        ...
        cs.only_lock_reg_coremode_updated((*old(cs)), set![], set![]),
    {
        ...
        if !MSR_GHCB().read(Tracked(&ghcb_msr_perm)).eq(&ghcb_addr) {
            proof {cs.snpcore.regs.tracked_insert(GHCB_REGID(), ghcb_msr_perm);}
        ...
        vmgexit(Tracked(&mut ghcb_msr_perm), Tracked(&mut cs.snpcore.coreid), Tracked(&mut op_ghcbpage_perm));
        ...
        let (exit_info_1, Tracked(ghcbpage_perm)) = ghcb_ptr.vmsa().sw_exit_info_1().copy_with::<GhcbPage>(Tracked(ghcbpage_perm));
        let (exit_info_2, Tracked(ghcbpage_perm)) = ghcb_ptr.vmsa().sw_exit_info_2().copy_with::<GhcbPage>(Tracked(ghcbpage_perm));
        let (sw_exit_code, Tracked(ghcbpage_perm)) = ghcb_ptr.vmsa().sw_exit_code().copy_with::<GhcbPage>(Tracked(ghcbpage_perm));
        ...
        let err = if (exit_info_1 & 0xffffffff) == 1 {
            ...
            if valid && (ty == SVM_EVTINJ_TYPE_EXEPT)
                && (vec == SVM_EVTINJ_VEC_X86_TRAP_GP || vec == SVM_EVTINJ_VEC_X86_TRAP_UD)
            {
                SvmStatus::Exception
            } else {
                SvmStatus::VmmError
    ...
    pub fn ghcb_read_msr(
        self,
        reg: u32,
        Tracked(cs): Tracked<&mut SnpCoreSharedMem>) -> (ret: (u64, Self))
    requires
        self.ghcb_wf(),
        (*old(cs)).inv(),
    ensures
        ...
        ret.1.ghcb_wf(),
    {
        ...
        match resp {
            SvmStatus::Ok => {
                ...
                let (ghcb_ptr, Tracked(ghcb_perm)) = ghcb.into_raw();
                let (ax, Tracked(ghcb_perm)) = ghcb_ptr.vmsa().rax().copy_with::<GhcbPage>(Tracked(ghcb_perm));
                let (dx, Tracked(ghcb_perm)) = ghcb_ptr.vmsa().rax().copy_with::<GhcbPage>(Tracked(ghcb_perm));
                rax = ax.into();
                rdx = dx.into();
    ...
    pub fn ghcb_write_msr(
        ...
        reg: u32,
        val: u64,
        Tracked(cs): Tracked<&mut SnpCoreSharedMem>) -> (ret: Self)
    requires
        self.ghcb_wf(),
        (*old(cs)).inv(),
    ensures
    ...
    cs.only_lock_reg_coremode_updated((*old(cs)), set![GHCB_REGID()], set![spec_PT().lockid()]),
{
    ...
    proof{page_perm0.tracked_insert(0, page_perm);}
...
pub fn ghcb_page_proto(
    ...
    Tracked(cs): Tracked<&mut SnpCoreSharedMem>) -> (ret: SvmStatus)
requires
    (*old(cs)).inv(),
