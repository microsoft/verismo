...
pub open spec fn mk_private_ensures_pageperm(
    ...
    &&& page_perm.snp() === prev_page_perm.snp().spec_set_pte(page_perm.snp().pte).spec_set_rmp(page_perm.snp().rmp)
    &&& page_perm.snp().rmp === SwSnpMemAttr::spec_default().rmp
}

pub open spec fn spec_is_shared_page_perms(page_perms: Map<int, SnpPointsToRaw>, start_page: int, npages: nat) -> bool {
    forall |i| start_page <= i < (start_page + npages) ==>
        #[trigger]page_perms.contains_key(i) &&
        page_perms[i]@.wf_range((i.to_addr(), PAGE_SIZE as nat)) &&
        page_perms[i]@.snp() === SwSnpMemAttr::shared()
}

pub open spec fn spec_is_vmprivate_const_perms(page_perms: Map<int, SnpPointsToRaw>, start_page: int, npages: nat) -> bool {
    forall |i| start_page <= i < (start_page + npages) ==>
        #[trigger]page_perms.contains_key(i) &&
        page_perms[i]@.wf_range((i.to_addr(), PAGE_SIZE as nat)) &&
...
pub fn early_mk_shared(start_page: u64,
    ...
    mk_shared_ensures_pageperm(old(page_perm0)[0]@, page_perm0[0]@),
{
    ...
    ghcb_change_page_state_via_msr(vn_to_pn(start_page, Tracked(&page_perm)) as usize, page_op, Tracked(&mut page_perm), Tracked(&mut cs.snpcore));
...
impl GhcbHandle {
        ...
        ret.1.ghcb_wf()
    {
        ...
        proof {
            ...
            oldcs.lemma_update_prop(prevcs, (*cs), set![], set![spec_ALLOCATOR_lockid()], set![], set![spec_PT_lockid()]);
            assert(set![spec_ALLOCATOR_lockid()].union(set![spec_PT_lockid()]) =~~= set![spec_ALLOCATOR_lockid(), spec_PT_lockid()]);
            assert(mk_shared_ensures_pageperm(old_page_perms[page as int]@, page_perms[page as int]@));
            lemma_mk_shared_default_to_shared(old_page_perms[page as int]@, page_perms[page as int]@);
            ...
            mk_shared_ensures_pageperm(old(page_perms)[i]@, page_perms[i]@),
    {
        ...
        let ret = self.ghcb_change_page_state_via_pg(
            ...
            npages, PageOps::Shared, Tracked(page_perms), Tracked(cs));
        let ghost prevcs = (*cs);
        proof {
            assert(forall |i| start_page <= i < (start_page + npages)
            ==> #[trigger]page_perms.contains_key(i) && ensure_page_perm_change_state(old_page_perms[i], page_perms[i], i, PageOps::Shared));
            assert forall |i| start_page <= i < (start_page + npages)
            implies
            #[trigger]page_perms.contains_key(i) &&
            ensure_page_perm_change_state(old_page_perms[i], page_perms[i], i, PageOps::Shared)
            by{
                assert(old_page_perms.contains_key(i));
                assert(page_perms.contains_key(i));
            }
            assert forall |i: int| start_page <= i < start_page + npages
            implies
                page_perms.contains_key(i) && page_perms[i]@.wf_range((i.to_addr(), PAGE_SIZE as nat))
            by{
                assert(page_perms.contains_key(i));
                assert(old_page_perms.contains_key(i));
        ...
        proof{
            oldcs.lemma_update_prop(prevcs, (*cs), set![], set![], set![], set![spec_PT().lockid()]);
            assert(set![].union(set![spec_PT().lockid()]) =~~= set![spec_PT().lockid()]);
            assert forall |i| start_page <= i < (start_page + npages)
            implies
                #[trigger]page_perms.contains_key(i) &&
                mk_shared_ensures_pageperm(old_page_perms[i]@, page_perms[i]@)
            by {
                assert(old_page_perms.contains_key(i));
    ...
    pub fn mk_private(
        ...
        start_page: u64, npages: u64,
        Tracked(cs): Tracked<&mut SnpCoreSharedMem>,
        Tracked(page_perms): Tracked<&mut Map<int, SnpPointsToRaw>>) -> (ret: Self)
    requires
        self.ghcb_wf(),
        (*old(cs)).inv(),
        npages > 0,
            ...
            mk_private_ensures_pageperm(old(page_perms)[i]@, page_perms[i]@),
    {
        ...
        let ret = self.ghcb_change_page_state_via_pg(
            vn_to_pn(start_page, Tracked(page_perms.tracked_borrow(start_page as int))),
            npages, PageOps::Private, Tracked(page_perms), Tracked(cs));
        let ghost prevcs = (*cs);
        proof {
            assert forall |i: int| start_page <= i < start_page + npages
            implies
        ...
        proof{
            ...
            implies
                #[trigger]page_perms.contains_key(i) &&
                spec_perm_requires_pvalidate(page_perms[i], i.to_addr(), PAGE_SIZE as nat, true)
            by {
                assert(prev_page_perms.contains_key(i));
                assert(page_perms.contains_key(i));
        ...
        pvalmem2(start_page.to_addr(), end_page.to_addr(), true, Tracked(page_perms), Tracked(&mut cs.snpcore));
        proof {
            assert forall |i| start_page <= i < (start_page + npages)
            implies
            ...
            by {
                ...
                assert(page_perm.snp().pte() === prev_page_perm.snp().pte().spec_set_encrypted(true));
                assert(page_perm.snp() === prev_page_perm.snp().spec_set_pte(page_perm.snp().pte).spec_set_rmp(page_perm.snp().rmp));
