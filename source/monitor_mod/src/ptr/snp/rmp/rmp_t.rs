...
verus! {
    ...
    pub fn pvalidate(vaddr: u64, psize: u64, validate: u64, rflags: &mut u64, Tracked(mycore): Tracked<&SnpCore>, Tracked(perm): Tracked<&mut SnpPointsToRaw>) -> (ret: u64)
    requires
        old(perm)@.snp().requires_pvalidate(vaddr as int, psize as int, validate as int),
        mycore.coreid@.vmpl == 0,
        mycore.inv(),
    ensures
        old(perm)@.snp.pvalidate_ret(perm@.snp, ret, *rflags, vaddr as int, psize as int, validate as int),
    ...
    #[verifier(external_body)]
    pub fn rmpadjust(vaddr: u64, psize: u64, attr: RmpAttr,
