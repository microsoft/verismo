...
verus! {
    pub open spec fn spec_perm_requires_pvalidate(
        perm: SnpPointsToRaw, addr: int, size: nat, val: bool
    ) -> bool {
        &&& perm@.snp().requires_pvalidate(
            addr, RMP_4K as int, if val {1} else {0})
        &&& perm@.snp_wf_range((addr, size as nat))
    }

    pub open spec fn spec_perms_requires_pvalidate(
        page_perms: Map<int, SnpPointsToRaw>, start_page: int, end_page: int, val: bool
    ) -> bool {
        forall |i| start_page <= i < end_page ==>
    ...
    pub open spec fn spec_perm_ensures_pvalidate(
        perm: SnpPointsToRaw, old_perm: SnpPointsToRaw, start: int, size: nat, val: bool
    ) -> bool {
    ...
    pub open spec fn spec_perms_ensures_pvalidate(
            ...
            start_page: int, end_page: int, val: bool) -> bool {
        &&& old_perms.dom() =~~= page_perms.dom()
        &&& forall |i| start_page <= i < end_page ==>
            #[trigger]page_perms.contains_key(i) &&
            spec_perm_ensures_pvalidate(page_perms[i], old_perms[i], i.to_addr(), PAGE_SIZE as nat, val)
    }

    pub fn pvalmem2(start: u64, end: u64, val: bool, Tracked(perms): Tracked<&mut Map<int, SnpPointsToRaw>>, Tracked(snpcore): Tracked<&mut SnpCore>)
    requires
        end as int % PAGE_SIZE!() == 0,
        start as int % PAGE_SIZE!() == 0,
        start < end,
        spec_perms_requires_pvalidate(*old(perms), (start as int).to_page(), (end as int).to_page(), val),
        old(snpcore).inv(),
        old(snpcore).coreid@.vmpl == 0,
    ensures
        spec_perms_ensures_pvalidate(*perms, *old(perms), (start as int).to_page(), (end as int).to_page(), val),
        snpcore.inv(),
        *old(snpcore) === *snpcore,
    {
        ...
        invariant
            ...
            forall |i| (start as int).to_page() <= i < (end as int).to_page() ==>
                #[trigger]old_perms.contains_key(i),
            forall |i| (vaddr as int).to_page() <= i < (end as int).to_page() ==>
                #[trigger]perms.contains_key(i) &&
                old_perms.contains_key(i) &&
                perms[i] === old_perms[i],
            spec_perms_requires_pvalidate(old_perms, (start as int).to_page(), (end as int).to_page(), val),
            spec_perms_requires_pvalidate(*perms, (vaddr as int).to_page(), (end as int).to_page(), val),
            spec_perms_ensures_pvalidate(*perms, old_perms, (start as int).to_page(), (vaddr as int).to_page(), val),
            snpcore.inv(),
            ...
            old_snpcore === *snpcore,
        {
            ...
            let Tracked(current_perm) = pvalmem(vaddr, next_vaddr, val, Tracked(current_perm), Tracked(snpcore));
            assert((vaddr as int).to_page() + 1 == (next_vaddr as int).to_page());
            vaddr = next_vaddr;
            proof {
                perms.tracked_insert(pn, current_perm);
                assert(perms.contains_key(pn));
                assert forall |i| (vaddr as int).to_page() <= i < (end as int).to_page()
                implies
                    #[trigger]perms.contains_key(i) &&
                    old_perms.contains_key(i) &&
                    perms[i] === old_perms[i]
                ...
                assert forall |i| (start as int).to_page() <= i < (vaddr as int).to_page()
                implies
                    #[trigger]perms.contains_key(i) &&
                    spec_perm_ensures_pvalidate(perms[i], old_perms[i], i.to_addr(), PAGE_SIZE as nat, val)
                by {
                    assert(old_perms.contains_key(i));
                    assert(perms.contains_key(i));
                }
                ...
                implies
                    #[trigger]perms.contains_key(i) &&
                    spec_perm_requires_pvalidate(perms[i], i.to_addr(), PAGE_SIZE as nat, val)
                by {
                    assert(old_perms.contains_key(i));
                    assert(perms.contains_key(i));
                }
                assert(spec_perms_ensures_pvalidate(
                        *perms, old_perms, (start as int).to_page(), (vaddr as int).to_page(), val
                ));
            }
        }
    }

    pub fn pvalmem(start: u64, end: u64, val: bool, Tracked(perm): Tracked<SnpPointsToRaw>, Tracked(snpcore): Tracked<&mut SnpCore>) -> (ret: Tracked<SnpPointsToRaw>)
    requires
        ...
        *old(snpcore) === *snpcore,
    {
        ...
        let ghost gvalidate: int = if val {1} else {0};
        ...
        invariant
            ...
            vaddr as int % PAGE_SIZE!() == 0,
            (start as int) <= (vaddr as int) <= (end as int),
            gvalidate == if val {1int} else {0int},
            ...
            oldsnpcore === *snpcore,
        {
            ...
            let validate = if val {1} else {0};
            let rc = pvalidate(vaddr, RMP_4K, validate, &mut rflags, Tracked(snpcore), Tracked(&mut current_perm));
            ...
            proof {
                ...
                retp = retp.trusted_join(current_perm);
                perm = next_perm;
                assert(retp@.snp.wf());
                assert(retp@.range() === (start as int, (vaddr - start) as nat));
                assert(old_perm@.bytes().wf() ==> current_perm@.bytes().wf());
            }
        }
        Tracked(retp)
    }

    pub open spec fn spec_rmpadjmem_requires_at(page_perm: SnpPointsToRaw, i: int, attr: RmpAttrSpec) -> bool {
    ...
    pub open spec fn spec_rmpadjmem_requires(page_perms: Map<int, SnpPointsToRaw>, start_page: int, npages: nat, attr: RmpAttrSpec) -> bool {
    ...
    pub open spec fn spec_ensures_rmpadjust(prev_perm: SnpPointsToRaw, perm: SnpPointsToRaw, page: int, attr: RmpAttrSpec) -> bool {
    ...
    pub open spec fn spec_ensures_rmpadjmem(prev_perms: Map<int, SnpPointsToRaw>, page_perms: Map<int, SnpPointsToRaw>, start_page: int, npages: nat, attr: RmpAttrSpec) -> bool {
        &&& forall |i| start_page <= i < (start_page + npages) ==>
            #[trigger]page_perms.contains_key(i) &&
    ...
    pub fn rmpadjmem(start_page: usize, npages: usize, attr: RmpAttr, Tracked(snpcore): Tracked<&mut SnpCore>, Tracked(perms): Tracked<&mut Map<int, SnpPointsToRaw>>)
    requires
        !attr@.is_vmsa(),
        ...
        *old(snpcore) === *snpcore,
    {
        ...
        invariant
            0 <= i <= npages,
            forall |k| start_page as int + i <= k < start_page as int + npages ==> old_perms[k] === page_perms[k],
            ...
            *snpcore === old_snpcore,
        {
            ...
            let rc = rmpadjust(vaddr, RMP_4K, attr, Tracked(snpcore), Tracked(None), Tracked(&mut current_perm));
            if rc != 0 {
                // failed validation ==> possible attack
                vc_terminate(SM_TERM_INVALID_PARAM, Tracked(snpcore));
            }
            ...
            proof {
                ...
                assert(page_perms.contains_key(page as int));
                assert forall |k| start_page as int + i <= k < start_page as int + npages
                implies
                    old_perms[k] === page_perms[k]
                by{
                    assert(old_perms.contains_key(k));
                    assert(page_perms.contains_key(k));
                }
                assert forall |k| start_page as int + i <= k < start_page as int + npages
                implies (#[trigger]page_perms.contains_key(k)) && spec_rmpadjmem_requires_at(page_perms[k], k, attr@)
                by {
                    assert(old_perms.contains_key(k));
                    assert(page_perms.contains_key(k));
                ...
                assert forall |k| start_page <= k < (start_page + i)
                implies
                    #[trigger]page_perms.contains_key(k) &&
                    spec_ensures_rmpadjust(old_perms[k], page_perms[k], k, attr@)
                by{
                    assert(page_perms.contains_key(k));
                    assert(old_perms.contains_key(k));
                }
    ...
    pub fn rmpadjust2(vaddr: u64, psize: u64, attr: RmpAttr,
        Tracked(mycore): Tracked<&SnpCore>,
        Tracked(newcore): Tracked<Option<CoreIdPerm>>,
        Tracked(perm): Tracked<&mut SnpPointsToRaw>) -> (ret: u64)
    requires
