...
verus! {
    impl SwSnpMemAttr {
        ...
        {
            let rmp = prev.rmp@;
            let asid = if shared {ASID_FOR_HV!()} else {rmp.spec_asid()};
            let hidden = HiddenRmpEntryForPSP {
                ...
                asid,
                gpn: rmp.spec_gpn(),
                size: if page_2m {PageSize::Size2m} else {PageSize::Size4k},
                perms: rmp_perm_init()
            };
        ...
        pub open spec fn requires_pvalidate(&self, vaddr: int, is_2m: int, val: int) -> bool {
            ...
            &&& (val % 2 == 0) == self.rmp@.spec_validated()
        ...
        pub open spec fn requires_rmpadjust(&self, vaddr: int, is_2m: int, attr: RmpAttrSpec, newcore: Option<CoreIdPerm>, memperm: SnpPointsToBytes) -> bool {
        ...
        pub open spec fn requires_rmpadjust_mem(&self, vaddr: int, is_2m: int, attr: RmpAttrSpec, newcore: Option<CoreIdPerm>) -> bool {
            &&& is_2m % 2 == 0  // Only support 4k page
            &&& 1 <= attr.spec_vmpl() <= 3
        ...
        pub open spec fn ensures_rmpadjust(&self, prev: Self, attr: RmpAttrSpec) -> bool {
            &&& self.rmp@ === prev.rmp@.spec_set_perms(prev.rmp@.perms.insert(attr.vmpl(), attr.perms())).spec_set_vmsa(attr.is_vmsa())
            &&& *self === prev.spec_set_rmp(self.rmp)
        }
    }
    ...
    impl HwSnpMemAttr {
        ...
        pub open spec fn valid_memmap(self, start: int, size: nat) -> bool {
            forall |vaddr| start <= vaddr < start + size ==> self.guestmap[vaddr] == self.rmpmap[self.sysmap[self.guestmap[vaddr]]]
        }
        ...
        // True -> Return
        // False -> Never return
        pub open spec fn rmpadjust_ret(self, new: Self, rax: u64, vmid: nat, vaddr: int, psize: PageSize, vmpl: VMPL, is_vmsa: bool, perms: PagePerm) -> bool
        recommends
            vmid > 0
        {
            let memid = MemID::Guest((vmid - 1) as nat, VMPL::VMPL0);
            let ret = self.rmp.rmpadjust(
                memid,
                vmpl, psize, GPA::new(self.guestmap[vaddr]).to_page(), is_vmsa, perms);
            let size = (if psize.is_Size4k() {PAGE_SIZE!()} else {0x2000_000}) as nat;
            let map_ok = new.valid_memmap(vaddr, size);
            if map_ok {
                if let ResultWithErr::Error(_, memerr) = ret {
                    let arch: Archx64 = arbitrary();
                    let memop: MemOp<GuestVir> = choose |memop: MemOp<GuestVir>| memop.is_RmpOp()&& memop.get_RmpOp_0().is_RmpAdjust();
                    let op = choose |op: Archx64Op| op.to_memid() === memid && op.is_MemOp() && op.get_MemOp_0() === memop;
        ...
        // True -> Return
        // False -> Never return
        pub open spec fn pvalidate_ret(self, new: Self, rax: u64, rflags: u64, vmid: nat, vaddr: int, psize: PageSize, val: bool) -> bool
        recommends
            vmid > 0
        {
            let memid = MemID::Guest((vmid - 1) as nat, VMPL::VMPL0);
            ...
            let size = if psize.is_Size4k() {PAGE_SIZE!()} else {0x2000_000};
            let map_ok = self.valid_memmap(vaddr, size as nat);

            if map_ok {
                if let ResultWithErr::Error(_, memerr) = ret {
                    let arch: Archx64 = arbitrary();
                    let memop: MemOp<GuestVir> = choose |memop: MemOp<GuestVir>| memop.is_RmpOp() && memop.get_RmpOp_0().is_Pvalidate();
                    let op = choose |op: Archx64Op| op.to_memid() === memid && op.is_MemOp() && op.get_MemOp_0() === memop;

                    let (trap, trans) = Archx64::handle_mem_err_fn(MemError::from_err(memerr, memop));
                    if !trap {
                        &&& rax == trans(arch, op).spec_regdb()[op.cpu_memid()].spec_index(RegName::Rax)
                        &&& rflags == trans(arch, op).spec_regdb()[op.cpu_memid()].spec_index(RegName::Rflags)
                        &&& self === new
                        &&& arch.is_run(op.cpu_memid())
                    } else {
                        false
    ...
    impl SnpMemAttr {
        ...
        pub open spec fn pvalidate_ret(self, new: Self, rax: u64, rflags: u64, vaddr: int, is_2m: int, val: int) -> bool
        {
        ...
        pub open spec fn rmpadjust_ret(self, new: Self, rax: u64, vaddr: int, is_2m: int, attr: RmpAttrSpec) -> bool {
            let psize = if is_2m % 2 == 0 {PageSize::Size4k} else {PageSize::Size4k};
            &&& self.hw.rmpadjust_ret(new.hw, rax, self.sw.rmp@.asid, vaddr, psize, attr.vmpl(), attr.is_vmsa(), attr.perms())
            &&& if (rax == 0) {
