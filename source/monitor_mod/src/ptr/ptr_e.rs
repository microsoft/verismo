...
verus! {
    ...
    impl<V: IsConstant + WellFormed + SpecSize> SnpPPtr<V> {
            ...
            v.wf(),
        {
            ...
            proof {
                HwSnpMemAttr::proof_hvupdate_rel_propograte(perm@.hw_snp(), old(perm)@.hw_snp(), old(perm)@.snp());
            }
            v
        }
    ...
    impl<F: IsConstant + WellFormed + SpecSize + VTypeCast<SecSeqByte> + Clone> SnpPPtr<F> {
        pub fn copy_with<T2: IsConstant + WellFormed + SpecSize + VTypeCast<SecSeqByte>>(
            &self, Tracked(perm): Tracked<SnpPointsTo<T2>>
        ...
        requires
            ...
            self.is_constant(),
        ensures
            perm@.snp().ensures_read(Some(field_at(perm@.get_value(), (self.id() - perm@.id())as nat)), ret.0),
            ret.1@@ === perm@,
        {
            proof {
                reveal(field_at);
            ...
            assert(fieldp@.bytes() =~~= perm@.bytes().subrange(offset as int, offset as int + spec_size::<F>()));
    ...
    impl<F: IsConstant + WellFormed + SpecSize + VTypeCast<SecSeqByte>> SnpPPtr<F> {
        ...
        pub fn replace_with<T2: IsConstant + WellFormed + SpecSize + VTypeCast<SecSeqByte>>(
            &self, in_v: F, Tracked(perm): Tracked<SnpPointsTo<T2>>
        ) -> (ret: (F, Tracked<SnpPointsTo<T2>>))
        requires
            in_v.wf(),
            ...
            ret.1@@.wf(),
        {
            ...
            proof {
                ...
                assert(right@.bytes() =~~= old_perm@.bytes().skip(offset as int + spec_size::<F>() as int));
                assert(
                    ret_perm@.bytes()=~~= left@.bytes() + in_v.vspec_cast_to() + right@.bytes()
                );
            }
            (ret, Tracked(ret_perm.tracked_into()))
        }
