...
requires
    ...
    page_perm@.snp().guestmap[(vaddr as int).to_page()] == spec_va_to_pa(vaddr as int).to_page(),
ensures
...
requires
    ...
    page_perm@.snp().guestmap[spec_pa_to_va(paddr as int).to_page()] == (paddr as int).to_page(),
ensures
...
pub closed spec fn pte_perms_wf_prev(perms: PtePerms, lvl: nat, addr: int) -> bool
{
    ...
    &&& (perms.contains_key(prev_idx) &&
    perms[prev_idx].has_next()) == perms.contains_key(idx)
    &&& perms[idx].val().value ===
    perms[prev_idx].next_tb()[spec_table_index(addr as u64, lvl)].vspec_cast_to()
}
...
pub open spec fn wf_ptes(m: Map<(nat, int), PtePerm>) -> bool {
    ...
    &&& forall |i| m.contains_key(i) ==> #[trigger]m[i].wf(i)
    // When an entry exists
    &&& forall |lvl: nat, addr:int| 0 <= lvl < 4
        ==> pte_perms_wf_prev(m, lvl, addr)
}
...
spec fn pte_perms_contains(perms: PtePerms, lvl: nat, addr: int) -> bool {
{
    ...
    &&& forall |l: nat| lvl <= l < 4 ==> #[trigger]perms.contains_key(lvl_index(l, addr))
...
{
    proof {proof_table_index(vaddr as u64, lvl as nat);}
    let ret = if lvl == 0 {
...
pub struct PTEWithPtr {
    ...
    pub fn check_is_encrypted(vaddr: usize,
    ...
    fn borrow_entry(vaddr: u64, lvl: u8,
        Tracked(mem_perm): Tracked<&SnpPointsToRaw>,
        Tracked(cs): Tracked<&mut SnpCoreSharedMem>)
        ...
        (ret.is_Some() && (lvl == 0)) ==>  wf_pte_mem_perm(ret.get_Some_0().pte, mem_perm)
    {
        ...
        let (tracked_ptr, Tracked(mut ptperm_perm), Tracked(pt_lock)) =
        PT().acquire(Tracked(pt_lock), Tracked(&cs.snpcore.coreid));
        ...
        let ret = _borrow_entry(vaddr, lvl, Tracked(mem_perm), Tracked(cr3perm), Tracked(&mut pt_perms));
    ...
    pub closed spec fn cr3_to_pte_ptr(cr3perm: RegisterPermValue<u64_s>) -> PTEWithPtr {
        PTEWithPtr {
            pte:  PTE::spec_new(cr3perm.value.vspec_cast_to()),
            ptr: None,
            index: 0,
        }
    }

    fn _borrow_entry(vaddr: u64, lvl: u8,
        Tracked(mem_perm): Tracked<&SnpPointsToRaw>,
    ...
    ensures
        *pt_perms =~~= *old(pt_perms),
        ret.is_Some() ==> ret.is_constant(),
        ret.is_Some() ==>
            pt_perms[lvl_index(lvl as nat, vaddr as int)].val() ===
            ret.get_Some_0().pte,
        ret.is_Some() ==>
            ret.get_Some_0().index == spec_table_index(vaddr, lvl as nat),
        ret.is_Some() ==>
            ret.get_Some_0().ptr.is_Some() == (lvl != PAGE_TABLE_LEVELS),
        ret.is_Some() && ret.get_Some_0().ptr.is_Some() ==>
            ret.get_Some_0().ptr.get_Some_0().id() == pt_perms[lvl_index(lvl as nat + 1, vaddr as int)].next_tbptr(),
        ...
        lvl == PAGE_TABLE_LEVELS ==> ret === Some(cr3_to_pte_ptr(cr3perm@)),
    {
        ...
        proof {lvl_idx = lvl_index(glvl, gaddr);}
        if lvl == PAGE_TABLE_LEVELS {
            ...
            return Some(PTEWithPtr{
                pte: PTE::new(cr3.into()),
                ptr: None,
                index: 0,
            });
        }
        ...
        let prev = _borrow_entry(vaddr, lvl + 1, Tracked(mem_perm), Tracked(cr3perm), Tracked(&mut prev_pt_perms));
        ...
        let ret = if let Some(tmp) = prev {
            ...
            if (lvl + 1) == PAGE_TABLE_LEVELS || (prev_entry.get_psize() == 0 && prev_entry.get_present() == 1) {
                proof {
                    ...
                    assert(PTE::spec_new(prev.get_Some_0().pte.value.vspec_cast_to()) === pt_perms[prev_lvl_idx].val());
...
pub open spec fn write_pte_requires_memperm(
    ...
    forall |i: int| start <= i < end && i % PAGE_SIZE!() == 0
        ==> #[trigger]mem_perms.contains_key(i) &&
...
requires
    ...
    old(pte_perms)[lvl_index(lvl + 1, addr)].next_tbptr() + spec_table_index(addr as u64, lvl) * 8 == self.id(),
    addr % (lvl_to_size(lvl) as int) == 0,
    write_pte_requires_memperm(val, addr, lvl, *old(mem_perms)),
ensures
    pte_perms[lvl_index(lvl, addr)].val === val,
    write_pte_ensures(addr, lvl, old(pte_perms)[lvl_index(lvl, addr)].val, val, *old(mem_perms), *mem_perms),
{
    ...
    mem_perm0[0]@.wf_range((vaddr as int, PAGE_SIZE as nat)),
{
    ...
    let lvl = 0;
    assert(wf_ptes(pt_perms));
    let pte_val_opt = _borrow_entry(vaddr, lvl, Tracked(mem_perm0.tracked_borrow(0)), Tracked(cr3perm), Tracked(&mut pt_perms));
    let ret = if let Option::Some(pte_with_ptr) = pte_val_opt {
        let PTEWithPtr{pte, ptr, index} = pte_with_ptr;
        assert(ptr.is_Some());
        let mut pte_addr: usize = ptr.unwrap().to_usize();
        let encryption = if enc{1} else {0};
        if pte.get_encrypted() != encryption {
            ...
            proof{assert(memmap_perm.contains_key(addr_id));}
            pte_ptr.write_pte(new_pte,
                Ghost(lvl as nat), Ghost(addr_id),
                Tracked(&mut pt_perms),  Tracked(&mut memmap_perm));
            proof{assert(memmap_perm.contains_key(addr_id));}
            let tracked mut mem_perm = memmap_perm.tracked_remove(addr_id);
            invlpg(vaddr, Tracked(&mut mem_perm));
            proof {mem_perm0.tracked_insert(0, mem_perm);}
            true
        } else {
...
pub fn set_pages_enc_dec(start_page: u64, size: u64, enc: bool,
    Tracked(cs): Tracked<&mut SnpCoreSharedMem>,
    Tracked(mem_perms): Tracked<&mut Map<int, SnpPointsToRaw>>
)
requires
    ...
    forall |page: int| start_page <= page < start_page + size ==>
        old(mem_perms).contains_key(page) && old(mem_perms)[page]@.wf_range((page.to_addr(), PAGE_SIZE as nat)),
ensures
    cs.inv(),
    cs.only_lock_reg_updated((*old(cs)), set![], set![spec_PT().lockid()]),
    mem_perms.dom() =~~= old(mem_perms).dom(),
    forall |page: int| start_page <= page < start_page + size ==>
        #[trigger]mem_perms.contains_key(page) &&
        ...
        ensures_mem_enc_dec_memperm(enc, old(mem_perms)[page], mem_perms[page])
{
    ...
    invariant
        ...
        forall |page: int| start_page <= page < start_page + size ==>
            #[trigger]mem_perms.contains_key(page) && mem_perms[page]@.wf_range((page.to_addr(), PAGE_SIZE as nat)),
        forall |page: int| start_page + i <= page < start_page + size ==>
            old_mem_perms[page] === #[trigger]mem_perms[page],
        forall |page: int| start_page <= page < start_page + i ==>
            #[trigger]mem_perms.contains_key(page) &&
            ensures_mem_enc_dec_memperm(enc, old_mem_perms[page], mem_perms[page])
    {
        ...
        proof {
            ...
            old_cs.lemma_update_prop(prev_cs, (*cs), set![], set![spec_PT().lockid()], set![], set![spec_PT().lockid()]);
...
pub open spec fn wf_pte_mem_perm(pte: PTE, perm: &SnpPointsToRaw) -> bool {
    &&& (pte@.spec_encrypted() == 1) == perm@.snp().pte().spec_encrypted()
    /*&&& (pte@.spec_nx() == 0)== perm@.snp().pte().spec_x()
