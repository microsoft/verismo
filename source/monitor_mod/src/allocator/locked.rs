...
impl VSpinLock<VeriSMoAllocator> {
    ...
    pub fn alloc_(&self, size: usize, align: usize,
    ...
    ensures
        ...
        res.0.is_Ok() ==> talloc_valid_ptr(size, res.0.get_Ok_0()) && (res.0.get_Ok_0().0 as int) % (align as int) == 0,
    {
        use crate::debug::VEarlyPrintAtLevel;
        let tracked alloc_lockperm = alloc_lockperm;
        (new_strlit("\n new")).leak_debug();
        let (ptr, Tracked(mut allocperm), Tracked(alloc_lockperm)) =
            self.acquire(Tracked(alloc_lockperm), Tracked(coreid));
    ...
    pub fn alloc_aligned(&self, size: usize, align: usize,
    ...
    ensures
        ...
        res.is_Ok() ==> talloc_valid_ptr(size, res.get_Ok_0()) && (res.get_Ok_0().0 as int) % (align as int) == 0,
    {
        let tracked alloc_perm = alloc_lockperm0.tracked_remove(0);
        let (ret, Tracked(alloc_perm)) = self.alloc_(size, align, Tracked(alloc_perm), Tracked(coreid));
        proof {alloc_lockperm0.tracked_insert(0, alloc_perm);}
        ret
    }

    pub fn dealloc_(&self, addr: usize, size: usize,
        ...
        self.lock_alloc_requires(coreid@.cpu, newlockperm@@),
    {
        ...
        let (ptr, Tracked(mut allocperm), Tracked(mut alloc_lockperm)) =
            self.acquire(Tracked(alloc_lockperm), Tracked(coreid));
        let mut allocator = ptr.take(Tracked(&mut allocperm));
