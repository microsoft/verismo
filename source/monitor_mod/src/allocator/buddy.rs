...
impl SpecBuddyAllocator {
    ...
    pub open spec fn wf_perm(&self, bucket: nat, i: nat) -> bool {
        if (bucket < self.free_lists.len() && i < self.free_lists[bucket as int]@.len()) {
            let SpecListItem {ptr: node_ptr, snp: node_snp, val} = self.free_lists[bucket as int]@[i as int];
    ...
    pub open spec fn spec_add_one_to_bucket2(
        ...
        &&& self.free_lists =~~= old_self.free_lists.update(bucket as int, self.free_lists[bucket as int])
        &&& newlist@ =~~= list@
        .push(SpecListItem {ptr: nodeptr, snp: node_perm@.snp(), val: node_perm@.value().get_Some_0().val})
    }
        ...
        old_self.inv() == self.inv(),
    {
        ...
        assert(self.inv()) by {
            ...
            //}
            assert forall |b: nat| b < self.free_lists.len()
            implies self.wf_bucket(b)
            by{
            ...
            assert forall |b: nat, i: nat| #[trigger] self.wf_perm(b, i)
            by{
        ...
        if self.inv() {
            assert(old_self.inv()) by {
                assert forall |b: nat| b < old_self.free_lists.len()
                implies old_self.wf_bucket(b)
                by{
                    assert(self.wf_bucket(b));
                }
                assert forall |b: nat, i: nat| #[trigger] old_self.wf_perm(b, i)
                by{
                    assert(self.wf_perm(b, i));
                }
            }
    ...
    pub open spec fn spec_remove_or_insert_one(&self, old_self: Self, bucket: nat, idx: nat) -> bool {
        let list = self.free_lists[bucket as int];
        let old_list = old_self.free_lists[bucket as int];
    ...
    pub open spec fn _spec_remove_or_insert_one2(
        ...
        &&& old_self.perms =~~= Map::new(
            ...
            |k: (nat, nat)| if k.0 != bucket || k.1 < idx {self.perms[k]} else {self.perms[(k.0, (k.1 + 1) as nat)]}
        )
        &&& self.perms[(bucket, idx)] === free_perm
        /* &&& self.perms =~~= Map::new(
        ...
        &&& self.free_lists =~~= old_self.free_lists.update(bucket as int, self.free_lists[bucket as int])
        &&& newlist@ =~~= list@.insert(idx as int, SpecListItem {
            ptr: nodeptr, snp: node_perm@.snp(), val: node_perm@.value().get_Some_0().val})
        ...
        old_self.inv() == self.inv(),
    {
        ...
        let left = list@.subrange(0, idx as int).push(SpecListItem {
            ptr: nodeptr, snp: node_perm@.snp(), val: node_perm@.value().get_Some_0().val});
...
impl BuddyAllocator {
        ...
        ensures
            ...
            forall |i: int| 0 <= i < ORDER_USIZE as int ==> ret@.free_lists[i]@.len() == 0,
            //forall |i: int| 0 <= i < ORDER_USIZE as int ==> ret@.free_lists[i].is_Some(),
        {
            ...
            proof {
                assert forall |bucket: nat| bucket < ret@.free_lists.len()
                implies ret@.wf_bucket(bucket)
                by{
        ...
        fn pop(
            &mut self,
            bucket: usize,
        ) -> (ret: Option<(usize_t, Tracked<SnpPointsToRaw>)>)
        requires
            old(self)@.inv(),
        ...
        ensures
            self@.inv(),
            ret.is_Some() ==> {
                ret.get_Some_0().1@@.wf_freemem((ret.get_Some_0().0 as int, spec_bit64(bucket as u64) as nat))
            },
            ...
            }
         {
            ...
            proof {bit_shl64_pow2_auto();}
            proof {assert(self@.wf_bucket(gbucket));}
            ...
            match ret {
                Some((nodeptr, tnode_perm)) => {
                    let tracked free_perm = self.perms.borrow_mut().tracked_remove((gbucket, list@.len()));
                    proof {
                        assert(old_self@.perms =~~= self@.perms.insert((gbucket, list@.len()), free_perm));
                        assert(old_self@.perms.remove((gbucket, list@.len())) =~~= self@.perms);
                        assert(old_self@.free_lists =~~= self@.free_lists.update(gbucket as int, old_self.free_lists[gbucket as int]));
                        assert(list@ =~~= old_list@.drop_last());
                        assert(old_list@.last() === SpecListItem {
                            ptr: nodeptr, snp: tnode_perm@@.snp(), val: tnode_perm@@.value().get_Some_0().val});
                        assert(old_list@.drop_last() =~~= list@);
                        old_self@.proof_add_one_to_bucket(&self@, gbucket, nodeptr, tnode_perm@, free_perm);
        ...
        pub fn alloc_inner(&mut self, size: usize, align: usize) -> (ret: Option<(usize, Tracked<SnpPointsToRaw>)>)
            requires
        ...
        pub fn _alloc_inner(&mut self, size: &mut usize_t, align: usize_t) -> (ret: Option<(usize_t, Tracked<SnpPointsToRaw>)>)
        requires
            spec_bit64_is_pow_of_2(align as int),
            ...
            size.is_constant(),
        {
            ...
            *size = max(
                max(align as u64, MIN_ADDR_ALIGN as u64),
                next_power_of_two((*size) as u64),
            ) as usize;
            assert(*size < POW2!(32));
                ...
                current_addr as u64 % (spec_bit64(current_bucket as u64)) == 0,
            {
                ...
                if let Some((ptr, tperm)) = removed.pop() {
                    ...
                    proof {
                        ...
                        removed_perm = removed_node_perm.trusted_into_raw().trusted_join(removed_free_perm);
                        let key_map = Map::new(
                            |k: (nat, nat)| k.0 < ORDER && k.1 < prev_self.free_lists[k.0 as int]@.len()
                                        && k !== (current_bucket as nat, (prev_list@.len() -1) as nat),
                            |k: (nat, nat)| if k.0 != current_bucket as nat || k.1 < removed_id as nat {
                                k
                            } else {
                                (k.0, k.1 + 1)
                            }
                        ...
                        assert forall|j1: (nat, nat), j2: (nat, nat)|
                            !builtin::spec_eq(j1, j2) && key_map.dom().contains(j1) && key_map.dom().contains(j2)
                        implies
                            !builtin::spec_eq(key_map.index(j1), key_map.index(j2))
                        by {
                            assert(prev_self.wf_perm(j1.0, j1.1));
                        }
                        assert forall|j| key_map.dom().contains(j)
                        implies
                            self@.perms.dom().contains(key_map.index(j))
                        by{
                            let (bb, ii) = j;
                        ...
                        assert(self@.inv()) by {
                            assert forall |k| k !== (current_bucket as nat, (prev_list@.len() - 1) as nat)
                            implies prev_self.perms.contains_key(k) == self@.perms.contains_key(k)
                            by {
                            ...
                            assert(prev_self.perms.dom().remove((current_bucket as nat, (prev_list@.len() - 1) as nat)) =~~= self@.perms.dom());
                            prev_self.proof_remove_or_add_idx(&self@, current_bucket as nat, removed_id as nat, ptr, removed_node_perm, removed_free_perm);
                        }
...
impl BuddyAllocator {
        ...
        spec_is_align_down_by_int(*old(end) as int, MIN_ADDR_ALIGN!() as int, *end as int),*/
    {
        ...
        *end = align_down_by((*end).into(), MIN_ADDR_ALIGN.into()).into();
        ...
        let tracked (mut removed_start_perm, mut perm2) = perm.trusted_split((current_start - oldstart) as nat);
        let tracked (mut total_perm, mut removed_end_perm) = perm2.trusted_split((current_end - current_start) as nat);
        ...
        invariant
            ...
            self@.inv(),
            total_perm@.wf_freemem(((current_start as int), (current_end - current_start) as nat)),
