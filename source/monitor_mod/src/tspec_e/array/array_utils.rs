...
impl<T: Copy, const N: IndexType> Array<T, N> {
    ...
    requires
        ...
        j.is_constant(),
    ensures
        forall |k: int| (0 <= k < self@.len() && k != i as int && k != j as int) ==>  self@[k] === old(self)@[k],
        ...
        self@ =~~= spec_swap(old(self)@, i as int, j as int)
    {
        ...
        self.update(i, x2);
        proof {
            assert forall |k: int| (0 <= k < self@.len() && k != i as int && k != j as int)
            implies  self@[k] === old(self)@[k]
            by{}
        }
    }
    ...
    ensures
        forall |k: int| start as int <= k < end as int ==> self@[k] === old(self)@[end as int + start as int - 1 - k],
        forall |k: int| 0 <= k < start as int || end as int <= k < Self::spec_len() ==> self@[k] === old(self)@[k],
    {
        ...
        invariant
            ...
            0 <= (start as int) < (end as int) <= Self::spec_len(),
            forall |k: int| start as int <= k < (i as int) || (j as int) < k < end as int ==> self@[k] === old(self)@[end as int + start as int - 1 - k],
            forall |k: int| i as int <= k <= j as int || 0 <= k < start as int || end as int <= k < Self::spec_len() ==> self@[k] === old(self)@[k],
            i.is_constant(),
            j.is_constant(),
        {
            assert(i <= j);
