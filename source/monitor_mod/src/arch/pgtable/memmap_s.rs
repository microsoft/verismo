...
impl<VT: AddrType, PT: AddrType> MemMap<VT, PT>
{
    ...
    pub open spec fn is_valid(&self) -> bool {
        forall |page: SpecPage<VT>| #[trigger] page.is_valid() && self.translate(page).is_Some()
    ...
    pub open spec fn translate_addr(&self, addr: SpecAddr<VT>) -> Option<SpecAddr<PT>> {
        ...
        match ppage {
            ...
            Option::Some(p) => {
                Option::Some(p.to_addr() + addr.to_offset())
            }
        }
    }
    ...
    pub open spec fn translate_addr_seq(&self, addrs: SpecMem<VT>) -> SpecMem<PT> {
        if self.translate(addrs.to_page()).is_None() {
            ...
            addrs.convert(
                self.translate(addrs.to_page()).get_Some_0()
             )
        }
    }
    ...
    {
        if exists |gvn| (#[trigger] self.translate(gvn)).is_Some() && (self.translate(gvn).get_Some_0() =~= page) {
            let ret = choose |gvn| (#[trigger] self.translate(gvn)).is_Some() && (self.translate(gvn).get_Some_0() =~= page);
            Option::Some(ret)
        } else {
            Option::None
        }
    ...
    pub open spec fn is_one_to_one_map(&self) -> bool
    {
        ...
        &&& (forall |ppage: SpecPage<PT>|
            ...
            self.translate(self.reverse(ppage).get_Some_0()).get_Some_0() =~= ppage))
    }

    #[verifier(opaque)]
    pub open spec fn is_identity_map(&self) -> bool
    {
        &&& (forall |vpage: SpecPage<VT>| ((#[trigger]self.translate(vpage)).is_Some()) ==>
        self.translate(vpage).get_Some_0().as_int() === vpage.as_int())
    }
}
...
impl MemMap<GuestVir, GuestPhy> {
    ...
    {
        &&& forall |gvn: GVN|
            gvn.is_valid() && self.need_c_bit(memid, gvn)
        ==>
            #[trigger] self.is_encrypted_or_none(gvn)
    }

    pub open spec fn need_c_bit(&self, memid: MemID, gvn: GVN) -> bool {
        ||| memtype(memid, self.translate(gvn).get_Some_0()).need_c_bit()
