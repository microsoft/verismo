...
impl Model1Eq for GuestPTRam {
    open spec fn model1_eq(&self, other: &Self, memid: MemID) -> bool {
        self.spec_ram().model1_eq(&other.spec_ram(), memid)
            && equal(self.l0_entry(memid), other.l0_entry(memid))
    }
}
...
impl GuestPTRam {
    ...
    pub open spec fn pgtb_walk_addrs_recursive_ok(
        ...
        if next_opt.is_None() {
            Option::Some(GPMem::from_range(
                l0_entry.addr_for_idx(idx),
                AsNat!(PT_ENTRY_SIZE!()),
            ))
        } else {
            ...
            if next_lvl.as_int() < lvl.as_int() {
                ...
                if next_pte_addrs.is_Some() {
                    ...
                    let next_pte = vram.get_enc_data_ok(AddrMemID {
                        range: next_pte_gpmem,
                        memid,
                    });
                    if next_pte.is_Some() {
    ...
    {
        forall |lvl: PTLevel|
            (!lvl.is_L0() && (#[trigger]self.map_entry_ok(memid, gvn, lvl)).is_Some())
        ==>
            memtype(memid, self.map_entry_ok(memid, gvn, lvl).get_Some_0().spec_ppn()).is_pt(lvl.child_lvl().get_Some_0())
    }
    ...
    {
        &&& (forall |gvn: GVN| (gvn.is_valid() && (#[trigger] self.map_entry_ok(memid, gvn, PTLevel::L0)).is_Some())
                ==> self.map_entry_ok(memid, gvn, PTLevel::L0).get_Some_0().spec_ppn().value() === gvn.value())
    }

    pub open spec fn need_c_bit(&self, memid: MemID, gvn: GVN) -> bool {
        let rmp= self.spec_ram().spec_rmp();
        let entry = self.map_entry_ok(memid, gvn, PTLevel::L0).get_Some_0();
        memtype(memid, entry.spec_ppn()).need_c_bit()
    ...
    pub open spec fn inv_encrypted_priv_mem_ok(&self, memid: MemID) -> bool
    {
        &&& (forall |gvn: GVN|
                (gvn.is_valid() &&
                self.need_c_bit(memid, gvn) &&
                self.map_entry_ok(memid, gvn, PTLevel::L0).is_Some())
            ==>
            #[trigger] self.map_entry_ok(memid, gvn, PTLevel::L0).get_Some_0().is_encrypted())
    }

    pub open spec fn map_entry_gpa_ok(&self, memid: MemID, gvn: GVN, lvl: PTLevel) -> Option<GPMem>
    {
        self.pgtb_walk_addrs_recursive_ok(memid, gvn, lvl)
    }

    pub open spec fn map_entry_ok(&self, memid: MemID, gvn: GVN, lvl: PTLevel) -> Option<SpecGuestPTEntry>
    {
        let ret = self.map_entry_exe_ok(memid, gvn, lvl);
        match ret {
            Option::Some(exe_ret) => {
                Option::Some(exe_ret.view())
            }
            _ => Option::None
        }
    }

    pub open spec fn map_entry_exe_ok(&self, memid: MemID, gvn: GVN, lvl: PTLevel) -> Option<GuestPTEntry>
    {
        let pte_gpa = self.map_entry_gpa_ok(memid, gvn, lvl);
        if pte_gpa.is_Some() {
            let pte_gpa = pte_gpa.get_Some_0();
            let entry = self.spec_ram().get_enc_data_ok::<GuestPTEntry>(AddrMemID{range: pte_gpa, memid});
            if entry.is_Some() {
