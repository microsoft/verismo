...
impl GuestPTRam {
    ...
    pub open spec fn pgtb_walk_addrs_recursive(&self, sysmap: SysMap, memid: MemID, gvn: GVN, lvl: PTLevel) -> Option<GPMem>
    decreases lvl.as_int()
    {
        ...
        if next_opt.is_None() {
            Option::Some(
                GPMem::from_range(l0_entry.addr_for_idx(idx),
    ...
    pub open spec fn hw_read_pte(&self, memid: MemID, sysmap: SysMap, gpa: GPMem) -> Option<SpecGuestPTEntry> {
    ...
    pub open spec fn map_entry_gpa(&self, sysmap: SysMap, memid: MemID, gvn: GVN, lvl: PTLevel) -> Option<GPMem>
    {
        self.pgtb_walk_addrs_recursive(sysmap, memid, gvn, lvl)
    }

    pub open spec fn map_entry(&self, sysmap: SysMap, memid: MemID, gvn: GVN, lvl: PTLevel) -> Option<SpecGuestPTEntry>
    {
