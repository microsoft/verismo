...
impl Archx64 {
        ...
        self.op(arch_op).inv_regdb_any_cpu(memid)
    {
        ...
        assert forall |cpu| new.inv_regdb(cpu, memid)
        by {
    ...
    ensures
        self.spec_memdb().op(arch_op.memop()).is_Ok() ||
        self.spec_memdb().op(arch_op.memop()).get_Error_1().is_RmpOp(),
        self.spec_memdb().to_mem_map(arch_op.to_memid()).translate(arch_op.memop().to_mem().to_page()).is_Some(),
    {
        if !self.spec_memdb().to_mem_map(arch_op.to_memid()).translate(arch_op.memop().to_mem().to_page()).is_Some() {
    ...
    requires
        ...
        self.spec_memdb().to_mem_map(arch_op.to_memid()).translate(arch_op.memop().to_page()).is_None(),
    ensures
        !self.op(arch_op).is_run(arch_op.cpu_memid()) ||
        self.op(arch_op).spec_memdb() === self.spec_memdb() ||
        arch_op.memop().is_RmpOp() && self.op(arch_op).spec_regdb()[arch_op.cpu_memid()][RegName::Rax] != 0
    {
        self.spec_memdb().lemma_op_error(arch_op.memop());
    }
    ...
    requires
        ...
        self.spec_memdb().to_mem_map_ok(arch_op.to_memid()).translate(arch_op.memop().to_page()).is_None(),
    ensures
        !self.op(arch_op).is_run(arch_op.cpu_memid()) ||
        self.op(arch_op).spec_memdb() === self.spec_memdb() ||
        arch_op.memop().is_RmpOp() && self.op(arch_op).spec_regdb()[arch_op.cpu_memid()][RegName::Rax] != 0
