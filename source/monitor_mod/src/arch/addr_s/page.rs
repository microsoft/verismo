...
impl<T> SpecAddr<T>
{
    ...
    pub open spec fn new2(val: int, dummy: Ghost<T>) -> Self {
        SpecAddr{
            value: spec_cast_integer::<_, nat>(val),
            dummy: dummy,
        }
    }
...
impl<T> SpecPage<T>
{
    ...
    pub open spec fn valid_as_size(&self, psize: PageSize) -> bool
    {
        match psize {
            ...
            PageSize::Size2m => {
                self.value() % (PAGE_2M_SIZE!() / PAGE_SIZE!()) == 0
            }
        }
...
impl<T> SpecMem<T>
{
        ...
        self.first().to_page() == self.to_page(),
    {
        ...
        assert((self.to_page().to_addr().value() + self.offset()) / PAGE_SIZE!() === self.to_page().value());
        if self.len() > 0 {
            ...
            assert((self.to_page().as_int() * PAGE_SIZE!() + self.offset() + self.len() -1)/PAGE_SIZE!() == self.to_page().as_int());
        } else {
    ...
    {
        SpecMem {
            first: pn.to_addr() + self.first().to_offset(),
            size: self.size,
        }
    }
        ...
        mem1 === mem2 || mem1.disjoint(mem2)
    {
        if !(mem1 =~= mem2) {
            assert(
                !(mem1.first() =~= mem2.first())
            );
            assert forall |i|
                0 <= i < mem1.len()
            implies
                !#[trigger]mem2.contains(mem1[i])
            by {
                assert forall |j|
                    0 <= j < mem2.len()
                implies
