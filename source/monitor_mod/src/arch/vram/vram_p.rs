...
verus! {
    impl VRamDB {
        pub proof fn proof_rmp_check_access_rmp_has_gpn_memid(&self, op: MemOp<GuestPhy>, sysmap: SysMap)
        requires
            ...
            rmp_has_gpn_memid(&self.rmp, op.to_mem().to_page(), op.to_memid()),
        {
            ...
            if op.is_Write() {
                rmp_proof_check_access_rmp_has_gpn_memid(&self.rmp, op.to_memid(), op.to_enc(), op.to_mem(), Perm::Write, spn);
            }
            if op.is_Read() {
                rmp_proof_check_access_rmp_has_gpn_memid(&self.rmp, op.to_memid(), op.to_enc(), op.to_mem(), Perm::Read, spn);
            }
        }
        ...
        ensures
            ...
            rmp_reverse(&self.spec_rmp(), op.to_memid(), op.to_mem().to_page()) === sysmap.translate(op.to_mem().to_page()).get_Some_0(),
        {
            reveal(VRamDB::op);
            ...
            assert(rmp_reverse(&self.spec_rmp(),op.to_memid(), self.spec_rmp()[spn].view().spec_gpn()) === spn) by {
        ...
        requires
            ...
            gpmem.is_valid(),
        ensures
            self.get_enc_bytes_ok(AddrMemID{memid, range:gpmem}).is_Some() === self.get_enc_byte_ok(memid, gpmem[i]).is_Some(),
            self.get_enc_bytes_ok(AddrMemID{memid, range:gpmem}).is_Some()  ==> self.get_enc_bytes_ok(AddrMemID{memid, range:gpmem}).get_Some_0()[i] === self.get_enc_byte_ok(memid, gpmem[i]).get_Some_0()
        {
            gpmem.proof_same_page();
        }

        /// TODO: function body check has been running for 2 seconds
        pub proof fn lemma_write_enc_bytes_effect_same_read(&self, other: &Self, sysmap: SysMap, memop: MemOp<GuestPhy>, memid: MemID, gpmem: GPMem)
        requires
            self.inv(),
            memop.is_Write(),
            memop.is_valid(),
        ...
        ensures
            (other.get_enc_bytes_ok(AddrMemID{memid, range:gpmem}).is_Some() &&
                memop.get_Write_1()) ==>
                (other.get_enc_bytes_ok(AddrMemID{memid, range:gpmem}).get_Some_0() === memop.get_Write_2()),
            (other.get_enc_bytes_ok(AddrMemID{memid, range:gpmem}).is_Some() &&
                !memop.get_Write_1()) ==> {
                other.get_enc_bytes_ok(AddrMemID{memid, range:gpmem}) === self.get_enc_bytes_ok(AddrMemID{memid, range:gpmem})
            },
            self.rmp.dom() === self.op(sysmap, memop).to_result().rmp.dom(),
        {
            ...
            if gpmem === memop.to_mem() && read2.is_Some() && w_enc {
                assert forall |i| 0 <= i < gpmem.len()
                implies
                    read2.get_Some_0()[i] === #[trigger] memop.get_Write_2()[i]
                by {
            ...
            if gpmem === memop.to_mem() && read2.is_Some() && !w_enc {
                assert forall |i| 0 <= i < gpmem.len()
                implies
                    read2.get_Some_0()[i] === #[trigger] read1.get_Some_0()[i]
                by {
                    if 0 <= i < gpmem.len() {
                        self.lemma_write_effect_in_range(other, sysmap, memop, memid, gpmem[i]);
                        other.proof_read_enc_byte_to_bytes(memid, gpmem, i);
        ...
        pub proof fn lemma_write_enc_bytes_effect_disjoint_read(&self, other: &Self, sysmap: SysMap, memop: MemOp<GuestPhy>, memid: MemID, gpmem: GPMem)
        requires
            self.inv(),
            memid.is_vmpl0(),
            memop.is_valid(),
            ...
            self.rmp.dom() === self.op(sysmap, memop).to_result().rmp.dom(),
        {
            ...
            if read2.is_Some()
            {
                assert forall |i| 0 <= i < gpmem.len()
                implies
                    read2.get_Some_0()[i] === read1.get_Some_0()[i]
                by {
                    if 0 <= i < gpmem.len() {
                        other.proof_read_enc_byte_to_bytes(memid, gpmem, i);
        ...
        pub proof fn lemma_write_bytes_effect_by_other_vm_or_shared(&self, other: &Self, sysmap: SysMap, memop: MemOp<GuestPhy>, memid: MemID, gpmem: GPMem)
        requires
            self.inv(),
            memid.is_vmpl0(),
            memop.is_valid(),
            ...
            self.op(sysmap, memop).is_Ok(),
            other === &self.op(sysmap, memop).to_result(),
            (memop.to_memid().to_asid() !== memid.to_asid() && memop.get_Write_1()) || !memop.get_Write_1()
        ensures
            other.get_enc_bytes_ok(AddrMemID{memid, range:gpmem}) === self.get_enc_bytes_ok(AddrMemID{memid, range:gpmem}),
            self.rmp.dom() === self.op(sysmap, memop).to_result().rmp.dom(),
        {
        ...
        pub proof fn lemma_write_sm_int_Rinv(&self, sysmap: SysMap, memop: MemOp<GuestPhy>, memid: MemID, gpa: GPA)
        requires
            self.inv(),
            self.inv_sw(memid),
        ...
        pub proof fn lemma_write_byte_othervm_or_shared(&self, other: &Self, sysmap: SysMap, memop: MemOp<GuestPhy>, memid: MemID, gpa: GPA)
        requires
            self.inv(),
            gpa.is_valid(),
            memop.is_valid(),
        ...
        pub proof fn lemma_write_byte_other_vm(&self, other: &Self, sysmap: SysMap, memop: MemOp<GuestPhy>, memid: MemID, gpa: GPA)
        requires
            memid.is_vmpl0(),
            self.inv(),
            self.inv_sw(memid),
        ...
        {
            reveal(VRamDB::op);
            assert(other.get_enc_byte_ok(memid, gpa).is_Some() === self.get_enc_byte_ok(memid, gpa).is_Some());
            let w_asid = if memop.get_Write_1() { memop.to_memid().to_asid() } else { ASID_FOR_HV!() };
            if self.get_enc_byte_ok(memid, gpa).is_Some() {
                ...
                assert(!memrange_contains_block(wspmem, idx(rspa))) by {
                    ...
                    assert((rbase_addr <= base_addr - PAGE_SIZE!()) || (rbase_addr >= base_addr + PAGE_SIZE!()));
                }
                self.spec_sram().lemma_write_unchange_byte_any_enc(w_asid, wspmem, memop.get_Write_2(), memid.to_asid(), rspa);
            }
        }

        pub proof fn lemma_write_effect_change_other_vm_enc(&self, other: &Self, sysmap: SysMap, memop: MemOp<GuestPhy>, memid: MemID, gpa: GPA, rsysmap: SysMap, renc: bool)
        requires
            self.inv(),
            self.inv_sw(memid),
            gpa.is_valid(),
        ...
        ensures
            ...
            other.read_bytes(AddrMemID{range: SpecMem::from_range(gpa, 1), memid}, renc, rsysmap).is_Ok() ==>
            (other.get_byte(memid, gpa, renc, rsysmap) === self.get_byte(memid, gpa, renc, rsysmap)),
            other.get_enc_byte_ok(memid, gpa).is_Some() ==> other.get_enc_byte_ok(memid, gpa) === self.get_enc_byte_ok(memid, gpa)
        {
            reveal(VRamDB::op);
            ...
            if other.read_bytes(AddrMemID{range: SpecMem::from_range(gpa, 1), memid}, renc, rsysmap).is_Ok() {
                assume(other.get_byte(memid, gpa, renc, rsysmap) === self.get_byte(memid, gpa, renc, rsysmap));
            }
            assume(other.get_enc_byte_ok(memid, gpa) === self.get_enc_byte_ok(memid, gpa));
        }

        pub proof fn lemma_write_shared_effect(&self, other: &Self, sysmap: SysMap, memop: MemOp<GuestPhy>, memid: MemID, gpa: GPA)
        requires
            self.inv(),
            self.inv_sw(memid),
            gpa.is_valid(),
            ...
            if let MemOp::Write(gpa_id, w_enc, bytes) = memop {
                ...
                if gpmem_id.range.contains(gpa) {
                    ...
                    if other.get_enc_byte_ok(memid, gpa).is_Some() {
                        ...
                        assert(other.get_byte(memid, gpa, false, sysmap).get_Some_0() === memop.get_Write_2()[gpa.value() - memop.to_mem().first().value()]);
                    }
                } else {
        ...
        pub proof fn lemma_write_effect_in_range(&self, other: &Self, sysmap: SysMap, memop: MemOp<GuestPhy>, memid: MemID, gpa: GPA)
        requires
            self.inv(),
            self.inv_sw(memid),
            gpa.is_valid(),
        ...
        ensures
            ...
            (other.get_enc_byte_ok(memid, gpa).get_Some_0() === memop.get_Write_2()[gpa.value() - memop.to_mem().first().value()]),
            !(other.get_enc_byte_ok(memid, gpa).is_Some() && memop.get_Write_1()) ==>
             (other.get_byte(memop.to_memid(), gpa, memop.get_Write_1(), sysmap).get_Some_0() === memop.get_Write_2()[gpa.value() - memop.to_mem().first().value()]),
            (!other.get_enc_byte_ok(memid, gpa).is_Some() && !memop.get_Write_1()) ==> {
                other.get_byte(memid, gpa, false, sysmap).get_Some_0() === memop.get_Write_2()[gpa.value() - memop.to_mem().first().value()]
            },
            !memop.get_Write_1() ==> {
                other.get_enc_byte_ok(memid, gpa) === self.get_enc_byte_ok(memid, gpa)
            },
        {
            ...
            if other.get_enc_byte_ok(memid, gpa).is_Some() && w_enc {
                ...
                let w_asid = if w_enc { w_memid.to_asid() } else { ASID_FOR_HV!() };
                ...
                if wspmem.to_page() !== rspn {
                    assert(0 <= rspa.as_int() - rspn.as_int() * (PAGE_SIZE as int) < (PAGE_SIZE as int));
                    self.spec_sram().lemma_write_unchange_byte_any_enc(w_asid, wspmem, data, memid.to_asid(), rspa);
                }
                ...
                assert(other.spec_sram().read_one_byte(w_asid, rspa_by_sysmap) === data[rspa_by_sysmap.value() - wspmem.first().value()]);
                assert(read_byte_sysmap.get_Some_0() === data[rspa_by_sysmap.value() - wspmem.first().value()]);
            }
        }

        pub proof fn lemma_write_effect_out_range(&self, other: &Self, sysmap: SysMap, memop: MemOp<GuestPhy>, memid: MemID, gpa: GPA)
        requires
            self.inv(),
            self.inv_sw(memid),
            memid.is_vmpl0(),
        ...
        pub proof fn lemma_write_effect_out_range_same_vm(&self, other: &Self, sysmap: SysMap, memop: MemOp<GuestPhy>, memid: MemID, gpa: GPA)
        requires
            self.inv(),
            self.inv_sw(memid),
            memop.is_valid(),
            ...
            if self.get_enc_byte_ok(memid, gpa).is_Some() {
                ...
                if w_enc {
                    ...
                    if wspmem.to_page() === rspa.to_page() {
                        ...
                        self.spec_sram().lemma_write_unchange_byte_any_enc(w_memid.to_asid(), wspmem, data, memid.to_asid(), rspa);
                    }
            ...
            self.op(sysmap, memop).to_result().inv(),
        {
            ...
            match memop {
                ...
                MemOp::Write(gpa_id, enc, data) => {
                    let use_asid = if enc { gpa_id.memid.to_asid() } else { ASID_FOR_HV!() };
                    if self.op(sysmap, memop).is_Ok() {
            ...
            match memop {
                MemOp::RmpOp(rmpop) => {
                    ...
                    if spn.is_Some() {
                        ...
                        rmp_proof_inv_memid_int(&self.spec_rmp(),rmpop.set_spn(spn.get_Some_0()), memid);
                    }
                }
                _ => {}
            }
        ...
        pub proof fn lemma_write_read_consistant(&self, sysmap: SysMap, memop: MemOp<GuestPhy>, rgpa_id: GPAMemID, enc: bool)
        requires
            memop.is_valid(),
            memop.is_Write(),
        ...
        ensures
            memop.get_Write_2() ===
                self.op(sysmap, memop).to_result()
                    .get_bytes(rgpa_id, enc, sysmap),
        {
            reveal(VRamDB::op);
            reveal(VRamDB::op_write);
            let data = memop.get_Write_2();
        ...
        ensures
            ...
            self.get_enc_bytes_ok(gpa_id).get_Some_0() === self.read_bytes(gpa_id, enc, sysmap).to_result(),
            sysmap.translate(gpa_id.range.to_page()).is_Some(),
            sysmap.translate(gpa_id.range.to_page()).get_Some_0() === rmp_reverse(&self.spec_rmp(),gpa_id.memid, gpa_id.range.to_page()),
        {
            reveal(VRamDB::op);
        ...
        pub proof fn lemma_read_diff_sysmap(&self, sysmap1: SysMap, sysmap2: SysMap, gpa_id: GPAMemID, enc: bool)
        requires
            enc,
            gpa_id.range.is_valid(),
        ...
        ensures
            ...
            self.read_bytes(gpa_id, enc, sysmap1).is_Error() ||
            self.read_bytes(gpa_id, enc, sysmap2).is_Error(),
            ...
            if other.get_enc_bytes_ok(gpa_id).is_None() {
                ...
                assert(!rmp_has_gpn_memid(&self.spec_rmp(),gpa.to_page(), memid)) by {
                    ...
                    assert forall |spn: SPN|
                        ...
                        self.spec_rmp().dom().contains(spn)
                    implies
                        #[trigger] self.spec_rmp()[spn].view().spec_gpn() !== gpa.to_page()
                    by {
        ...
        requires
            ...
            memid.is_vmpl0(),
        ensures
            memop.to_memid().is_sm(memid) || memop.to_memid().to_asid() !== memid.to_asid() || !memop.get_Write_1(),
        {
            reveal(RmpEntry::check_access);
            reveal(VRamDB::op);
            let rmp = self.spec_rmp();
        ...
        pub proof fn lemma_write_enc_must_has_gpn_in_rmp(&self, memid: MemID, memop: MemOp<GuestPhy>, sysmap: SysMap)
        requires
        ...
        ensures
            self.get_enc_bytes_ok(gpa_id).is_Some() ==> self.get_enc_bytes_ok(gpa_id) === other.get_enc_bytes_ok(gpa_id),
            other.get_enc_bytes_ok(gpa_id).is_None() ==> self.get_enc_bytes_ok(gpa_id).is_None(),
        {
            reveal(RmpEntry::check_access);
            ...
            if self.get_enc_bytes_ok(gpa_id).is_Some() {
                ...
                assert(read_bytes1 === read_bytes2) by {
                    ...
                    assert(bytes1 === bytes2) by {
                        assert forall |i: int| 0 <= i < spmem1.len()
                        implies
                            bytes1[i] === bytes2[i]
                        by {
                            let spa = spmem1[i];
                            ...
                            assert(vmpl >= VMPL::VMPL1 ||  !other.rmp[spa.to_page()].view().check_vmpl(VMPL::VMPL1, Perm::Write)) by {
                                rmp_inv_memid_int(&other.rmp, gpa_id.memid);
                            }
                            assert(vmpl >= VMPL::VMPL2 ||  !other.rmp[spa.to_page()].view().check_vmpl(VMPL::VMPL2, Perm::Write));
                            assert(vmpl >= VMPL::VMPL3 ||  !other.rmp[spa.to_page()].view().check_vmpl(VMPL::VMPL3, Perm::Write));
                            assert(self.spec_sram().spec_data()[spa.value()] === other.spec_sram().spec_data()[spa.value()]) by {
        ...
        pub proof fn lemma_possible_ops_mod_enc_bytes(self, anysysmap: SysMap, op: MemOp<GuestPhy>, gpa: GPA, memid: MemID)
        requires
            gpa.is_valid(),
            self.get_enc_byte_ok(memid, gpa) !== self.op(anysysmap, op).to_result().get_enc_byte_ok(memid, gpa),
