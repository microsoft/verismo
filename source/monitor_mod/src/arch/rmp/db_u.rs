...
verus! {
    ...
    pub open spec fn rmp_model_eq(rmp: &RmpMap, other: &RmpMap) -> bool
    {
        ...
        &&& (forall |memid|(rmp_inv_sw(other, memid) ==> rmp_inv_sw(rmp, memid))) // tODO: prove this
        &&& rmp.dom() === other.dom()
        &&& (forall |spn: SPN| rmp[spn].view().spec_validated()
                ==> #[trigger]rmp[spn] === #[trigger] other[spn])
    ...
    pub open spec fn rmp_hv_update(rmp: &RmpMap, newrmp: RmpMap, memid: MemID) -> RmpMap {
        RmpMap::new(
            ...
            |spn: SPN| {
                rmp.spec_index(spn)
                    .rmpupdate(newrmp.spec_index(spn))
                    .to_result()
            },
        )
    }
