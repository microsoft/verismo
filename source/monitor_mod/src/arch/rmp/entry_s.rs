...
verus! {
    impl HiddenRmpEntryForPSP {
        pub open spec fn inv_hvupdate_rel(self, preventry: Self) -> bool {
            &&& self.is_valid()
            &&& !preventry.validated ==> !self.validated
            ...
            &&& (self !== preventry)  ==> {
                ...
                &&& (self.perms[VMPL::VMPL1] === PagePerm::empty() || self.perms[VMPL::VMPL1] === preventry.perms[VMPL::VMPL1])
                &&& (self.perms[VMPL::VMPL2] === PagePerm::empty() || self.perms[VMPL::VMPL2] === preventry.perms[VMPL::VMPL2])
                &&& (self.perms[VMPL::VMPL3] === PagePerm::empty() || self.perms[VMPL::VMPL3] === preventry.perms[VMPL::VMPL3])
            }
        }
        ...
        pub open spec fn is_valid(&self) -> bool {
            ...
            &&& self.perms[VMPL::VMPL0] === Set::full()
        ...
        pub open spec fn check_guest_reverse_mut_size_no_gpn(&self, asid: ASID, size: PageSize) -> bool {
            &&& self.spec_immutable() == false
            &&& self.spec_assigned() == true
            &&& self.spec_asid() == asid
            &&& self.spec_size() == size
        ...
        pub open spec fn check_guest_reverse_mut_size(&self, asid: ASID, gpn: GPN, size: PageSize) -> bool {
            &&& self.check_gpn(gpn)
