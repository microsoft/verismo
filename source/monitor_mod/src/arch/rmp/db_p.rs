...
verus! {
    pub proof fn rmp_proof_check_access_rmp_has_gpn_memid(rmp: &RmpMap, memid: MemID, enc: bool, gpmem: GPMem, perm: Perm, spn: SPN)
    requires
        ...
        rmp_inv_memid_int(&rmp_op(rmp, op).to_result(), memid),
    {
        ...
        assert forall |spn: SPN, vmpl: VMPL| {
            ...
            !#[trigger]new[spn].view().check_vmpl(vmpl, Perm::Write)
        by {
            assert(rmp[spn].inv());
    ...
    requires
        op.is_Pvalidate(),
        (op.to_page_memid().memid.to_vmpl().is_VMPL0() && (memid.to_asid() === op.to_page_memid().memid.to_asid()))  ==> {
            !op.get_Pvalidate_1().val ||
            !rmp_has_gpn_memid(rmp, op.get_Pvalidate_1().gpn, memid)
        },
        rmp_inv_sw(rmp, memid),
    ensures
        rmp_inv_sw(&rmp_op(rmp, op).to_result(), memid),
    {
        ...
        implies
            (rmp_reverse(&new, memid, rmp[spn].view().spec_gpn()) === spn)
        by {
            ...
            if !val {
                ...
                if rmp[spn] !== new[spn] {
                    ...
                    assert forall |spn_test: SPN| {
                        ...
                        ((#[trigger]new[spn_test]).view().spec_gpn() !== gpn) ||
                ...
                assert forall |spn_test: SPN| {
                    &&& #[trigger]rmp.dom().contains(spn_test)
                    &&& new[spn_test].view().spec_asid() === memid.to_asid()
                    &&& ((#[trigger]new[spn_test]).view().spec_gpn() === rmp[spn].view().spec_gpn())
                    &&& new[spn_test].view().spec_validated()
                } implies
                    spn_test === spn
                by {
    ...
    ensures
        forall |i|
            (rmp.dom().contains(i) &&
            (#[trigger]rmp_hv_update(rmp, newrmp, hv_id)[i] !== rmp[i])) ==> (!rmp_hv_update(rmp, newrmp, hv_id)[i]@.spec_validated() && rmp_hv_update(rmp, newrmp, hv_id)[i]@.spec_perms() === rmp_perm_init()),
    {
        reveal(rmp_inv);
    }


    pub proof fn rmp_lemma_hv_update_restrict_at(rmp: &RmpMap, hv_id: MemID,  newrmp: RmpMap, memid: MemID, enc: bool, gpmem: GPMem, perm: Perm, spn: SPN)
    requires
        rmp_inv(rmp),
        memid.is_Guest(),
        enc,
    ensures
        ...
        (rmp_check_access(&rmp_hv_update(rmp, newrmp, hv_id), memid, enc, gpmem, perm, spn) === rmp_check_access(rmp, memid, enc, gpmem, perm, spn))),
    {
        reveal(RmpEntry::check_access);
