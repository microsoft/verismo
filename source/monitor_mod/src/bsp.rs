...
verus! {
        ...
        cs.inv_stage_ap_wait(),
    {
        ...
        crate::security::run_richos(
            ghcb_hv_h, guest_channel, vmsa, cpu.secret,
            Tracked(nextvmpl_id), Tracked(&mut cs)
        );
        loop{}
    }
}
...
verus! {
    ...
    requires
        ...
        initial_locks.contains_clean_locks(
            memcc.cc.snpcore.cpu(), shared_mem_range()),
        contains_PT(initial_locks),
        initial_locks[spec_PT_lockid()]@.is_unlocked(memcc.cc.snpcore.cpu(), spec_PT_lockid(),  initial_locks[spec_PT_lockid()]@.points_to.range()),
        ...
        false
    {
        ...
        let (mpbox, hvparam_page, Tracked(mut cc),
            Tracked(mut mp_gperms),
        ) = init_mem(
            mp_ptr,
            Tracked(mp_perms),
            Tracked(alloc_perm),
        ...
        proof {
            ...
            assert(console_lock@.is_unlocked(cs.snpcore.cpu(), console_lock@.lockid(), console_lock@.ptr_range()));
            assert(alloc_lock@.is_unlocked(cs.snpcore.cpu(), alloc_lock@.lockid(), alloc_lock@.ptr_range()));
            assert(cs.wf_pt());
        ...
        let secret_page = VBox::<SnpSecretsPageLayout>::from_raw(mparam.secret_page.into(), Tracked(secret.tracked_into()));
        ...
        let mut ghcb_hv_h = ghcb_hv_h.start_all_ap(
            mparam.cpu_count.into(), secret_page.borrow(), gdt, Tracked(ap_ids), Tracked(&mut cs));
