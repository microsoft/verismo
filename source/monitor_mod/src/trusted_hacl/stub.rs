...
ensures
    ...
    dst_perm0[0]@.wf(),
    ret == dst_addr ==> dst_perm0[0]@.bytes() =~~= (src_perm@.bytes() +  old(dst_perm0)[0]@.bytes().skip(len as int)),
    (ret == 0) ==> (*dst_perm0) =~~= (*old(dst_perm0)),
{
    if dst_len < len {
        return 0;
    } else {
        let tracked (mut dst_perm, right_perm) = dst_perm0.tracked_remove(0).trusted_split(len as nat);
