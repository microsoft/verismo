...
verus! {
    pub closed spec fn get_hv_mem_count_ensures(
        arr: Seq<HyperVMemMapEntry>, ret: nat) -> bool {
        &&& ret <= arr.len()
    ...
    pub fn fmt_hvparam<'a>(hv_param: &'a mut HvParamTable, n: usize_t) -> (ret: Option<&'a [HyperVMemMapEntry]>)
    requires
        old(hv_param).is_constant(),
        n <= old(hv_param).mem_table@.len(),
        n.is_constant(),
        forall |i: int| 0 <= i < (n as int) ==> old(hv_param).mem_table@[i].numpages@.val != 0,
        n < old(hv_param).mem_table@.len() ==> old(hv_param).mem_table@[n as int].numpages@.val == 0,
    ensures
        ret.is_Some() ==> fmt_hvparam_ensures(*old(hv_param), *hv_param, n as nat, ret.get_Some_0()),
    {
        ...
        proof {
            ...
            implies
                (#[trigger]seq[i]).spec_real_range().0.is_constant() && seq[i].spec_real_range().1.is_constant()
            by{
            ...
            assert(memtb.is_constant()) by{
                ...
                by{
                    ...
                    if i < validn as int {
                        ...
                        let j = choose |j| entry === prev_memtb[j].spec_set_range(entry.spec_real_range()) &&
                    0 <= j && j < prev_memtb.len();
    ...
    impl<'a> MutFnWithCSTrait<'a, SnpCoreConsole, FmtHvParamCall, FmtHvParamOut<'a>> for HvParamTable {
        open spec fn spec_update_cs_requires(&self, params: FmtHvParamCall, cs: SnpCoreConsole) -> bool {
            &&& cs.wf()
            &&& self.is_constant()
        }

        open spec fn spec_update_cs(&self, prev: &Self, params: FmtHvParamCall, oldcs: SnpCoreConsole, ret: FmtHvParamOut<'a>, cs: SnpCoreConsole) -> bool {
            ...
            &&& ret@.is_constant()
            &&& exists |n: nat|
                #[trigger] get_hv_mem_count_ensures(prev.mem_table@, n) &&
                #[trigger] fmt_hvparam_ensures(*prev, *self, n, ret)
        }

        fn box_update_cs(
            &'a mut self, params: FmtHvParamCall, Tracked(cs): Tracked<&mut SnpCoreConsole>
        ) -> (ret: FmtHvParamOut<'a>)
        {
