...
verus! {
    proof fn lemma_validate_e820_single(
        prev_memperm: RawMemPerms, memperm: RawMemPerms,
        pre_validated: Set<(int, nat)>,
        prev_validated_range: (int, nat), end_addr: int,
        toval_range: (int, nat),
    ...
    requires
        ...
        prev_memperm.contains_init_except(range(prev_validated_range.end(), VM_MEM_SIZE as int), pre_validated),
        prev_memperm.contains_init_except(range(end_addr as int, VM_MEM_SIZE as int), pre_validated),
        ...
        (toval_range.1 > 0) ==> toval_range.end() == spec_min(end_addr, spec_max(entry_r.0, prev_validated_range.0)),
        prev_validated_range.end() <= end_addr,
    ensures
        memperm.contains_default_except(validated_range, pre_validated),
        memperm.contains_init_except(range(validated_range.end(), VM_MEM_SIZE as int), pre_validated),
        ...
        forall |r| range_disjoint_(r, toval_range) ==> memperm.eq_at(prev_memperm, r),
    {
        ...
        assert forall |r: (int, nat)|
            r.1 != 0 && inside_range(r, validated_range) && ranges_disjoint(pre_validated, r)
        implies memperm.contains_range(r) && memperm.contains_default_mem(r)
        by {
            ...
            let leftr = if toval_range.1 != 0 {
                let leftr = memperm.lemma_with_except_union(prev_validated_range, toval_range, snp, pre_validated);
                leftr
            } else {
                prev_validated_range
            };
            let fullr = if leftr.0 <= entry_r.0 && leftr.end() >= entry_r.0 && leftr.end() <= entry_r.end() {
        ...
        assert(memperm.contains_init_except(next_init_range, pre_validated)) by {
            ...
            prev_memperm.lemma_with_except_sub(next_init_range, prev_init_range, SwSnpMemAttr::init(), pre_validated);
            assert forall |r: (int, nat)|
                r.1 != 0 && inside_range(r, next_init_range) && ranges_disjoint(pre_validated, r)
            implies memperm.contains_range(r) && memperm.contains_init_mem(r)
            by {
        ...
        assert forall |r: (int, nat)|
            r.1 != 0 && inside_range(r, end_init_range) && ranges_disjoint(pre_validated, r)
        implies #[trigger]memperm.contains_range(r) && memperm.contains_init_mem(r)
        by {
            assert(prev_memperm.contains_init_mem(r));
        }
    }
    ...
    spec fn validate_e820_loop_inv(
        e820: Seq<E820Entry>, n: usize,
        start_addr: usize, end_addr: usize,
        pre_validated: Set<(int, nat)>) -> bool
    {
        &&& n == e820.len()
    ...
    proof fn lemma_validated_range_disjoint_e820(e820: Seq<E820Entry>, i: int, start_addr: int, end_addr: int, toval_range: (int, nat))
    requires
        0 <= i < e820.len(),
        ...
        ranges_disjoint(e820.to_aligned_ranges(), toval_range),
    {
        ...
        assert forall |r|  #[trigger]pre_validated.contains(r)
        implies range_disjoint_(toval_range, r)
        by {
    ...
    spec fn validate_e820_iter_val_range(e820: Seq<E820Entry>, i: int, start: int, end: int) -> (int, nat) {
        range(validate_e820_iter_val_start(e820, i, start, end), validate_e820_iter_val_end(e820, i, end))
    }

    pub fn validate_e820(e820: &[E820Entry], start_addr: usize_t, end_addr: usize_t, Tracked(memcc): Tracked<SnpMemCoreConsole>) -> (newmemcc: Tracked<SnpMemCoreConsole>)
    requires
        e820@.is_constant(),
        mem_range_formatted(e820@),
        start_addr < end_addr <= VM_MEM_SIZE,
        ...
        end_addr as int % PAGE_SIZE!() == 0,
        memcc.wf(),
        memcc.memperm.contains_init_except(range(start_addr as int, VM_MEM_SIZE as int), e820@.to_aligned_ranges()),
    ensures
        newmemcc@.wf_core(memcc.cc.snpcore.cpu()),
        newmemcc@.cc.snpcore.only_reg_coremode_updated(
            memcc.cc.snpcore,
            set![GHCB_REGID()]),
        forall |r| range_disjoint_(r, range(start_addr as int, end_addr as int))
            ==> newmemcc@.memperm.eq_at(memcc.memperm, r),
        newmemcc@.memperm.contains_default_except(range(start_addr as int, end_addr as int), e820@.to_aligned_ranges()),
        newmemcc@.memperm.contains_init_except(range(end_addr as int, VM_MEM_SIZE as int), e820@.to_aligned_ranges()),
    {
        ...
        proof {
            memperm.lemma_with_except_sub(range(end_addr as int, VM_MEM_SIZE as int), range(start_addr as int, VM_MEM_SIZE as int), SwSnpMemAttr::init(), pre_validated);
            memperm.lemma_empty_contains_except(range(start_addr as int, val_end as int), SwSnpMemAttr::spec_default(), pre_validated);
        }
        ...
        invariant
            ...
            cc.snpcore.only_reg_coremode_updated(
                oldmemcc.cc.snpcore,
                set![GHCB_REGID()]),
            memperm.contains_default_except(range(start_addr as int, val_end as int), pre_validated),
            memperm.contains_init_except(range(val_end as int, VM_MEM_SIZE as int), pre_validated),
            memperm.contains_init_except(range(end_addr as int, VM_MEM_SIZE as int), pre_validated),
            forall |r| range_disjoint_(r, range(start_addr as int, end_addr as int))
            ==> memperm.eq_at(oldmemcc.memperm, r),
            val_end as int % PAGE_SIZE!() == 0,
            val_end >= start_addr,
            val_end == validate_e820_iter_val_start(e820@, index as int, start_addr as int, end_addr as int),
        {
            ...
            if cur_addr < end_addr {
                next_start = if cur_end > start_addr {cur_end} else {start_addr};
                val_end = cur_addr;
            } else {
                next_start = end_addr;
                val_end = end_addr;
            ...
            proof {
                assert(ranges_disjoint(pre_validated, toval_range)) by {
                    lemma_validated_range_disjoint_e820(e820@, index as int, start_addr as int, end_addr as int, toval_range);
                }
            }
            proof {
                if toval_range.1 > 0 {
                    assert(toval_range.0 == prev_end);
                    memperm.lemma_with_except_sub(toval_range, prev_init_range, SwSnpMemAttr::init(), pre_validated);
            ...
            if val_end > val_start {
                ...
                let Tracked(pperm) = pvalmem(val_start as u64, val_end as u64, true, Tracked(pperm), Tracked(&mut cc.snpcore));
                proof {
                    assert(memperm.contains_default_except(prev_validated_range, pre_validated));
            ...
            proof {
                ...
                lemma_validate_e820_single(
                    prev_memperm, memperm,
                    pre_validated,
                    prev_validated_range, end_addr as int,
                    toval_range,
                    validated_range,
                    entry_r
                );
            ...
            val_end = next_start;
            proof {
                assert(val_end as int % PAGE_SIZE!() == 0&&
                val_end >= start_addr&&
                index == 0 ==> val_end == start_addr&&
                index > 0 ==> val_end == spec_max(start_addr as int, e820@[index as int - 1].spec_aligned_range().end()));
            }
        }
        ...
        if val_end < end_addr {
            ...
            let tracked pperm = memperm.tracked_remove(toval_range);
            let Tracked(pperm) = pvalmem(val_end as u64, end_addr as u64, true, Tracked(pperm), Tracked(&mut cc.snpcore));
            proof {
                assert(memperm.contains_default_except(prev_validated_range, pre_validated));
                memperm.tracked_insert(toval_range, pperm);
                assert(memperm.contains_default_mem(toval_range));
                ...
                assert(memperm.contains_default_except(validated_range, pre_validated)) by {
                    memperm.lemma_with_except_union(prev_validated_range, toval_range,
                        SwSnpMemAttr::spec_default(), pre_validated);
                }
            }
        } else {
            proof {
                memperm.lemma_with_except_sub(
                    validated_range, range(start_addr as int, val_end as int),
                    SwSnpMemAttr::spec_default(), pre_validated);
            }
        }
        proof {
