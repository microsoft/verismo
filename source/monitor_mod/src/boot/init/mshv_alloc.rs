...
verus! {
    pub open spec fn init_allocator_requires(
        ...
        e820: &[E820Entry],
        static_start: usize_t, static_end: usize_t,
        memcc: SnpMemCoreConsole) -> bool  {
    ...
    fn init_allocator(
        ...
        hv_mem_tb: &[HyperVMemMapEntry],
        e820: &[E820Entry], static_start: usize_t, static_end: usize_t,
        Tracked(memcc): Tracked<SnpMemCoreConsole>) -> (newcc: Tracked<SnpCoreConsole>)
    requires
            ...
            set![GHCB_REGID()])
    {
        ...
        proof {
            assert forall |i: int| (idx as int) <= i < (len as int)
            implies
                memcc.memperm.contains_default_except(hv_mem_tb@[i].range(), except_ranges)
            by{
        ...
        invariant
            ...
            static_start < static_end,
            len == hv_mem_tb@.len(),
            verismo_static === range(
                static_start as int, static_end as int),
            ...
            forall |i: int| (idx as int) <= i < (len as int) ==>
                memcc.memperm.contains_default_except(
                    (#[trigger]hv_mem_tb@[i]).range(), except_ranges),
            ...
            prev_end as int <= verismo_static.0 || prev_end as int >= verismo_static.end(),
        {
            ...
            if inside {
                proof {
                    ...
                    if static_end@ < end@ {
                        lemma_contains_except_remove(memperm, range(static_end as int, end as int), g_current_range, except_ranges, verismo_static);
                    }
                    if start@ < static_start@ {
                        lemma_contains_except_remove(memperm, range(start as int, static_start as int), g_current_range, except_ranges, verismo_static);
                    }
                }
                ...
                if start < static_start {
                    proof {
                        ...
                        assert(memperm.contains_default_except(used_range, except_ranges)) by {
                            memperm.lemma_with_except_sub(used_range, g_current_range, SwSnpMemAttr::spec_default(), except_ranges);
                        }
                        remain_range = range(static_start as int, end as int);
                        assert(memperm.contains_default_except(remain_range, except_ranges)) by {
                            memperm.lemma_with_except_sub(remain_range, g_current_range, SwSnpMemAttr::spec_default(), except_ranges);
                        }
                        ...
                        assert(inside_range(used_range, g_current_range) && range_disjoint_(verismo_static, used_range));
                        assert(hv_memperm.contains_default_except(used_range, except_ranges)) by {
                            assert forall |r|
                                inside_range(r, used_range) && ranges_disjoint(except_ranges, r) && r.1 != 0
                            implies
                                #[trigger]hv_memperm.contains_range(r) && hv_memperm[r].snp() === SwSnpMemAttr::spec_default()
                            by {
                    ...
                    let Tracked(tmpcc) = init_allocator_e820(allocator, e820, start, static_start, Tracked(memcc));
                    proof {cc = tmpcc;}
                }
                if static_end < end {
                    proof {
                        ...
                        assert(memperm.contains_default_except(used_range, except_ranges)) by {
                            memperm.lemma_with_except_sub(used_range, remain_range, SwSnpMemAttr::spec_default(), except_ranges);
                        }
                        ...
                        assert(hv_memperm.contains_default_except(used_range, e820@.to_valid_ranges())) by {
                            assert forall |r| (inside_range(r, used_range) && r.1 != 0 && ranges_disjoint(e820@.to_valid_ranges(), r))
                            implies
                                (#[trigger]hv_memperm.contains_range(r) && hv_memperm[r].snp() === SwSnpMemAttr::spec_default())
                            by {
                                assert(ranges_disjoint(except_ranges, r)) by {
                            ...
                            assert(forall |r| (inside_range(r, used_range) && r.1 != 0 && ranges_disjoint(e820@.to_valid_ranges(), r)) ==> (#[trigger]hv_memperm.contains_range(r) && hv_memperm[r].snp() === SwSnpMemAttr::spec_default()));
                        }
                    }
                    let Tracked(tmpcc) = init_allocator_e820(allocator, e820, static_end, end, Tracked(memcc));
                    proof {cc = tmpcc;}
                }
            } else {
                proof {
                    ...
                    let tracked mut hv_memperm = memperm.tracked_split(used_range);
                    assert(hv_memperm.contains_default_except(used_range, except_ranges));
                    assert(inside_range(used_range, g_current_range) &&
                        range_disjoint_(verismo_static, used_range));
                    assert(hv_memperm.contains_default_except(used_range, e820@.to_valid_ranges())) by {
                        assert forall |r| inside_range(r, used_range) && r.1 != 0 && ranges_disjoint(e820@.to_valid_ranges(), r)
                        implies
                            hv_memperm.contains_range(r) && hv_memperm[r].snp() === SwSnpMemAttr::spec_default()
                        by {
                            assert(ranges_disjoint(except_ranges, r)) by {
                                assert(range_disjoint_(verismo_static, used_range));
                            }
...
verus! {
    ...
    pub type InitAllocParams<'a, 'b> = (
        InitAllocFn, &'a[HyperVMemMapEntry],
        &'b[E820Entry], usize, usize, Tracked<SnpMemCoreConsole>);
    use crate::vbox::MutFnTrait;
    impl<'a, 'b, 'c> MutFnTrait<'c, InitAllocParams<'a, 'b>, Tracked<SnpCoreConsole>> for VeriSMoAllocator {
        open spec fn spec_update_requires(
            &self, params: InitAllocParams<'a, 'b>
        ) -> bool {
            init_allocator_requires(
                *self,
                params.1, params.2, params.3, params.4, params.5@)
        }

        open spec fn spec_update(&self, prev: &Self, params: InitAllocParams<'a, 'b>, ret: Tracked<SnpCoreConsole>) -> bool {
            &&& ret@.wf_core(params.5@.cc.snpcore.cpu())
            &&& self@.inv()
            &&& ret@.snpcore.only_reg_coremode_updated(
                params.5@.cc.snpcore,
                set![GHCB_REGID()])
        }
