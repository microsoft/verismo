...
verus! {
    ...
    impl InitMem for RawMemPerms {
        ...
        open spec fn mem_perms_e820_valid(&self, e820: Seq<E820Entry>) -> bool {
            ...
            &&& forall |r| e820.to_aligned_ranges().contains(r) ==>
                self.contains_default_except(r, e820.to_valid_ranges())
        }

        open spec fn out_e820_invalidated(&self, e820: Seq<E820Entry>, bound: (int, nat)) -> bool {
            // contains all memory permission not validated
            forall |r| (inside_range(r, bound) && ranges_disjoint(e820.to_aligned_ranges(), r) && r.1 > 0) ==>
    ...
    pub proof fn lemma_contains_except_remove(memperm: RawMemPerms, r: (int, nat), current_range: (int, nat), ranges: Set<(int, nat)>, toremove: (int, nat))
    requires
        ...
        memperm.contains_default_except(r, ranges.remove(toremove)),
    {
        ...
        assert forall |r2| (inside_range(r2, r) && ranges_disjoint(newrange, r2) && r2.1 > 0)
        implies
            memperm.contains_default_mem(r2)
        by{
