...
fn debug_handler(code: u64_s, stack_frame: &InterruptStackFrame, Tracked(cs): Tracked<&mut SnpCoreSharedMem>)
requires
...
ensures
    ...
    cs.only_lock_reg_coremode_updated(
        *old(cs), set![GHCB_REGID()], set![spec_CONSOLE_lockid()]),
{
    proof {
...
#[no_mangle]
pub extern "C" fn dummy_handler_fn(stack_frame: InterruptStackFrame, error: u64_s,
...
ensures
    ...
    cs.only_lock_reg_updated(
        (*old(cs)),
        set![RegName::IdtrBaseLimit], set![spec_ALLOCATOR_lockid()])
{
    ...
    let dtp = Idtr{
        base: idt_addr.as_u64().into(),
        limit: 0xffffu64.into(),
    };
