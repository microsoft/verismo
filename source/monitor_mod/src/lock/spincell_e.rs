...
impl<T: IsConstant + WellFormed + SpecSize + VTypeCast<SecSeqByte>> VSpinLock<T> {
        ...
        pub open spec fn unlock_requires(&self, cpu: nat, lockperm: LockPermToRaw, memperm: SnpPointsToData<T>) -> bool {
        ...
        pub open spec fn unlock_default_mem_requires(&self, cpu: nat, lockperm: LockPermToRaw, memperm: SnpPointsToData<T>) -> bool {
            &&& self.unlock_requires(cpu, lockperm, memperm)
            &&& lockperm.points_to.snp() === SwSnpMemAttr::spec_default()
        }

        pub open spec fn ensures_lock(&self, oldp: LockPermToRaw, newp: LockPermToRaw, ret_perm: SnpPointsToData<T>) -> bool {
            &&& self.spec_lock().ensures_lock_value(oldp, newp, ret_perm)
        }

        pub open spec fn ensures_unlock(&self, oldp: LockPermToRaw, newp: LockPermToRaw, in_perm: SnpPointsToData<T>) -> bool {
        ...
        {
            proof {lockperm@.invfn.lemma_inv::<T>();}
        ...
        ensures
            *lockperms =~~= old(lockperms).insert(self.spec_lock().id(), (lockperms)[self.spec_lock().id()]),
        ...
        pub const fn new(value: T) -> Self {
            VSpinLock {
                lock: SpinLock::new(),
                data: VCell::new(value),
            }
        }

        pub fn release_(&self,
            Tracked(lockperms): Tracked<&mut Map<int, LockPermRaw>>,
            Tracked(perm): Tracked<SnpPointsTo<T>>,
            Tracked(core): Tracked<&CoreIdPerm>,
        )
        ...
        ensures
            self.ensures_unlock(old(lockperms)[self.spec_lock().id()]@, lockperms[self.spec_lock().id()]@, perm@),
            *lockperms =~~= old(lockperms).insert(self.spec_lock().id(), lockperms[self.spec_lock().id()]),
        {
            let tracked mut lockperm = lockperms.tracked_remove(self.spec_lock().id());
