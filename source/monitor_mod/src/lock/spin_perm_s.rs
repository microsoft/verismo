...
impl LockPermRaw {
    ...
    requires
        ...
        points_to@.wf_not_null_at(points_to@.id()) || (points_to@.wf() && spec_size::<T>() == 0),
        inv(points_to@.get_value()),
        old(self)@.ptr_range() == points_to@.range_id(),
        !old(self)@.locked,
    ensures
        self@.invfn.value_invfn() === inv,
        self@ === old(self)@.spec_set_invfn(self@.invfn).spec_set_points_to(points_to@.vspec_cast_to()),
        self@.wf(),
    {
...
pub trait MapRawLockTrait {
    ...
    forall |prev: &Self, cur: &Self, next: &Self, locks1: Set<int>, locks2: Set<int>|
        (#[trigger]cur.updated_lock(prev, locks1) &&
        #[trigger]next.updated_lock(cur, locks2)) ==>
...
impl MapRawLockTrait for LockMap {
    ...
    open spec fn inv_locked(&self, cpu: nat, locks: Set<int>) -> bool {
        &&& forall |id: int| self.contains_key(id) && !locks.contains(id) ==> (#[trigger]self[id])@.is_unlocked(cpu, id, self[id]@.points_to.range())
        &&& forall |id: int| self.contains_key(id) && locks.contains(id) ==> (#[trigger]self[id])@.is_locked(cpu, id, self[id]@.points_to.range())
    }
    ...
    open spec fn updated_lock(&self, prev: &Self, locks: Set<int>) -> bool {
        &&& prev.dom() =~~= self.dom()
        &&& (forall |id: int| (locks.contains(id) && #[trigger]self.contains_key(id))
    ...
    open spec fn contains_clean_locks(&self, cpu: nat, locks: Map<int, (int, nat)>) -> bool {
        &&& forall |id: int| locks.contains_key(id) ==>
            (#[trigger]self[id])@.is_clean_lock_for(locks[id], cpu)
            && self.contains_key(id)
        &&& forall |id: int| locks.contains_key(id) ==>
                #[trigger]self.contains_key(id)
    }
    ...
    proof fn lemma_lock_update_auto()
    {
        ...
        by {
            ...
            assert forall |id: int| locks.contains(id) && prev.contains_key(id)
            implies next.contains_key(id) &&
            ...
            assert forall |id: int| !locks.contains(id) && prev.contains_key(id)
            implies #[trigger]next[id] === prev[id] && next.contains_key(id)
            by {
            ...
            ==> next.contains_key(id) &&
                (#[trigger]next[id])@.points_to.only_val_updated(prev[id]@.points_to));
            assert(forall |id: int| !locks.contains(id) && prev.contains_key(id)
                ==> #[trigger]next[id] === prev[id] && next.contains_key(id));
        }
    }
}
