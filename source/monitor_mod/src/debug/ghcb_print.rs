...
verus! {
        ...
        (forall |k: int| 2 <= k < (ret.1 as int) ==> ascii_is_num(ret.0@[k]))
    {
        ...
        {
            proof {
                assert(n as u64 / base as u64 <= n as u64 /2) by(nonlinear_arith) requires n >= 0, base > 1;
                bit_shl64_pow2_auto();
                if i < 63 {
...
verus! {
    fn ghcb_prints_with_lock<'a>(s: &StrSlice<'a>, Tracked(cc): Tracked<&mut SnpCoreConsole>) -> (ret: usize)
    requires
        old(cc).wf(),
        s@.len() < MAXU64,
        s.is_ascii(),
        ...
        print_ensures_cc(*old(cc), *cc),
    {
        ...
        let (ret, Tracked(console)) = ghcb_prints_with_lock2(s, Tracked(&mut cc.snpcore), Tracked(console));
        proof{
    ...
    fn ghcb_prints_with_lock2<'a>(s: &StrSlice<'a>, Tracked(snpcore): Tracked<&mut SnpCore>, Tracked(console): Tracked<SnpPointsToRaw>) -> (ret: (usize, Tracked<SnpPointsToRaw>))
    requires
        ...
        print_ensures_snp_c(*old(snpcore), (console), *snpcore, ret.1@),
    {
            ...
            console@.only_val_updated(oldconsole),
        {
            ...
            proof {console = some_console.tracked_unwrap();}
            index = index + len;
        }
...
verus! {
    fn ghcb_print_bytes_with_lock<'a>(s: &[u8_t], Tracked(cc): Tracked<&mut SnpCoreConsole>) -> (ret: usize)
    requires
        old(cc).wf(),
        s@.len() < MAXU64,
    ensures
        ...
        print_ensures_cc(*old(cc), *cc),
    {
        let (ret, Tracked(console)) = ghcb_print_bytes_with_lock2(
            s, Tracked(&mut cc.snpcore), Tracked(cc.console.tracked_remove(0)));
        proof{cc.console.tracked_insert(0, console);}
        ret
    }

    fn ghcb_print_bytes_with_lock2<'a>(s: &[u8_t], Tracked(snpcore): Tracked<&mut SnpCore>, Tracked(console): Tracked<SnpPointsToRaw>) -> (ret: (usize, Tracked<SnpPointsToRaw>))
    requires
        snpcore_console_wf(*old(snpcore), console),
        s@.len() < MAXU64,
    ensures
...
impl<T: ?Sized + VPrint> VPrintLock for T {
    ...
    fn print(&self, Tracked(cs): Tracked<&mut SnpCoreSharedMem>) {
        ...
        let (_, Tracked(console), Tracked(mut consolelock)) = console_ref.acquire(Tracked(consolelock), Tracked(&cs.snpcore.coreid));
        ...
        console_ref.release(Tracked(&mut consolelock), Tracked(console.trusted_into()), Tracked(&cs.snpcore.coreid));
        proof {
