...
verus! {
    ...
    impl RawMemPerms {
        ...
        pub open spec fn deep_equal(self, other: Self) -> bool {
            forall |r|
                #[trigger]self.contains_range(r) ===  #[trigger] other.contains_range(r)
                && self[r] === other[r]
        }

        pub closed spec fn wf(self) -> bool {
            &&& forall |k: int| self.perms.contains_key(k) ==> (#[trigger]self.perms[k])@.range() === (k, 1)
        ...
        pub closed spec fn contains_range(self, range: (int, nat)) -> bool {
            ...
            &&& forall |i: int| within_range(i, range) ==>
                (#[trigger]perms[i])@.snp() === perms[range.0]@.snp()
        }

        pub open spec fn contains_except(&self, range: (int, nat), ranges: Set<(int, nat)>) -> bool {
            forall |r: (int, nat)| inside_range(r, range) && ranges_disjoint(ranges, r) && r.1 != 0
                ==> #[trigger]self.contains_range(r)
        }

        pub open spec fn contains_with_snp_except(&self, range: (int, nat), snp: SwSnpMemAttr, ranges: Set<(int, nat)>) -> bool {
            forall |r: (int, nat)| (inside_range(r, range) && ranges_disjoint(ranges, r) && r.1 != 0)
                ==> (#[trigger]self.contains_range(r) && self[r].snp() === snp)
        }

        #[verifier(inline)]
        pub open spec fn contains_default_except(&self, range: (int, nat), ranges: Set<(int, nat)>) -> bool {
            self.contains_with_snp_except(range, SwSnpMemAttr::spec_default(), ranges)
        }

        #[verifier(inline)]
        pub open spec fn contains_init_except(&self, range: (int, nat), ranges: Set<(int, nat)>) -> bool {
        ...
        pub open spec fn contains_default_ranges(&self, ranges: Set<(int, nat)>) -> bool {
            ...
            &&& forall |r| (ranges.contains(r) && r.1 > 0) ==>
                #[trigger]self.contains_range(r) && self.contains_default_mem(r)
        }

        pub open spec fn no_range(self, range: (int, nat)) -> bool {
            &&& !self.contains_range(range)
            &&& forall |r: (int, nat)| inside_range(r, range) ==>
                !self.contains_range(r)
        }
    ...
    impl RawMemPerms {
        ...
        {
            assert forall |r2| inside_range(r2, r)
            implies self.eq_at(self.restrict(r), r2) by {
            ...
            self.eq_at(self.restrict(r), r2)
        {
            ...
            assert forall |i: int| within_range(i, r2)
            implies
                perms.contains_key(i) == self.perms.contains_key(i) &&
                (perms.contains_key(i) ==> (#[trigger]perms[i]) === self.perms[i])
            by {
            ...
            if restrict.contains_range(r2) {
                assert forall |i: int| within_range(i, r2)
                implies
                    (#[trigger]self.perms[i])@.snp() === self.perms[r2.0]@.snp()
                by {
            ...
            !self.remove_range(r1).contains_range(r2),
        {
            ...
            if ret.contains_range(r2) {
                assert(self.contains_range(r2)) by {
                    ...
                    assert forall |i: int| range2set(r2).contains(i)
                    implies
            ...
            self.remove_range(r1)[r2] === self[r2]
        {
            ...
            assert(self.perms.remove_keys(range2set(r1)).restrict(range2set(r2)) =~~= self.perms.restrict(range2set(r2)));
            let s2 = range2set(r2);
        ...
        ensures
            ...
            range_disjoint_(r1, r2) && self.contains_range(r2) ==> ret.contains_range(r2) && ret[r2] === self[r2],
        ...
        spec fn ext_eq_contains_except(self, ret: Self, r2: (int, nat), rs: Set<(int, nat)>) -> bool {
            forall |r| inside_range(r, r2) && r.1 != 0 && ranges_disjoint(rs, r)
            ==>
                (#[trigger]self.contains_range(r) == #[trigger]ret.contains_range(r)) &&
                (self.contains_range(r) ==> self[r] === ret[r])
        }

        proof fn lemma_except_contains_eq(self, ret: Self, r2: (int, nat), snp: SwSnpMemAttr, rs: Set<(int, nat)>)
        requires
        ...
        ensures
            ...
            (#[trigger]self.contains_with_snp_except(r2, snp, rs) == #[trigger]ret.contains_with_snp_except(r2, snp, rs.insert(range))),
            forall |snp, r2, rs| range_disjoint_(r2, range) ==>
                (self.contains_with_snp_except(r2, snp, rs) == ret.contains_with_snp_except(r2, snp, rs)),
                ...
                (self.contains_with_snp_except(r2, snp, rs) == self.restrict(range).contains_with_snp_except(r2, snp, rs))
        {
            ...
            by {
                ...
                implies
                    ...
                    (self.contains_range(r) ==> self[r] === self.restrict(range)[r])
                by {
                    self.proof_remove_range_ensures(range);
                }
            ...
            assert forall |snp, r2, rs| range_disjoint_(r2, range)
            implies
            (#[trigger]self.contains_with_snp_except(r2, snp, rs) == #[trigger]ret.contains_with_snp_except(r2, snp, rs.insert(range)))
            by {
                assert forall |r| inside_range(r, r2) && r.1 != 0
                implies
                    ranges_disjoint(rs, r) == ranges_disjoint(rs.insert(range), r)
                by{
        ...
        ensures
            ...
            range.1 > 0 ==> forall |r2| inside_range(r2, range) ==> self.eq_at(self.restrict(range), r2),
            //forall |snp, r2, rs| range_disjoint_(r2, range) ==>
            //(self.contains_with_snp_except(r2, snp, rs) == ret.contains_with_snp_except(r2, snp, rs)),
            //forall |snp, r2, rs| inside_range(r2, range) ==>
            //(self.contains_with_snp_except(r2, snp, rs) == self.restrict(range).contains_with_snp_except(r2, snp, rs))
        {
            ...
            assert forall |r: (int, nat)| r.1 > 0 && range_disjoint_(range, r) && self.contains_range(r)
            implies ret.contains_range(r) && ret[r] === self[r]
            by {
                self.lemma_remove_range_ensures(range, r);
            }
            assert forall |r: (int, nat)| r.1 > 0 && !self.contains_range(r)
            implies !ret.contains_range(r)
            by {
                self.lemma_remove_range_ensures(range, r);
            }
            assert forall |r: (int, nat)| inside_range(r, range)
            implies !#[trigger]ret.contains_range(r)
            by {
                self.lemma_remove_range_ensures(range, r);
            }
            if range.1 > 0 {
        ...
        {
            assert forall |i: int| within_range(i, r1)
            implies
                #[trigger]self.perms.contains_key(i) &&
                self.perms[i]@.snp() === self.perms[r1.0]@.snp()
            by {
                assert(within_range(i, r2));
                assert(self.perms.contains_key(i));
        ...
        pub proof fn lemma_contain_except_range_sub(self, r1: (int, nat), r2: (int, nat), rs: Set<(int, nat)>)
        requires
        ...
        {
            assert forall |r| inside_range(r, r1) && r.1 != 0 && ranges_disjoint(rs, r)
            implies #[trigger] self.contains_range(r)
            by {
        ...
        pub proof fn lemma_with_except_sub(self, r1: (int, nat), r2: (int, nat), snp: SwSnpMemAttr, rs: Set<(int, nat)>)
        requires
            self.contains_with_snp_except(r2, snp, rs),
        ...
        {
            assert forall |r| inside_range(r, r1) && r.1 != 0 && ranges_disjoint(rs, r)
            implies self.contains_range(r) && self.contains_snp(r, snp)
            by {
                assert(inside_range(r, r2));
                assert(self.contains_range(r));
            }
            ...
            self.contains_with_snp_except(r1, self[r1].snp(), rs),
        {
            let snp = self[r1].snp();
            assert forall |r| inside_range(r, r1) && r.1 != 0 && ranges_disjoint(rs, r)
            implies self.contains_snp(r, snp)
            by {
        ...
        pub proof fn lemma_empty_contains_except(self, r1: (int, nat), snp: SwSnpMemAttr, rs: Set<(int, nat)>)
        requires
            ...
            self[r3].snp() === self[r1].snp(),
        {
            ...
            assert forall |i: int| within_range(i, ret)
            implies
                self.perms.contains_key(i) &&
                (#[trigger]self.perms[i])@.snp() === self[ret].snp()
            by {
        ...
        pub proof fn lemma_with_except_union(self, r1: (int, nat), r2: (int, nat), snp: SwSnpMemAttr, rs: Set<(int, nat)>) -> (r3: (int, nat))
        requires
            ...
            self.contains_with_snp_except(r3, snp, rs) || r3.1 == 0,
        {
            ...
            assert forall |r| inside_range(r, ret) && r.1 != 0 && ranges_disjoint(rs, r)
            implies self.contains_range(r) && self.contains_snp(r, snp)
            by {
                assert forall |i: int| within_range(i, r)
                implies
                    self.perms.contains_key(i) &&
                    (#[trigger]self.perms[i])@.snp() === snp
                by{
                    assert forall |rr| rs.contains(rr) implies
                        range_disjoint_(rr, (i, 1)) by{}
                ...
                assert(range2set(r).subset_of(self.perms.dom())) by {
                    assert forall |i| within_range(i, r)
                    implies
                        self.perms.contains_key(i)
                    by {
                        assert(self.perms[i]@.snp() === snp);
        ...
        ensures
            ...
            ret@.snp.wf(),
            self.wf(),
            forall |i| (!(range.0 <= i < range.0 + range.1) && old(self).perms.contains_key(i)) ==> self.perms.contains_key(i),
            forall |i| (range.0 <= i < range.0 + range.1) ==> !self.perms.contains_key(i),
            forall |i| !(range.0 <= i < range.0 + range.1) && old(self).perms.contains_key(i) ==> #[trigger]self.perms[i] === old(self).perms[i],
        decreases
            range.1
        {
            let (start, size) = range;
        ...
        pub proof fn tracked_remove(tracked &mut self, range: (int, nat)) -> (tracked ret: SnpPointsToRaw)
        requires
            old(self).contains_range(range),
            range.1 > 0,
            old(self).wf(),
            ...
            self.no_range(range),
        {
            ...
            assert(self.perms.remove_keys(s) =~~= self.perms.remove_keys(s.intersect(self.perms.dom())));
        ...
        pub proof fn lemma_union_propograte_except(
            mem1: Self, mem2: Self,
            range: (int, nat), snp: SwSnpMemAttr,
            except1: Set<(int, nat)>, except2: Set<(int, nat)>)
        requires
            mem1.contains_with_snp_except(range, snp, except1),
            forall |r| except1.contains(r) ==>
                mem2.contains_with_snp_except(r, snp, except2),
        ensures
