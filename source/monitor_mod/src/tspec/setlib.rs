        ...
        s1.union(s2)=~~=(s1.union(s2).union(s2)),
{
    let ss1 = s1;
    let ss2 = s1.union(s2);
    assert forall|a: A| ss1.contains(a)
    implies ss2.contains(a)
    by{}
}
...
#[verifier(inline)]
pub open spec fn set_op<T1, T2, T3>(s1: Set<T1>, s2: Set<T2>, op_fn: spec_fn(T1, T2) -> T3) -> Set<T3> {
...
pub open spec fn set_uop<T1, T2>(s1: Set<T1>, op_fn: spec_fn(T1) -> T2) -> Set<T2> {
    Set::new(|val:T2| exists |v1| s1.contains(v1) && val === op_fn(v1))
    //set_op(s1, Set::empty().insert(arbitrary::<T1>()), uop_to_bop(op_fn))
}

spec fn set_bop_recursive<T1, T2, T3>(s1: Set<T1>, s2: Set<T2>, op_fn: spec_fn(T1, T2) -> T3) -> Set<T3>
recommends
    s1.finite(),
decreases
    s1.len()
{
    if s1.finite() {
        if !s1.is_empty() {
            ...
            if(ss1.len() < s1.len()) {
            set_bop_recursive(ss1, s2, op_fn)
            .union(set_op1(s1.choose(), s2, op_fn))
            } else {
                // unreacheable
                Set::empty()
            }
...
proof fn lemma_setop1<T1, T2, T3>(v1: T1, s2: Set<T2>, op_fn: spec_fn(T1, T2) -> T3) -> (ret: Set<T3>)
requires
    s2.finite(),
ensures
...
proof fn lemma_setop_without_loss_disjoint<T1, T2, T3>(s1: Set<T1>, s2: Set<T2>, vv1: T1, op_fn: spec_fn(T1, T2) -> T3, reverse_op_fn: spec_fn(T3) -> (T1, T2))
requires
    s1.len() > 0,
    s1.contains(vv1),
    ...
    set_op1(vv1, s2, op_fn).disjoint(set_op(s1.remove(vv1), s2, op_fn)),
{
    ...
    assert forall |val|
        ...
        (r2.contains(val) ==> !r1.contains(val))
    by {
        if r1.contains(val) {
            let (v1, v2) = choose |v1, v2| ss1.contains(v1) && s2.contains(v2) && val === op_fn(v1, v2);
...
proof fn lemma_setop_3_union<T1, T2, T3>(s1: Set<T1>, s2: Set<T2>, op_fn: spec_fn(T1, T2) -> T3, vv1: T1) -> (ret: (Set<T3>, Set<T3>, Set<T3>))
requires
    s1.len() > 0,
    s1.contains(vv1),
    s1.finite(),
    ...
    ret.0 =~~= ret.1.union(ret.2),
{
    ...
    let ss1 = s1.remove(vv1);
    assert forall |val| #[trigger]r0.contains(val) == r1.union(r2).contains(val)
    by {
        if r0.contains(val) {
            let (v1, v2) = choose |v1, v2| s1.contains(v1) && s2.contains(v2) && val === op_fn(v1, v2);
...
pub open spec fn exchange_spec_fn<T1, T2, T3>(op_fn: spec_fn(T1, T2) -> T3) -> spec_fn(T2, T1) -> T3 {
    |v1, v2| op_fn(v2, v1)
}

pub proof fn lemma_op_exchange<T1, T2, T3>(s1: Set<T1>, s2: Set<T2>, op_fn: spec_fn(T1, T2) -> T3) -> (ret: Set<T3>)
requires
    s1.len() > 0,
    s1.finite(),
    s2.finite(),
...
proof fn lemma_set_bop_len_recursive<T1, T3, T2>(s1: Set<T1>, s2: Set<T2>, op_fn: spec_fn(T1, T2) -> T3) -> (ret: Set<T3>)
requires
    s1.finite(),
    s2.finite(),
ensures
    ...
    s1.len(),
{
    ...
    if !s1.is_empty() {
        ...
        if s2.is_empty() {
            ...
            assert(!ret.is_empty()) by {
                assert(ret.contains(op_fn(s1.choose(), s2.choose())))
            }
        }
    } else {
...
proof fn lemma_setop_eq_recursive<T1, T2, T3>(s1: Set<T1>, s2: Set<T2>, op_fn: spec_fn(T1, T2) -> T3) -> (ret: Set<T3>)
requires
    s1.finite(),
    s2.finite(),
ensures
...
pub proof fn lemma_setop_len<T1, T2, T3>(s1: Set<T1>, s2: Set<T2>, op_fn: spec_fn(T1, T2) -> T3) -> (ret: Set<T3>)
requires
    s1.finite(),
    s2.finite(),
ensures
