...
#[verifier(opaque)]
pub open spec fn field_at<T: VTypeCast<SecSeqByte>, F: VTypeCast<SecSeqByte>>(val: T, offset: nat) -> F {
...
#[verifier(opaque)]
pub open spec fn field_set<T: VTypeCast<SecSeqByte>, F: VTypeCast<SecSeqByte>>(prev_val: T, val: T, offset: nat, f: F) -> bool {
    let prev_bytes: SecSeqByte = prev_val.vspec_cast_to();
    let bytes: SecSeqByte = val.vspec_cast_to();
    bytes =~~= prev_bytes.take(offset as int) + f.vspec_cast_to() + prev_bytes.skip((offset + spec_size::<F>()) as int)
}

pub proof fn proof_field_set_at<T: SpecSize + IsConstant + VTypeCast<SecSeqByte>, F: SpecSize + IsConstant + VTypeCast<SecSeqByte>>(
    ...
    field_at(val, offset) === f,
{
    ...
    let bytes = (prev_bytes.take(offset as int) + fb + prev_bytes.skip((offset + spec_size::<F>()) as int));
    assert(bytes.subrange(offset as int, (offset + spec_size::<F>()) as int) =~~= fb);
}

pub broadcast proof fn proof_field_set_constant<T: SpecSize + IsConstant + VTypeCast<SecSeqByte>, F: SpecSize + IsConstant + VTypeCast<SecSeqByte>>(
    prev_val: T, val: T, offset: nat, f: F)
requires
    offset < spec_size::<T>(),
    f.is_constant(),
...
#[verifier(external_body)]
pub proof fn proof_into_is_constant_to<T1: VTypeCast<T2> + IsConstant, T2: IsConstant>(v: T1, vmpl: nat)
requires
    1<= vmpl <= 4,
ensures
    ...
    (b1.is_constant_to(vmpl) && b2.is_constant_to(vmpl)) <==> (b1 + b2).is_constant_to(vmpl),
{
    ...
    if (b1.is_constant_to(vmpl) && b2.is_constant_to(vmpl)) {
        assert forall |i| 0 <= i < b.len()
        implies b[i].is_constant_to(vmpl)
        by {
            if i < b1.len() {
                assert(b[i] === b1[i]);
            } else {
