...
verus! {
    ...
    pub open spec fn bool_to_stream(data: bool) -> ByteStream {
        u8_to_stream(if data {1u8} else {0u8})
    }
    ...
    pub open spec fn u64_from_stream(data: ByteStream) -> u64 {
        (u32_from_stream(data.subrange(0, 4)) + u32_from_stream(data.subrange(4, 8)) * 0x10000_0000) as u64
    }

    #[verifier(inline)]
    pub open spec fn u128_from_stream(data: ByteStream) -> u128 {
        (u64_from_stream(data.subrange(0, 8)) + u64_from_stream(data.subrange(8, 16)) * 0x10000_0000_0000_0000) as u128
    }
        ...
        data == 0
    {
        ...
        if data != 0 {
            ...
            implies #[trigger](u32_to_stream(l)[i]) == 0
            by {
                assert(u64_to_stream(data)[i] == 0);
            }
            proof_u32_non_zero(l);
            assert forall |i| 0 <= i < 4
            implies #[trigger](u32_to_stream(h)[i]) == 0
            by {
        ...
        data == 0
    {
        ...
        assert forall |i| 0 <= i < 2
        implies #[trigger](u16_to_stream(l)[i]) == 0
        by {
            assert(u32_to_stream(data)[i] == 0);
        }
        proof_u16_non_zero(l);
        assert forall |i| 0 <= i < 2
        implies #[trigger](u16_to_stream(h)[i]) == 0
        by {
