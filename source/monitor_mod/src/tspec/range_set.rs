    ...
    n,
{
    ...
    if n > 0 {
        ...
        assert(range_to_set(first, n)=~~=(range_to_set(first, (n - 1) as nat).insert(first + (n-1)))) by {
            assert forall |v| range_to_set(first, n).contains(v)
            implies
                range_to_set(first, (n - 1) as nat).contains(v) || v === (first + (n-1))
            by{
            ...
            assert forall |v| range_to_set(first, (n - 1) as nat).contains(v) || v === (first + (n-1))
            implies
                range_to_set(first, n).contains(v)
            by{
            }
        }
    }
    ...
    ranges_disjoint(rs, r2) == ranges_disjoint(rs.insert(range), r2)
{
    ...
    implies
        ranges_disjoint(rs2, r)
    by {
        assert forall |v| rs2.contains(v) implies
            range_disjoint_(v, r)
        by {
            if v !== range {
    ...
    assert forall |r| inside_range(r, r2) && r.1 != 0 && ranges_disjoint(rs.insert(range), r)
    implies
        ranges_disjoint(rs, r)
    by {
        assert forall |v| rs.contains(v) implies
            range_disjoint_(v, r)
        by {
    ...
    ret === range_to_set(f1, n1).disjoint(range_to_set(f2, n2)),
{
    ...
    if ret {
        assert forall |a| set1.contains(a)
        implies !set2.contains(a)
        by {}
    } else {
...
requires
    n1 <= n2,
ensures
    range_to_set(f1, n2)=~~=(range_to_set(f1, n1).union(range_to_set(f1 + n1 as int, (n2 - n1) as nat))),
    range_to_set(f1, n1)=~~=(range_to_set(f1, n2).difference(range_to_set(f1 + n1 as int, (n2 - n1) as nat))),
{
...
ensures
    forall |s1: Set<int>, s2: Set<int>, s3: Set<int>| s1.union(s2).union(s3) === s1.union(s2.union(s3)),
    ...
    forall |s1: Set<int>, s2: Set<int>| s2.subset_of(s1) ==> s1.union(s2) === s1,
{
    ...
    by {
        assert forall |a| s1.union(s2).union(s3).contains(a) === s1.union(s2.union(s3)).contains(a)
        by{}
    }
    ...
    assert forall |s1: Set<int>, s2: Set<int>| s2.subset_of(s1)
    implies s1.union(s2)=~~=(s1)
    by {
