...
pub const SECURITY_MONITOR_GUEST_ID: u64 = 0x123; // Any non-zero values work
...
ensures
    handles.wf(),
    cs.inv(),
    cs.only_lock_reg_coremode_updated(*old(cs), set![GHCB_REGID()], set![spec_PT().lockid(), spec_CONSOLE().lockid()]),
{
    let GhcbHyperPageHandle(ghcb_handle, hyperpage_handle) = handle;
    let ghost cs1 = *cs;
    let ghcb_handle = ghcb_handle.ghcb_write_msr(HV_X64_MSR_GUEST_OS_ID, SECURITY_MONITOR_GUEST_ID, Tracked(cs));
    let ghost cs2 = *cs;
    ...
    proof {
        cs1.lemma_update_prop(cs3, *cs, set![], set![spec_PT().lockid()], set![GHCB_REGID()], set![spec_CONSOLE().lockid()]);
        assert(set![spec_PT().lockid()].union(set![spec_CONSOLE().lockid()]) =~~= set![spec_PT().lockid(), spec_CONSOLE().lockid()]);
