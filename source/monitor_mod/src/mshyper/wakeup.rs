...
impl GhcbHyperPageHandle {
    ...
    {
        let ghost cs1 = (*cs);
        let handle = self.set_vp_reg(HV_REGISTER_VSM_PARTITION_CONFIG, 0x89e, REG_NO_USE_VTL, Tracked(cs));
        let ghost cs2 = (*cs);
        let (val, handle) = handle.get_vp_reg(HV_REGISTER_VSM_PARTITION_CONFIG, REG_NO_USE_VTL, Tracked(cs));
        if val != 0x89e {
        ...
        handle.wf(),
    {
        ...
        let ghost oldcs = (*cs);
        let (mut val, handle) = self.get_vp_reg(HV_REGISTER_GUEST_VSM_PARTITION_CONFIG, REG_NO_USE_VTL, Tracked(cs));
        let ghost prevcs = (*cs);
        val = (val & 0xffff_ffff_ffff_ffcfu64) | 0x10u64;
        let handle = handle.set_vp_reg(HV_REGISTER_GUEST_VSM_PARTITION_CONFIG, val, REG_NO_USE_VTL, Tracked(cs));
        proof {
    ...
    pub fn register_vmsa(self, vmpl: u8, vmsa: VBox<VmsaPage>,
        ...
        ret.1@ === vmsa@,
    {
        ...
        let ghost cs3 = (*cs);
        let mut rmp_attr = RmpAttr::empty().set_vmpl(vmpl as u64).set_vmsa(1).set_perms(0);
        ...
        proof {
            ...
            assert(vmsa_perm@.snp().requires_rmpadjust_mem(vmsa_addr as int, 0, rmp_attr@, Some(nextvmpl_id)));
        }
        assert(vmsa_perm@.wf());
        assert(vmsa_perm@.snp() === SwSnpMemAttr::spec_default());
        let rc = rmpadjust(vmsa_addr, RMP_4K, rmp_attr,
                Tracked(&cs.snpcore),
                Tracked(Some(nextvmpl_id)), Tracked(&mut vmsa_perm));
        if rc != 0 {
    ...
    pub fn snp_start_ap(self,
        cpu: VBox<PerCpuData>,
        gdt: &GDT,
        Tracked(ap_id): Tracked<CoreIdPerm>,
        Tracked(cs): Tracked<&mut SnpCoreSharedMem>) -> (handle: Self)
    requires
        self.wf(),
        old(cs).inv(),
        ap_id@.vmpl == 0,
        ...
        handle.wf()
    {
        ...
        let ghost cs2 = *cs;
        vmsabox.box_update_cs(InitAPParams{
            fun: InitApVmsa, cpu, gdt}, Tracked(cs));
        let ghost cs3 = *cs;
        ...
        let (ap_hyperv_h, bsp_ghcb_h)  = VBox::<OnePage>::new_shared_page(PAGE_SIZE, bsp_ghcb_h, Tracked(cs));
        let ghost cs4 = (*cs);
        ...
        let (_, Tracked(ap_hyperv_perm)) = input_ptr.replace_with::<OnePage>(input, Tracked(ap_hyperv_perm));
        let ap_hyperv_h =  HyperPageHandle::from_raw(ap_hyperv_ptr.to_usize(), Tracked(ap_hyperv_perm));
        ...
        let rc = rmpadjust(
            vmsa_vaddr, 0, rmp_attr,
            Tracked(&mut cs.snpcore),
            Tracked(Some(ap_id)), Tracked(&mut vmsa_perm));
        if rc != 0 {
        ...
        let (ret, bsp_for_ap_handle) = bsp_for_ap_handle.hv_call_with_retry(
            hvcall_code(HVCALL_START_VIRTUAL_PROCESSOR, 0), true, false,
            Tracked(cs)
        );
        match ret {
            Ok(status) if status == 0 => {
        ...
        proof{
            ...
            cs1.lemma_update_prop(cs4, cs5, set![], set![spec_ALLOCATOR_lockid()], set![], set![spec_PT_lockid()]);
            assert(set![spec_ALLOCATOR_lockid()].union(set![spec_PT_lockid()])=~~= set![spec_ALLOCATOR_lockid(), spec_PT_lockid()]);
            cs1.lemma_update_prop(cs5, *cs, set![], set![spec_ALLOCATOR_lockid(), spec_PT_lockid()], set![], set![]);
        }
        ...
        handle.wf(),
    {
        ...
        invariant
            cs.inv(),
            cs.only_lock_reg_coremode_updated(oldcs, set![], set![spec_ALLOCATOR_lockid(), spec_PT_lockid()]),
            handle.wf(),
            ...
            gdt.is_constant(),
        {
            if cpu != BSP as u32 {
                ...
                proof {
                    oldcs.lemma_update_prop(cs1, cs2, set![], set![spec_ALLOCATOR_lockid(), spec_PT_lockid()], set![], set![spec_ALLOCATOR_lockid()]);
                    assert(set![spec_ALLOCATOR_lockid(), spec_PT_lockid()].union(set![spec_ALLOCATOR_lockid()]) =~~= set![spec_ALLOCATOR_lockid(), spec_PT_lockid()]);
                    oldcs.lemma_update_prop(cs2, (*cs), set![], set![spec_ALLOCATOR_lockid(), spec_PT_lockid()], set![], set![spec_ALLOCATOR_lockid(), spec_PT_lockid()]);
                    assert(set![spec_ALLOCATOR_lockid(), spec_PT_lockid()].union(set![spec_ALLOCATOR_lockid(), spec_PT_lockid()]) =~~= set![spec_ALLOCATOR_lockid(), spec_PT_lockid()]);
                    assert forall |i: int|
                        (i != BSP) && i >= (cpu + 1)
                    implies
                        spec_cpumap_contains_cpu(ap_ids, i, 0)
                    by {
...
impl GhcbHandle{
    ...
    pub fn switch_to_next_vmpl(self,
        vmsa: VBox<VmsaPage>,
        Tracked(snpcore): Tracked<&mut SnpCore>) -> (ret: (Self, VBox<VmsaPage>))
    requires
        self.ghcb_wf(),
        vmsa.is_vmsa_page(),
        (*old(snpcore)).inv(),
        ...
        ret.1.is_vmsa_page(),
    {
            ...
            vmsa.is_vmsa_page(),
        {
            ...
            vmgexit(Tracked(&mut ghcb_msr_perm), Tracked(&mut snpcore.coreid), Tracked(&mut op_ghcbpage_perm));
