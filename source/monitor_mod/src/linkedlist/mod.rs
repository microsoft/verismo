...
verus! {
    ...
    {
        pub fn set_next(&self, next: usize_t, Tracked(perm): Tracked<SnpPointsTo<Node<T>>>) -> (newperm: Tracked<SnpPointsTo<Node<T>>>)
        requires
...
impl<T: IsConstant + WellFormed + SpecSize+ VTypeCast<SecSeqByte>> LinkedList<T> {
        ...
        recommends i < self.ptrs@.len()
    {
        ...
        &&& (i > 0 ==> self.perms@[i].view().value().get_Some_0().next as int == self.ptrs@[i-1 as int].ptr.id())
        &&& (i == 0) == (!self.perms@[i].view().value().get_Some_0().next.spec_valid_addr_with(spec_size::<Node<T>>()))
    }
    ...
    pub open spec fn contains_ptr_at(&self, ptr: SnpPPtr<Node<T>>, idx: int) -> bool {
        &&& ptr.not_null() ==>
            (0 <= idx < self@.len() && self@[idx].ptr.id() == ptr.id())
    ...
    T: IsConstant + WellFormed + SpecSize + VTypeCast<SecSeqByte>,
{
        ...
        ensures
            (*self).inv(),
            self@ === old(self)@.push(SpecListItem{ptr, snp: perm@.snp(), val: perm@.value().get_Some_0().val}),
        {
            self.insert(SnpPPtr::nullptr(), ptr, Ghost(self@.len() as int), Tracked(perm));
        }

        pub open spec fn spec_valid_item(&self, ptr: SnpPPtr<Node<T>>, perm: SnpPointsTo<Node<T>>) -> bool {
            let next = perm@.value().get_Some_0().next;
            &&& perm@.ptr_not_null_private_wf(ptr)
            //&&& next.spec_valid_addr_with(spec_size::<Node<T>>())
        }

        pub open spec fn spec_pop(new: Self, olds: Self, ret: Option<(SnpPPtr<Node<T>>, Tracked<SnpPointsTo<Node<T>>>)>) -> bool {
            let len = olds@.len();
            &&& if let Some(ptr) = ret {
                &&& len > 0
                &&& new@ === olds@.drop_last()
        ...
        pub open spec fn spec_pop2(new: Self, olds: Self, ret: Option<(SnpPPtr<Node<T>>, Tracked<SnpPointsTo<Node<T>>>)>) -> bool {
            let len = olds@.len();
            &&& if let Some(ptr) = ret {
                &&& len > 0
                &&& new@ === olds@.drop_last()
            ...
            if self.is_empty() {
                ...
                proof {
                    ...
                    assert(self.wf_perms()) by {
                        assert forall|i: nat| 0 <= i < self.ptrs@.len()
                ...
                assert(ret.get_Some_0().1@ === old(self).perms@[(old(self).ptrs@.len() - 1) as nat]);
                }
        ...
        pub fn remove_iter<F: FnOnce(SnpPPtr<Node<T>>) -> bool + core::marker::Copy>(&mut self, cond_fn: F, max_len: usize) -> (ret: (Self, Ghost<Seq<int>>, Ghost<Seq<int>>))
        requires
            old(self).inv(),
        ...
        ensures
            ...
            ret.0@.len() < max_len as nat ==>
                forall |k: int| 0 <= k < self@.len() ==>
                    cond_fn.ensures((self@[k].ptr, ), false),
            forall |k: int| 0 <= k < ret.0@.len() ==>
                cond_fn.ensures((ret.0@[k].ptr, ), true),
            ...
            ret.0.inv(),
        {
            ...
            invariant
                ...
                forall |k: int| self@.len() - d <= k < self@.len() ==>
                    cond_fn.ensures((self@[k].ptr, ), false),
                forall |k: int| 0 <= k < ret@.len() ==>
                    cond_fn.ensures((ret@[k].ptr, ), true),
                is_subseq_via_index(self@, old(self)@, keep_idx),
                is_subseq_via_index(ret@, old(self)@, removed_idx),
                ret@.len() == 0 ==> (old(self) === self && keep_idx ===  Seq::new(self@.len(), |i: int| i) && removed_idx === Seq::empty()),
                ret@.len() == 1 ==> self@ =~~= old(self)@.remove(removed_idx[0]),
            {
                ...
                proof {
                    ...
                    assert(self.perms@ =~~= self.perms@.remove(i as nat).insert(i as nat, self.perms@[i as nat]));
                }
                assert(self.wf_perm(i as nat));
                ...
                if cond_fn(cptr) {
                    ...
                    if let Option::Some((cur_ptr, cur_perm)) = self._remove(prevptr, cur_ptr, Ghost(i)) {
                        let ghost (mut keep, mut removeditems)  = (prev_self@, ret@);
                        let ghost cur_ptr_perm = SpecListItem{
                            ptr: cur_ptr, snp: cur_perm@@.snp(), val: cur_perm@@.value().get_Some_0().val};
                        proof {
                            assert(self@ =~~= prev_self@.remove(i));
                            if removed == 0 {
                                assert(keep_idx[i] == i);
                            }
                            proof_remove_keep(old(self)@, keep, removeditems, &mut keep_idx, &mut removed_idx, i);
                ...
                proof {
                    d = d + 1;
                    let ghost i = self@.len() - d - 1;
                    assert forall |k: int| i < k < self@.len() implies
                    cond_fn.ensures((self@[k].ptr, ), false) by {
                    ...
                    assert forall |k: int| 0 <= k < ret@.len() implies
                    cond_fn.ensures((ret@[k].ptr, ), true) by {
        ...
        fn _remove(&mut self, prev: SnpPPtr<Node<T>>, cur: SnpPPtr<Node<T>>, Ghost(i): Ghost<int>) -> (ret: Option<(SnpPPtr<Node<T>>, Tracked<SnpPointsTo<Node<T>>>)>)
        requires
            1 <= i + 1 <= old(self)@.len(),
            old(self).perms@[i as nat]@.ptr_not_null_private_wf(cur),
            i + 1 < old(self)@.len() ==> old(self).perms@[(i + 1) as nat]@.ptr_not_null_private_wf(prev),
        ...
        ensures
            ...
            (old(self)@.len() > 0) ==>
                ret === Some((cur, ret.get_Some_0().1)) &&
                ret.get_Some_0().1@ === old(self).perms@[i as nat] &&
                old(self).spec_valid_item(cur, ret.get_Some_0().1@),
        {
            if self.is_empty() {
                return None;
            }
