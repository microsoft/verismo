...
impl RawMemPerms {
    ...
    pub proof fn tracked_split_ranges(tracked &mut self, ranges: Set<(int, nat)>) -> (tracked ret:
        ...
        requires
            forall|r| #[trigger] ranges.contains(r) && r.1 > 0 ==> old(self).contains_range(r),
            old(self).wf(),
        ensures
            self.wf(),
            ret.wf(),
            forall|r| #[trigger] old(self).contains_range(r) ==> self.contains_except(r, ranges),
            forall|r, snp|
            ...
            forall|r| #[trigger] ranges.contains(r) ==> ret.eq_at(*old(self), r),
            forall|r| #[trigger] ranges.contains(r) ==> self.no_range(r),
            forall|r|
                #![trigger ranges_disjoint(ranges, r)]
